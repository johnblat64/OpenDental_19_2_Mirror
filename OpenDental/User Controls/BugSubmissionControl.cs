...
namespace OpenDental.UI {
	public partial class BugSubmissionControl:UserControl {
		...
		private void RefreshViews() {
			...
			_subCur.ListCategoryTags.ForEach(x => listBoxCategories.Items.Add(x));
		...
		private void FillOfficeInfoGrid(BugSubmission sub){
			...
			if(sub!=null) {
				...
				gridOfficeInfo.Rows.Add(new ODGridRow("Other","") { ColorBackG=ODColorTheme.GridHeaderBackBrush.Color,Bold=true,Tag=true });
		...
		private void SetCustomerInfo(BugSubmission sub,bool isCustomerGridRefresh=true) {
			...
			switch(_groupSelection) {
				...
				case 5:
					...
					gridCustomerSubs.Columns.Add(new ODGridColumn("RegKey",0,HorizontalAlignment.Center));
					gridCustomerSubs.Rows.Clear();
					_listSubs.ForEach(x => { 
						Patient rowPat;
