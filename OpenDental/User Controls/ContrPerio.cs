	...
	public class ContrPerio : System.Windows.Forms.Control{
		...
		*/
		...
		private void AdvanceCell(bool isReverse){
			if(!FindNextCell(CurCell,isReverse,out Point pointToSet)) {
				return;
			}
		...
		public bool FindNextCell(Point curCell,bool isReverse,out Point nextCell,bool doSetDirection=true) {
			nextCell=curCell;
			...
			while(limit<400){//the 400 limit is just a last resort. Should always break manually.
				...
				intTooth=GetToothNumFromPoint(newSection,nextCell);
				...
				if(RowTypes[newSection][GetSectionRow(nextCell.Y)]==PerioSequenceType.Mobility){
