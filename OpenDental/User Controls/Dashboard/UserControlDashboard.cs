	...
	public partial class UserControlDashboard:UserControl {
		...
		public void RefreshDashboard() {
			if(_threadRefresh!=null && !_threadRefresh.HasQuit) {
				_threadRefresh.QuitSync(1);//Kill a running refresh/setdata
			}
			StartRefreshThread();
		}

		private void StartRefreshThread() {
			StartRefreshThread((widget) => widget.TryRefreshData(),"DashboardRefresh_Thread");
		}
		...
		private void SetData(ODEventArgs e) {
			...
			StartRefreshThread((widget) => widget.TrySetData(data),"DashboardSet_Thread");
		}
