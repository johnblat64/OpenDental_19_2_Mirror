...
namespace OpenDental {
	public class SheetFiller {
			...
			,StaticTextData staticTextData=null) 
		{
			...
			switch(sheet.SheetType) {
				...
				case SheetTypeEnum.RoutingSlip:
					...
					if(apt==null) {
						return Lan.g("SheetFiller","Appointment no longer exists.");
					}
					patNum=apt.PatNum;
					pat=(pat==null || pat.PatNum!=patNum ? Patients.GetPat(patNum) : pat);
		...
		public static List<long> GetListProcCodeNumsForStaticText(List<long> listIntraoralAndBiteWings,List<long> listOralEvals
			,List<long> listProphy,ref List<long> listIntraoralComplete,ref List<long> listBiteWingSingle,ref List<long> listBiteWingsTwo
			,ref List<long> listBiteWingsThree,ref List<long> listBiteWingsFour,ref List<long> listVertBiteWings7to8,ref List<long> listPerioOralEval
			,ref List<long> listLimitedOralEval,ref List<long> listCompOralEval,ref List<long> listDetailedExtensiveOralEval
			,ref List<long> listPerioMaintenance,ref List<long> listPanoramicFilm,ref List<long> listProphylaxisAdult,ref List<long> listProphylaxisChild
		...
		private static void FillFieldsInStaticText(Sheet sheet,Patient pat,Family fam=null,StaticTextData data=null) {
			...
			if(pat!=null) {;
				...
				listProcCodeNums=GetListProcCodeNumsForStaticText(listIntraoralAndBiteWings,listOralEvals,listProphy,ref listIntraoralComplete
				...
				if(Sheets.ContainsStaticField(sheet,"patientPortalCredentials")) {
					...
					if(UserWebs.UpdateNewPatientPortalCredentials(userWeb)) {
						patientPortalCredentials=Lan.g("PatientPortal","Patient Portal Login")+"\r\n"
							+Lan.g("PatientPortal","UserName:")+" "+userWeb.Item1.UserName+"\r\n"
							+Lan.g("PatientPortal","Password:")+" "+userWeb.Item2;
					}
				}
			...
			foreach(SheetField field in sheet.SheetFields) {
				...
				//ATTENTION: Any static text fields added here must also be added to Sheets.StaticTextData._dictDependencies (exceptions thrown if not).
				...
				if(pat!=null) {
					...
					fldval=fldval.Replace(StaticTextField.gender.ToReplacementString(),Lan.g("enumPatientGender",pat.Gender.ToString()));
