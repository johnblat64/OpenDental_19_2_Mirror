...
namespace OpenDental {
	public partial class SheetEditMobileCtrl:UserControl {
		...
		///<summary>Event thrown when user clicks Add Header button.</summary>
		...
		///<summary>Event thrown when user edits MobileHeader's UiLabelMobile.</summary>
		public event EventHandler<NewMobileFieldValueArgs> NewMobileHeader;
		///<summary>Event thrown when user edits StaticTexts's FieldValue.</summary>
		...
		public SheetDef SheetDef {
			set {
				...
				var mobileFields=_sheetDef.SheetFieldDefs.Where(x => OpenDentBusiness.SheetFieldDefs.IsMobileFieldType(x.FieldType,x.TabOrderMobile,x.FieldName)).ToList();
				...
				var removeSheetDefNums=radioFields1
					...
					.Union(radioFields2.SelectMany(x => x.Items.Select(y => y.SheetFieldDefNum))).ToList();
				...
				foreach(List<SheetFieldDef> sheetFields in finalFields) {
					...
					SheetEditMobilePanel panel=new SheetEditMobilePanel(isReadOnly) {
						...
						IsHeaderValid=!string.IsNullOrEmpty(sheetField.UiLabelMobile),
					...
					var mouseLeave=new EventHandler((sender,e) => SetControlAndChildrenBackColor(panel,_colorPanelBG));
					...
					var dragLeave=new EventHandler((sender,e) => SetControlAndChildrenBackColor(panel,_colorPanelBG));
					...
					if(sheetFields.Count>1 || !string.IsNullOrEmpty(sheetFields[0].RadioButtonValue)) {
						#region RadioGroup
		...
		///If an item is mobile-friendly, it will be given a TabOrderMobile>0 and a non-empty UiLableMobile. TabOrderMobile will repeat for items in the same radio group.
		...
		public bool MergeMobileSheetFieldDefs(SheetDef sheetDef,bool requireValidation,Action<string> err=null) {
			try {
				...
				List<SheetFieldDef> listMobileSheetDefs=SheetFieldDefs;
				...
				var listRemoveStatics=sheetDef.SheetFieldDefs
					.Where(x => x.FieldType==SheetFieldType.StaticText && x.TabOrderMobile<=0)
					.Select(x => x.SheetFieldDefNum);
				//Add in StaticText fields that have a TabOrderMobile but are not already included in our list.
				var listAddStatics=sheetDef.SheetFieldDefs
					.Where(x => x.FieldType==SheetFieldType.StaticText && x.TabOrderMobile>0 && !listMobileSheetDefs.Any(y => y.SheetFieldDefNum==x.SheetFieldDefNum));
				...
				List<SheetFieldDef> listNonMobileSheetDefs=_sheetDef.SheetFieldDefs.Where(x => !sheetDefNumsMobile.Contains(x.SheetFieldDefNum)).ToList();
				...
				if(requireValidation && sheetDef.HasMobileLayout && listMobileSheetDefs.Any(x => {
					...
					if(x.FieldType==SheetFieldType.CheckBox) {
						...
						//If this item is a checkbox and has other checkboxes in its group, then UiLabelMobile (group caption) cannot be empty.
						if(x.FieldName=="misc") {
							if(
								//Has a group.
								!string.IsNullOrEmpty(x.RadioButtonGroup) &&
								//More than 1 item in the group.
								checkBoxes.Count(y => y.CtrlTag.FieldType==SheetFieldType.CheckBox&&y.CtrlTag.RadioButtonGroup==x.RadioButtonGroup)>1&&
								//Has no group caption.
		...
		private void panel_DragDrop(object sender,DragEventArgs e) {
			...
			if(controlDragging is SheetEditMobileRadioButton) {
				...
				panelDrop=(Panel)controlDroppedOnto.Parent;
				...
				SetControlAndChildrenBackColor(panelDrop,_colorPanelBG);
			}
			else {				
				if(controlDroppedOnto!=panelPreview) { //Dropped onto a sub-control so find its position in the panel.
					if(controlDroppedOnto is Panel) { //Dropped onto whitespace on a panel.
						panelDrop=(Panel)controlDroppedOnto;
					}
		...
		public void ShowHideModeless(bool forceOpen,string title,Control owner=null) {
			...
			DockStyle dockBefore=this.Dock;
			AnchorStyles anchorBefore=this.Anchor;
			_formFloat=new Form() {
				MinimizeBox=false,
				MaximizeBox=false,
				ShowIcon=false,
			...
			Control parentBefore=this.Parent;
			//Order matters here. Remove from current parent first, then set new control state, finally add to new parent.
			...
			Rectangle rFloat=System.Windows.Forms.Screen.FromControl(_formFloat).WorkingArea;
			_formFloat.Bounds=new Rectangle(rFloat.Left,0,_formFloat.Width,rFloat.Height);
			if(owner!=null) {
				Rectangle rOwner=System.Windows.Forms.Screen.FromControl(owner).WorkingArea;
				owner.Bounds=new Rectangle(_formFloat.Bounds.Right,0,owner.Width,rOwner.Height);
			}

			_formFloat.Controls.Add(this);
			_formFloat.FormClosing+=new FormClosingEventHandler((sender1,e1) => {
				//Closing the form so re-dock in parent and reset old state.
				_formFloat.Controls.Remove(this);
				this.Dock=dockBefore;
				this.Anchor=anchorBefore;
				this.Location=locationBefore;
				if(parentBefore!=null) {
					this.Size=new Size(parentBefore.Width-locationBefore.X,parentBefore.Height-locationBefore.Y);
					parentBefore.Controls.Add(this);
				}
				_formFloat=null;
			});
			//Non-modal so it floats.
			_formFloat.Show(owner);
		}
		#endregion
	}
