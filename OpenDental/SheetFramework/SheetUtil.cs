	...
	public class SheetUtil {
		...
		private static DataTable GetTable_PayPlanMain(Sheet sheet) {
			...
			for(int i = 0;i<payPlanChargeList.Count;i++) {
				...
				retVal.Rows.Add(PayPlanEdit.CreateRowForPayPlanChargeDT(retVal,payPlanChargeList[i],i-count));
			}
			//===payplan payments===
			if(payPlan.PlanNum==0) {//===normal payplan===
				for(int i = 0;i<payPlanSplitList.Count;i++) {
					retVal.Rows.Add(PayPlanEdit.CreateRowForPayPlanSplitDT(retVal,payPlanSplitList[i],bundledPayments.Rows[i],i));
				}
			}
			else {//===insurance payplan===
				for(int i = 0;i<bundledClaimProcs.Rows.Count;i++) {
					retVal.Rows.Add(PayPlanEdit.CreateRowForClaimProcsDT(retVal,bundledClaimProcs.Rows[i]));
				}
			}
			...
			for(int i = 0;i<payPlanList.Count;i++) {
				...
				sortRetVal.Rows.Add(PayPlanEdit.CreateRowForPayPlanListDT(sortRetVal,payPlanList[i],i-count));
			}
		...
		private static DataTable GetTable_TreatPlanMain(Sheet sheet) {
			...
			foreach(TpRow tpRow in rowsMain){
					...
					|| tpRow.Description==Lan.g("TableTP","Subtotal") || tpRow.Description==Lan.g("TableTP","Total")) 
				{
					...
					dRow["Allowed"]              =tpRow.FeeAllowed.IsGreaterThan(-1) ? tpRow.FeeAllowed.ToString("F") : "X";//-1 means the proc is DoNotBillIns
				}
		...
		public static void FillComboGrowthBehavior(ComboBox combo,GrowthBehaviorEnum growthBehaviorSelected,bool isDynamicSheetType=false,bool isGridCombo=false) {
			...
			combo.SetItems(listGrowthOptions,
				((growthOption) => growthOption.ToString()),//This is what shows in the comboBox
				((growthOption) => growthOption==growthBehaviorSelected)//This determines which item is selected.
			);
		}
		...
		///The logic in this function assume the targetControl is always and completely inside the parentControl.</summary>
		public static void SetControlSizeAndAnchors(SheetFieldDef fieldDef,Control targetControl,Control parentControl,int fillHeightAdj=0) {
