...
namespace OpenDental {
	public class SheetPrinting {
...
#if DEBUG
		private static bool _printCalibration=false;
#endif
		...
		///<summary></summary>
		public static void TryPrint(Sheet sheet, DataSet dataSet, int copies=1, bool isRxControlled=false,Statement stmt=null,MedLab medLab=null,bool isPrintDocument=true,bool isPreviewMode = false) {
			_dataSet=dataSet;
		...
		public static void DrawFieldSpecial(Sheet sheet,SheetField field,Graphics g,XGraphics gx) {
			switch(field.FieldName) {
				...
				case "toothChartLegend":
					...
					DrawToothChartLegend(field.XPos,field.YPos,sheet.Width,_yPosPrint,listDefs,g,gx);
					break;
				default:
					//do nothing
					break;
		...
		///<summary>Generates an Image of the ToothChart.  Set isForSheet=true for dimensions and colors appropriate for Sheets, false for dimensions and
		///colors appropriate for display in Patient Dashboard (matches dimensions for .</summary>
			...
			,bool isInPatientDashboard=false) 
		{
			...
			toothChart.ColorBackground=listDefs[colorBackgroundIndex].ItemColor;
			...
			if(listProceduresFilteredOverride!=null) {
				...
				listProceduresFiltered=FilterProceduresForToothChart(listProceduresAll,treatPlan,showCompleted);
			}
			...
			switch(sheet.SheetType+"."+field.FieldName) {
				case "TreatmentPlan.TreatPlanMain":
					...
					try {
						...
						hasSalesTax=ContrTreat.HasSalesTax(sheet.PatNum,Columns);
					}
					catch {
		...
		private static void DrawImages(Sheet sheet,Graphics graphic,XGraphics xGraphic,bool drawAll=false) {
			...
			foreach(SheetField field in sheet.SheetFields) {
				...
				switch(field.FieldType) {
					case SheetFieldType.Image:
						filePathAndName=FileAtoZ.CombinePaths(SheetUtil.GetImagePath(),field.FieldName);
						break;
					case SheetFieldType.PatImage:
						if(field.FieldValue=="") {
							//There is no document object to use for display, but there may be a baked in image and that situation is dealt with below.
