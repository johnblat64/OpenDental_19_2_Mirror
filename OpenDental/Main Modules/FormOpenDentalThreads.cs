...
namespace OpenDental {
	...
	public partial class FormOpenDental:ODForm {
		...
		private void SetThreads(bool doStart) {
			if(doStart) {
				...
				Enum.GetValues(typeof(FormODThreadNames)).Cast<FormODThreadNames>().ForEach(threadName => {
					switch(threadName) {
						//Do not kill these.
						case FormODThreadNames.CacheFillForFees:
		...
		#region CacheFillForFeesThread
				
		///<summary>Starts a thread that will fill the fee cache while Open Dental is loading. Only allowed to start once.</summary>
		private void BeginCacheFillForFeesThread() {
			if(IsThreadAlreadyRunning(FormODThreadNames.CacheFillForFees)) {
				return;
			}
