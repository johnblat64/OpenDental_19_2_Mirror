	...
	public class ContrImages:System.Windows.Forms.UserControl {
		...
		public void SelectTreeNode(TreeNode node,string localPath="") {
			...
			if(nodeId.NodeType==ImageNodeType.Category) {
				...
				List<Document> listDocs=new List<Document>() { DocSelected };
				ImagesCur=ImageStore.OpenImages(listDocs.ToArray(),PatFolder,localPath);
				...
				PatientDashboardDataEvent.Fire(ODEventType.ModuleSelected
					,new PatientDashboardDataEventArgs() {
						...
						BitmapImagesModule=ImagesCur[0],
					}
				);
				actionCloseDownloadProgress?.Invoke();
				bool isExportable=pictureBoxMain.Visible;
				if(ImagesCur[0]==null) {
					if(ImageHelper.HasImageExtension(DocSelected.FileName)) {
						...
						LoadPdf(PatFolder,DocSelected.FileName,localPath,ref isExportable,"Downloading Document...");
					}
				}
					...
					if(copied) {
						...
						if(FormD.DialogResult!=DialogResult.OK) {
							...
							if(doc.ImgType==ImageType.Photo) {
								PatientEvent.Fire(ODEventType.Patient,PatCur);//Possibly updated the patient picture.
							}
							nodeId=MakeIdDoc(doc.DocNum);
							DocSelected=doc.Copy();
						}
