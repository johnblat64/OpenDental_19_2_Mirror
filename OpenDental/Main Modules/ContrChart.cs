...
namespace OpenDental {
	///<summary></summary>
	public class ContrChart : UserControl	{
		...
		private Label labelSearchClear;
		private ODInternalCustomerGrids odInternalCustomerGrids;
		private MenuItem menuItemSetEC;
		private MenuItem menuItemSetEO;
		private MenuItem menuItemGroupMultiVisit;
		private MenuItem menuItemUngroupMultiVisit;
		private Panel panelToothTrackBar;
		private Panel panelTPpriority;
		...
		private List<DataRow> _procListForGraphical;
		...
		private SheetLayoutController _sheetLayoutControler;
		...
		private void InitializeComponent(){
			...
			this.menuItemUngroupMultiVisit = new System.Windows.Forms.MenuItem();
			this.panelToothTrackBar = new System.Windows.Forms.Panel();
			this.panelTPpriority = new System.Windows.Forms.Panel();
			this.gridProgNav = new OpenDental.UI.ODGridPageNav();
			...
			this.checkAudit.Location = new System.Drawing.Point(169, 165);
			...
			this.checkShowTeeth.Location = new System.Drawing.Point(169, 149);
            ...
            this.menuItemUngroupMultiVisit});
			// 
			// menuItemDelete
			// 
			...
			this.labelSearchClear.Location = new System.Drawing.Point(274, 200);
			...
			this.checkTPChart.Location = new System.Drawing.Point(169, 181);
			...
			this.butShowDateRange.Location = new System.Drawing.Point(133, 157);
			...
			this.textShowDateRange.Location = new System.Drawing.Point(6, 159);
			...
			this.labelCustView.Location = new System.Drawing.Point(160, 217);
			...
			this.textSearch.Location = new System.Drawing.Point(47, 198);
			...
			// 
			this.menuItemGroupMultiVisit.Index = 9;
			...
			// 
			// menuItemUngroupMultiVisit
			// 
			this.menuItemUngroupMultiVisit.Index = 10;
			this.menuItemUngroupMultiVisit.Text = "Ungroup from Multiple Visits";
			this.menuItemUngroupMultiVisit.Click += new System.EventHandler(this.menuItemUngroupMultiVisit_Click);
			// 
			// panelToothTrackBar
			// 
			...
			// odGridPageNav1
			// 
			this.gridProgNav.Anchor = ((System.Windows.Forms.AnchorStyles)(((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left) 
            | System.Windows.Forms.AnchorStyles.Right)));
			this.gridProgNav.BackColor = System.Drawing.Color.Transparent;
			this.gridProgNav.Grid = this.gridProg;
			this.gridProgNav.Location = new System.Drawing.Point(0, 111);
			this.gridProgNav.MinimumSize = new System.Drawing.Size(143, 26);
			this.gridProgNav.Name = "odGridPageNav1";
			this.gridProgNav.Size = new System.Drawing.Size(520, 26);
			this.gridProgNav.TabIndex = 218;
			// 
			// gridProg
			// 
			...
			this.gridProg.EditableEnterMovesDown = false;
			...
			this.panelGridProg.Controls.Add(this.gridProgNav);
		...
		public void LayoutToolBar(){
			...
			if(_sheetLayoutControler!=null && _sheetLayoutControler.ListSheetDefsLayout.Count>0) {
				...
				long selectedLayoutSheetDefNum=(_sheetLayoutControler.SheetDefDynamicLayoutCur?.SheetDefNum??PrefC.GetLong(PrefName.ChartDefaultLayoutSheetDefNum));
				listMenuItems.AddRange(
					_sheetLayoutControler.ListSheetDefsLayout.FindAll(x => x.SheetDefNum>0)//add all custom SheetDefs
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private void menuItemChartBig_Click(object sender,EventArgs e) {
			...
			FormToothChartingBig FormT=new FormToothChartingBig(checkShowTeeth.Checked,ToothInitialList,_procListForGraphical);
			FormT.Show();
		}
		...
		public void FillPtInfo(bool doRefreshData = true) {
			...
			col=new ODGridColumn("",0);//HScrollVisible is false, dynamic col width.
			gridPtInfo.Columns.Add(col);
			gridPtInfo.Rows.Clear();
			ODGridCell cell;
			ODGridRow row;
		...
		public void FillProgNotesInternal(bool retainToothSelection=false,bool doRefreshData=true,bool isSearch=false,bool isForceFirstPage=false) {
			...
			_procListForGraphical=new List<DataRow>();
			...
			if(PatCur!=null) {
				if(gridProg.Tag!=null && ((long)gridProg.Tag)==PatCur.PatNum) {
					isSamePat=true;
				}
				gridProg.Tag=PatCur.PatNum;
			}
			...
			List<DataRow> listDataRows=table.Select()
				.Where(x => DoesGridProgRowPassFilter(x,_loadData.ListProcGroupItems,procNumList))
				.ToList();
			...
			gridProg.FuncConstructGridRow=((t) => GridProgRowConstruction((t as DataRow),fields));
			...
			if(PatCur!=null && gridTreatPlans.SelectedIndices.Length>0) {
			...
			List<string> listProcNumStrs=_procListForGraphical.Select(x => x["ProcNum"].ToString()).ToList();
		...
		///<summary>Mimics old ChartLayoutHelper logic.
		///Called in various places when we want to ensure that checkTreatPlans is shown when it needs to.</summary>
		...
		private bool DoesGridProgRowPassFilter(DataRow rowCur,List<ProcGroupItem> procGroupItems,List<long> procNumList) {
			...
			if(procNumCur!=0) {//if this is a procedure 
				...
				if(rowCur["ProcCode"].ToString()==ProcedureCodes.GroupProcCode && checkShowTeeth.Checked) {
					...
					if(ShouldDisplayProc(rowCur)) {
						_procListForGraphical.Add(rowCur);//show it in the graphical tooth chart
		...
		private ODGridRow GridProgRowConstruction(DataRow rowCur,List<DisplayField> fields) {
			long procNumCur=PIn.Long(rowCur["ProcNum"].ToString());//increase code efficiency
		...
		private void FillTpProcDisplay() {
			...
			gridTpProcs.Columns.Add(new ODGridColumn(Lan.g("TableTP","Description"),0));
		...
		private void FillToothChart(bool retainSelection,DateTime dateLimit){
			...
			if(checkTreatPlans.Checked) {
				...
				foreach(DataRow rowCur in tableCur.Rows) {
						...
						|| listProcNumsAll.Contains(PIn.Long(rowCur["ProcNum"].ToString())))
					{
						...
						_procListForGraphical.Add(rowCur);
					}
				}
			}
			else {
				...
				_procListForGraphical=tableCur.Select().Where(x => listOrigProcNumStrs.Contains(x["ProcNum"].ToString()) && ValidateProcRow(x,dateLimit)).ToList();
			}
			...
			foreach(DataRow row in _procListForGraphical) {
				DateTime dateTP;
		...
		private void DrawProcGraphics(DateTime dateLimit){
			...
			for(int i=0;i<_procListForGraphical.Count;i++) {
				...
				ProcedureCode procCode=ProcedureCodes.GetProcCode(_procListForGraphical[i]["ProcCode"].ToString());
				if(procCode.PaintType==ToothPaintingType.None || procCode.TreatArea==TreatmentArea.Mouth) {
					continue;
				}
				if(procCode.PaintType==ToothPaintingType.Extraction && (
					PIn.Long(_procListForGraphical[i]["ProcStatus"].ToString())==(int)ProcStat.C
					|| PIn.Long(_procListForGraphical[i]["ProcStatus"].ToString())==(int)ProcStat.EC
					|| PIn.Long(_procListForGraphical[i]["ProcStatus"].ToString())==(int)ProcStat.EO
					)) {
				...
				switch(procCode.PaintType){
					case ToothPaintingType.BridgeDark:
						if(ToothInitials.ToothIsMissingOrHidden(_listToothInitialsCopy,_procListForGraphical[i]["ToothNum"].ToString())){
							toothChart.SetPontic(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						}
						else{
							toothChart.SetCrown(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						}
						break;
					case ToothPaintingType.BridgeLight:
						if(ToothInitials.ToothIsMissingOrHidden(_listToothInitialsCopy,_procListForGraphical[i]["ToothNum"].ToString())) {
							toothChart.SetPontic(_procListForGraphical[i]["ToothNum"].ToString(),cLight);
						}
						else {
							toothChart.SetCrown(_procListForGraphical[i]["ToothNum"].ToString(),cLight);
						}
						break;
					...
					case ToothPaintingType.DentureDark:
						if(_procListForGraphical[i]["Surf"].ToString()=="U"){
							teeth=new string[14];
							for(int t=0;t<14;t++){
								teeth[t]=(t+2).ToString();
							}
						}
						else if(_procListForGraphical[i]["Surf"].ToString()=="L") {
							teeth=new string[14];
							for(int t=0;t<14;t++) {
							...
							teeth=_procListForGraphical[i]["ToothRange"].ToString().Split(new char[] {','});
						}
					...
					case ToothPaintingType.Extraction:
						toothChart.SetBigX(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					case ToothPaintingType.FillingDark:
						toothChart.SetSurfaceColors(_procListForGraphical[i]["ToothNum"].ToString(),_procListForGraphical[i]["Surf"].ToString(),cDark);
						break;
				  case ToothPaintingType.FillingLight:
						toothChart.SetSurfaceColors(_procListForGraphical[i]["ToothNum"].ToString(),_procListForGraphical[i]["Surf"].ToString(),cLight);
						break;
					case ToothPaintingType.Implant:
						toothChart.SetImplant(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					case ToothPaintingType.PostBU:
						toothChart.SetBU(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					case ToothPaintingType.RCT:
						toothChart.SetRCT(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					case ToothPaintingType.Sealant:
						toothChart.SetSealant(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
					case ToothPaintingType.Veneer:
						toothChart.SetVeneer(_procListForGraphical[i]["ToothNum"].ToString(),cLight);
						break;
					case ToothPaintingType.Watch:
						toothChart.SetWatch(_procListForGraphical[i]["ToothNum"].ToString(),cDark);
						break;
				}
			}
		}
		...
		private void gridProg_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(row["ProcNum"].ToString()!="0"){
			...
			else if(row["WebChatSessionNum"].ToString()!="0") {
				...
				FormA.ShowDialog();
				if(FormA.CloseOD) {//Only true when using ECW and user clicks FormApptEdit.butComplete.
					this.FindForm().Close();
					return;
				}
		...
		private bool AddProcHelper(Procedure ProcCur,List<Fee> listFees) {
			...
			Procedures.ComputeEstimates(ProcCur,PatCur.PatNum,ref listClaimProcs,true,PlanList,PatPlanList,BenefitList,
				null,null,true,
				PatCur.Age,SubList,
				null,false,false,_listSubstitutionLinks,false,
				listFees);
			listClaimProcs=ClaimProcs.GetForProc(ClaimProcs.Refresh(PatCur.PatNum),ProcCur.ProcNum);
			long verifyCode;
		...
		private void butAddProc_Click(object sender,System.EventArgs e) {
			...
			if(_listSubstitutionLinks==null){
				_listSubstitutionLinks=SubstitutionLinks.GetAllForPlans(PlanList);
			}
			List<Fee> listFees=Fees.GetListFromObjects(new List<ProcedureCode>(){procedureCode },null,//no proc, so medical code won't have changed
				...
				PatCur.PriProv,PatCur.SecProv,PatCur.FeeSched,PlanList,new List<long>(){PatCur.ClinicNum},ApptList.ToList(),_listSubstitutionLinks,PatCur.DiscountPlanNum);
		...
		private void ProcButtonClicked(ProcButton procButton,ProcButtonQuick procButtonQuick=null) {
			...
			if(procButton==null){//Quick Button
				codeList=new long[1];
				codeList[0]=ProcedureCodes.GetCodeNum(procButtonQuick.CodeValue);
				if(codeList[0]==0) {
				...
				codeList=ProcButtonItems.GetCodeNumListForButton(procButton.ProcButtonNum);
				autoCodeList=ProcButtonItems.GetAutoListForButton(procButton.ProcButtonNum);
				//if(codeList.
			}
			...
			for(int i=0;i<codeList.Length;i++){
				listProcedureCodes.Add(ProcedureCodes.GetProcCode(codeList[i]));//could be a harmless empty code
			}
			string toothNum;
			for(int i=0;i<autoCodeList.Length;i++){//this is just a quick loop for fees. The real one is down further
				for(int n=0;n==0 || n<toothChart.SelectedTeeth.Count;n++) {
					isValid=true;
					if(toothChart.SelectedTeeth.Count!=0) toothNum=toothChart.SelectedTeeth[n]; else toothNum="";
					string surf="";
					...
					bool willBeMissing=Procedures.WillBeMissing(toothNum,PatCur.PatNum);//db call, but this NEEDS to happen.
					long codeNum=AutoCodeItems.GetCodeNum(autoCodeList[i],toothNum,surf,isAdditional,PatCur.PatNum,PatCur.Age,willBeMissing);
		...
		private void AddQuadProcs(Procedure procCur,List<Fee> listFees) {
			if(toothChart.SelectedTeeth.Count>0){
				List<string> listSelectedQuads=Tooth.GetQuadsForTeeth(toothChart.SelectedTeeth);
		...
		private void SearchProgNotes() {
			...
			if(_listSearchResults!=null) {
				foreach(DataRow curRow in _listSearchResults) {
					DataRow row=_loadData.TableProgNotes.NewRow();
		...
		///<summary>Searches the given row at the given column for any matching search terms in searchInput. If a match is found, the search term
		///is removed from searchInput.</summary>
			...
			string currentSearchText) 
		{
			...
			List<DisplayField> fields;
			if(ChartViewCurDisplay==null) {//No chart views, Use default values.
				fields=DisplayFields.GetDefaultList(DisplayFieldCategory.None);
			}
			else {
				fields=DisplayFields.GetForChartView(ChartViewCurDisplay.ChartViewNum);
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				List<string> searchInput=new List<string>(listSearchWords);
				if(checkNotes.Checked) {
					CheckForSearchMatch("note",rowCur,ref searchInput);
				}
				for(int f=0;f<fields.Count;f++) {
					switch(fields[f].InternalName) {
						case DisplayFields.InternalNames.ChartView.Date:
							CheckForSearchMatch("procDate",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Time:
							CheckForSearchMatch("procTime",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Th:
							CheckForSearchMatch("toothNum",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Surf:
							CheckForSearchMatch("surf",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Dx:
							CheckForSearchMatch("dx",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Description:
							CheckForSearchMatch("description",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Stat:
							CheckForSearchMatch("procStatus",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Prov:
							CheckForSearchMatch("prov",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Amount:
							CheckForSearchMatch("procFee",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.ProcCode:
							CheckForSearchMatch("ProcCode",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.User:
							CheckForSearchMatch("user",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Signed:
							CheckForSearchMatch("signature",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Priority:
							CheckForSearchMatch("priority",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.DateEntry:
							CheckForSearchMatch("dateEntryC",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Prognosis:
							CheckForSearchMatch("prognosis",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.DateTP:
							CheckForSearchMatch("dateTP",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.EndTime:
							CheckForSearchMatch("procTimeEnd",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Quadrant:
							CheckForSearchMatch("quadrant",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.ScheduleBy:
							CheckForSearchMatch("orionDateScheduleBy",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.StopClock:
							CheckForSearchMatch("orionDateStopClock",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.DPC:
							CheckForSearchMatch("orionDPC",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.EffectiveComm:
							CheckForSearchMatch("orionIsEffectiveComm",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.OnCall:
							CheckForSearchMatch("orionIsOnCall",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Stat2:
							CheckForSearchMatch("orionStatus2",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.DPCpost:
							CheckForSearchMatch("orionDPCpost",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Length:
							CheckForSearchMatch("length",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Abbr: //abbreviation for procedures
							CheckForSearchMatch("AbbrDesc",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Locked:
							CheckForSearchMatch("isLocked",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.HL7Sent:
							CheckForSearchMatch("hl7Sent",rowCur,ref searchInput);
							break;
						case DisplayFields.InternalNames.ChartView.Clinic:
							CheckForSearchMatch("ClinicNum",rowCur,ref searchInput,isClinicAbbr:true);
							break;
						case DisplayFields.InternalNames.ChartView.ClinicDesc:
							CheckForSearchMatch("ClinicNum",rowCur,ref searchInput,isClinicDesc:true);
							break;
						default:
							break;
					}
					if(searchInput.Count==0) {//All the passed in search terms match.
			...
			if(_dateTimeLastSearch==dateTimeStartSearch) {//If the user has not typed something while the search occured
				this.Invoke(() => {
		...
		private void menuItemSetComplete_Click(object sender,EventArgs e) {
			...
			List<DataRow> listSelectedRows=gridProg.SelectedIndices.Where(x => x>-1 && x<gridProg.Rows.Count).Select(x => (DataRow)gridProg.Rows[x].Tag).ToList();
		...
		private bool CanEditProcStatus() {
			//Security check is not done here so we can use the date.
		...
		private void MenuItemSetSelectedProcsStatus(ProcStat newProcStatus) {
			...
			List<ClaimProc> listClaimProcs=ClaimProcs.Refresh(PatCur.PatNum);
			PaySplit[] listPaySplits=PaySplits.Refresh(PatCur.PatNum);
			Adjustment[] listAdjustments=Adjustments.Refresh(PatCur.PatNum);
			foreach(DataRow row in listSelectedRows) {
				...
				if(procCur.AptNum!=0) {//if attached to an appointment
					appt=Appointments.GetOneApt(procCur.AptNum);
					if(appt.AptDateTime.Date > MiscData.GetNowDateTime().Date) {
				...
				if(procDate.Date.Year<1880) {//If not textDate entered or if appt.AptDateTime is invalid.
					procDate=MiscData.GetNowDateTime();
				}
				if(procOld.ProcStatus.In(ProcStat.C,ProcStat.EO,ProcStat.EC)) {
					if(!ProcedureL.CheckPermissionsAndGlobalLockDate(procOld,procCur,procDate)) {
						return;
					}
				}
				...
				if(procOld.ProcStatus==ProcStat.C) {
					...
					if(Adjustments.GetForProc(procCur.ProcNum,listAdjustments).Count!=0
					...
					double sumPaySplits=PaySplits.GetForProc(procCur.ProcNum,listPaySplits).ToArray().ToList().Sum(x => ((PaySplit)x).SplitAmt);
					if(sumPaySplits!=0) {
			...
			foreach(DataRow row in listSelectedRows) {
				...
				if(procNew.ProcStatus==ProcStat.C) {//Proc set complete.
					...
					if(ProcedureCodes.GetStringProcCode(procNew.CodeNum).In("D9986","D9987")) {
						Procedures.SetDateFirstVisit(procNew.ProcDate,2,PatCur);
					}
					procCodeList.Add(ProcedureCodes.GetStringProcCode(procNew.CodeNum));
					#endregion
				}
				...
				procNew.SiteNum=PatCur.SiteNum;
			...
			Recalls.Synch(PatCur.PatNum);
			if(procCodeList.Count>0) {//Only do if we are completing the procedures. 
		...
		private void menuItemGroupMultiVisit_Click(object sender,EventArgs e) {
			...
			if(listProcs.Count<2) {
				MsgBox.Show(this,"At least two procedures must be selected to create a multiple visit group.");
				return;
			}
			List <ProcMultiVisit> listPmvs=ProcMultiVisits.GetGroupsForProcsFromDb(listProcs.Select(x => x.ProcNum).ToArray());
			if(listPmvs.Count > 0) {
		...
		private void menuItemUngroupMultiVisit_Click(object sender,EventArgs e) {
			...
			foreach(int i in gridProg.SelectedIndices) {
				...
				if(procNum==0) {
					MsgBox.Show(this,"Some of the selected items are not procedures.\r\n"
						+"Select only procedures only and try again.");
					return;
				}
				listProcNums.Add(procNum);
			}
			List <ProcMultiVisit> listPmvs=ProcMultiVisits.GetGroupsForProcsFromDb(listProcNums.ToArray());
      ...
      Pen penBlue=new Pen(Color.FromArgb(97,136,173));
		...
		private void tabProc_MouseDown(object sender,MouseEventArgs e) {
			...
			_sheetLayoutControler.RefreshGridVerticalSpace(GetSheetLayoutMode(),GetSheetFieldDefControlDict());
		}
		...
		*/
		...
		///<summary>Returns a dictionary such that the key is a sheetFieldDef.FieldName and the value is a corresponding control from this instance of ContrChart.
		///Dictionary values can be null if control is not matched or it is matched to an HQ only control and not at HQ.</summary>
