...
namespace OpenDental {
	...
	public class ContrAppt:System.Windows.Forms.UserControl {
		...
		///<summary>Important that this is constructed here. Use ContrApptSingle.ResetData() to set it's data fields.  Used when dragging.</summary>
		...
		///<summary>Returns the current selected start date.</summary>
		private DateTime DateStart {
			get {
				DateTime startDate=AppointmentL.DateSelected;
		...
		///<summary>Returns the current selected end date.</summary>
		private DateTime DateEnd {
			get {
				DateTime endDate=AppointmentL.DateSelected;
				if(ApptDrawing.IsWeeklyView) {
					endDate=WeekEndDate;
				}
				return endDate;
			...
			if(disposing) {
				...
				for(int i=0;i<pinBoard.ApptList.Count;i++) {
					...
					if(row.AptStatus==ApptStatus.UnschedList && row.AptDateTime.Year<1880) {
						...
						if(aptCur!=null && aptCur.AptDateTime.Year<1880) {//if the date was not updated since put on the pinboard
		...
		private void InitializeComponent() {
			...
			this.comboView.DropDownClosed += new System.EventHandler(this.ComboView_DropDownClosed);
			// 
			// label2
			// 
		...
		public void ModuleSelected(long patNum,List<long> listPinApptNums=null,List<long> listOpNums=null,List<long> listProvNums=null) {
			...
			LayoutScrollOpProv();
			//There is no "LoadData" in this Module, so at least pass PatCur.
			PatientDashboardDataEvent.Fire(ODEventType.ModuleSelected,PatCur);
		...
		public void LayoutScrollOpProv() {
			...
			if(vScrollBar1.Value > scrollNew) {
				if(scrollNew >= 0) {//but don't allow setting negative number
		...
		///<summary>On right or left click of the operatory panel, display the scheduled provider's information in a new menu.</summary>
		private void OpPanelProv_MouseDown(object sender,MouseEventArgs e) {
			...
			string strProduction=GetOperatoryProduction(ContrApptSheet2.ListContrApptSingles.FindAll(x => x.OpNum==op.OperatoryNum),op);
			if(!string.IsNullOrEmpty(strProduction)) {
				_menuOp.Items.Add(new ToolStripMenuItem(strProduction));
			}
			if(sender.GetType()==typeof(Panel)) {
		...
		private void OpLabel_MouseHover(object sender,EventArgs e) {
			if(sender.GetType()!=typeof(Label)) {
				return;//Event should only happen for labels. Do nothing.
			}
			Label labelHover=(Label)sender;
			object tag=labelHover.Tag;
			if(tag.GetType()!=typeof(Tuple<Provider,Operatory>)) {
				return;
			}
			Tuple<Provider,Operatory> tupleProvOp=(Tuple<Provider,Operatory>)tag;
			...
			string strDisplayMsg=labelHover.Text;//This will be OpName.
			string strProduction=GetOperatoryProduction(ContrApptSheet2.ListContrApptSingles.FindAll(x => x.OpNum==op.OperatoryNum),op);
			if(!string.IsNullOrEmpty(strProduction)) {
				strDisplayMsg+=$"\r\n{strProduction}";
			}
			toolTip1.SetToolTip(labelHover,strDisplayMsg);
		}
		...
		private void ComboView_DropDownClosed(object sender,EventArgs e) {
			//Fixes a bug where opening the Appt Views combobox and highlighting a different view, but instead of clicking on it, the user clicks elsewhere
			//which results in the combobox closing, the highlighted option being selected, but the SelectionChangeCommitted event does not fire.
			//Setting doSetOnNoChange=false results in setting/refreshing the view only when a different Appt View is selected with this workflow.
			ComboViewChanged(false);
		}
		...
		private void ComboViewChanged(bool doSetOnNoChange=true) {
			ApptView viewSelected;
			if(comboView.SelectedIndex==0){
				viewSelected=new ApptView() {
					ApptViewNum=0,//none
				};
			}
			else{
				viewSelected=_listApptViews[comboView.SelectedIndex-1];//-1 for 'none' view (shows in comboView, but not in _listApptViews)
			}
			if(!doSetOnNoChange && viewSelected.ApptViewNum==(ApptViewItemL.ApptViewCur?.ApptViewNum??0)) {
				//Allowing the view to be set when doSetOnNoChange=true (i.e. from SelectionChangeCommitted) maintains current behavior.  The only added
				//behavior from this change is to cause the view to be set when the user opens the combobox, highlights a different view than the currently
				//displayed view, and clicks elsewhere, which causes the combobox to collapse, the selection changes, but the SelectionChangeCommitted event
				//does not fire, which results in an incorrect dataset being loaded for the view.
				return;
			}
			SetView(viewSelected.ApptViewNum,true);
		}
		...
		public void InitializeOnStartup() {
			...
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,"Clear All Blockouts for Day, Op only"),ClearBlockoutsOp_Click);
			menuItem.Name=MenuItemNames.ClearAllBlockoutsForDayOpOnly;
			if(PrefC.HasClinicsEnabled) {
				menuItem=menuBlockout.MenuItems.Add(Lan.g(this,"Clear All Blockouts for Day, Clinic only"),ClearBlockoutsClinic_Click);
				menuItem.Name=MenuItemNames.ClearAllBlockoutsForDayClinicOnly;
			}
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,"Edit Blockout Types"),BlockTypes_Click);
			...
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,"Text ASAP List"),TextASAPList_Click);
			menuItem.Name=MenuItemNames.TextAsapList;
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,MenuItemNames.TextApptsForDayOp),TextApptsForDayOp_Click);
			menuItem.Name=MenuItemNames.TextApptsForDayOp;
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,MenuItemNames.TextApptsForDayView),TextApptsForDayView_Click);
			menuItem.Name=MenuItemNames.TextApptsForDayView;
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,MenuItemNames.TextApptsForDay),TextApptsForDay_Click);
			menuItem.Name=MenuItemNames.TextApptsForDay;
			menuItem=menuBlockout.MenuItems.Add(Lan.g(this,"Update Provs on Future Appts"),UpdateProvs_Click);
		...
		private void SetView(long apptViewNum,bool saveToDb) {
			...
			for(int i=0;i<_listApptViews.Count;i++) {
				if(apptViewNum==_listApptViews[i].ApptViewNum) {
					...
					ApptViewItemL.ApptViewCur=_listApptViews[i];
					break;
				}
			}
			if(!InitializedOnStartup) {
		...
		private ApptView GetApptViewForUser() {
			...
			if(apptViewCur==null //if no entry in the ComputerPref table (or "none" view in ComputerPref table)
				&& _listApptViews.Count>0 //There is an appointment view other than "none" to select
				&& (!InitializedOnStartup //and ContrAppt has NOT been initialized yet.
					|| (PrefC.GetBool(PrefName.EnterpriseNoneApptViewDefaultDisabled) && comboView.SelectedIndex==-1)))
					//or enterprise preference is on to never default to 'None' and no selection has been made thus preserving appointment view bevavior and
			...
			//If apptViewCur==null at this point, the user has explicitly selected the "none" view and we must honor this.
			return apptViewCur;
		}
		...
		private void FillProduction(DateTime start,DateTime end) {
			...
			textProduction.Text=GetProduction(ContrApptSheet2.ListContrApptSingles,listOpsForApptView,listProvNumsForApptView,start,end);
		}

		///<summary>Returns a the production amount for the appointments as a string. Also includes net production. Uses either the listOpNums or 
		///listProvNums to calculate the net production.</summary>
		private string GetProduction(List<ContrApptSingle> listContrApptSingle,List<long> listOpNums,List<long> listProvNums,
			DateTime start,DateTime end) 
		{
			decimal grossproduction=0;
			decimal netproduction=0;
			foreach(ContrApptSingle ctrl in listContrApptSingle) {
				int indexProv=-1;
				if(PrefC.GetBool(PrefName.ApptModuleProductionUsesOps)) {
					if(ctrl.IsHygiene) {
		...
		///<summary>Returns true if the Appointment view has either Production or NetProduction fields.</summary>
		private bool ApptViewHasProduction() {
			return ApptViewItemL.ApptRows.Exists(x => x.ElementDesc.In("Production","NetProduction"));
		}

		///<summary>Returns the production for the operatory appointments as a string.</summary>
		private string GetOperatoryProduction(List<ContrApptSingle> listContrApptSingle,Operatory op) {
			if(!ApptViewHasProduction()) {
				return "";
			}
			...
			return "Production: "+GetProduction(listContrApptSingle.FindAll(x => x.OpNum==op.OperatoryNum),new List<long> { op.OperatoryNum },
				new List<long>(),DateStart,DateEnd);
		}
		...
		private void pinBoard_MouseUp(object sender,MouseEventArgs e) {
			...
			try {
				...
				if(!AppointmentL.IsSpecialtyMismatchAllowed(PatCur.PatNum,aptCur.ClinicNum)) {//ClinicNum just set using either curOp or PatCur.
					return;
				}
				...
				Procedures.SetProvidersInAppointment(aptCur,procsForSingleApt,isUpdatingFees,procFeeHelper);
				pinBoard.ClearSelected();
		...
		private void ContrApptSheet2_MouseUp(object sender,System.Windows.Forms.MouseEventArgs e) {
			...
			try {
				...
				Procedures.SetProvidersInAppointment(apt,procsForApt,isUpdatingFees,procFeeHelper);
				RefreshModuleDataPatient(PatCur.PatNum);
				FormOpenDental.S_Contr_PatientSelected(PatCur,true,false);
				RefreshPeriod();				
		...
		private void InfoBubbleDraw(Point p) {
			...
			if(aptNum!=bubbleAptNum) {
				...
				if(infoBubble.BackgroundImage==null) {
					...
					infoBubble.BackgroundImage=new Bitmap(infoBubble.Width,backImgHeight);
				}
				else {
					...
					infoBubble.Size=new Size(infoBubble.Width,backImgHeight);
				}
		...
		private void ContrApptSheet2_DoubleClick(object sender,System.EventArgs e) {
			...
			//not on apt, so trying to schedule an appointment---------------------------------------------------------------------
				...
				DateTime dateTimeStart=AppointmentL.GetApptDrawingSelectedDateTime();
				...
				if(FormPS.NewPatientAdded) {
					...
					dateTimeStart=AppointmentL.GetApptDrawingSelectedDateTime();
		...
		private void ProcessOtherDlg(OtherResult result,long patNum,string strDateJumpTo,params long[] arraySelectedAptNums) {
			...
			switch(result) {
				...
				case OtherResult.CreateNew:
					...
					Appointment apt=Appointments.GetOneApt(ContrApptSingle.SelectedAptNum);
					if(apt==null) {
						MsgBox.Show(this,"Appointment could not be created.");
						RefreshPeriod();
						break;
					}
					Appointment aptOld=apt.Copy();
		...
		private void OnUnsched_Click() {
			...
			if(PrefC.GetBool(PrefName.UnscheduledListNoRecalls) && Appointments.IsRecallAppointment(apt)) {
			...
			if(AppointmentL.DoPreventChangesToCompletedAppt(apt,PreventChangesApptAction.Unsched)) {
				return;
			}
		...
		private void OnBreak_Click() {
			...
			if(AppointmentL.DoPreventChangesToCompletedAppt(apt,PreventChangesApptAction.Break)) {
				return;
			}
			...
			if(hasBrokenProcs) {//If true, user can not get here from right click 'Break Appointment' directly.
				...
				if(formAB.ShowDialog()!=DialogResult.OK) {
					if(formAB.FormApptBreakSelection==ApptBreakSelection.Delete) {
						...
						OnDelete_Click(true);//Skip the standard "Delete Appointment?" prompt.
					}
					return;
				}
		...
		private void OnDelete_Click(bool isSkipDeletePrompt=false) {
			...
			if(AppointmentL.DoPreventChangesToCompletedAppt(apt,PreventChangesApptAction.Delete)) {
				return;
			}
			if(apt.AptStatus == ApptStatus.PtNote | apt.AptStatus == ApptStatus.PtNoteCompleted) {
		...
		private Schedule GetClickedSchedule(ScheduleType schedType) {
			Schedule[] listForType=Schedules.GetForType(SchedListPeriod,schedType,0);
			DateTime sheetClickedonTime=new DateTime(2000,1,1,SheetClickedonHour,SheetClickedonMin,0);
			Schedule schedCur=null;
			foreach(Schedule sched in listForType) {
				if(!sched.Ops.Contains(SheetClickedonOp)) {
					continue;
				}
				if(sched.SchedDate.Date!=WeekStartDate.AddDays(SheetClickedonDay)) {
					continue;
				}
				if(sched.StartTime <= sheetClickedonTime.TimeOfDay
					&& sheetClickedonTime.TimeOfDay < sched.StopTime) 
				{
					schedCur=sched;
				}
			}
		...
		private void ClearBlockouts_Click(object sender,EventArgs e) {
			if(!Security.IsAuthorized(Permissions.Blockouts)) {
				return;
			}
		...
		private void ClearBlockoutsOp_Click(object sender,EventArgs e) {
			if(!Security.IsAuthorized(Permissions.Blockouts)) {
				return;
			}
		...
		private void ClearBlockoutsClinic_Click(object sender,EventArgs e) {
			if(!Security.IsAuthorized(Permissions.Blockouts)) {
				return;
			}
		...
		private void BlockTypes_Click(object sender,EventArgs e) {
		...
		private void TextASAPList_Click(object sender,EventArgs e) {
			if(PrefC.GetBool(PrefName.WebSchedAsapEnabled)) {
		...
		private void TextApptsForDayOp_Click(object sender,EventArgs e) {
		...
		private void TextApptsForDayView_Click(object sender,EventArgs e) {
			List<long> listPatNums=ContrApptSheet2.ListContrApptSingles.Where(x => x.AptDateTime.Date==GetDateTimeClicked().Date)
		...
		private void TextApptsForDay_Click(object sender,EventArgs e) {
		...
		private void UpdateProvs_Click(object sender,EventArgs e) {
		...
		private class OpPanel {
			...
			public void ResetOpPanel(ContrAppt apptModule,Operatory op,Provider prov,int panelHeight,Point location) {
				...
				_labelOpName.MouseHover-=new EventHandler(apptModule.OpLabel_MouseHover);
				_labelOpName.MouseHover+=new EventHandler(apptModule.OpLabel_MouseHover);
