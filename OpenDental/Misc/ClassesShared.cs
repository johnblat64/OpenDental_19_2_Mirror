	...
	public class GotoModule {
		...
		public static void PinToAppt(List<long> pinAptNums,long patNum) {
			if(PrefC.GetBool(PrefName.ApptModuleUses2019Overhaul)){
				//it might be complicated to try to set the date, and I also don't think anyone wants the date to jump to today.
				OnModuleSelected(new ModuleEventArgs(DateTime.MinValue,pinAptNums,0,0,0,patNum,0));
			}
			else{
