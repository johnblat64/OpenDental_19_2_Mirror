...
namespace OpenDental {
	///<summary></summary>
	public class FormRpProdGoal : ODForm {
		...
		private void RunProdGoal() {
			...
			if(PrefC.HasClinicsEnabled) {
				...
				foreach(Provider prov in listProvs) {
					List<Schedule> listConflicts=Schedules.GetClinicOverlapsForProv(_dateFrom,_dateTo,prov.ProvNum,listClinics.Select(x => x.ClinicNum).ToList());
					if(listConflicts.Count>0) {
						//We have overlaps
						errorMsg=string.Join("\r\n",listConflicts.Select(x => prov.Abbr+" "+x.SchedDate.ToShortDateString()+" "+x.StartTime.ToShortTimeString()
							+" - "+x.StopTime.ToShortTimeString()));
					}
				}
				if(errorMsg!="") {
					errorMsg="This report is designed to show production goals by clinic and provider.  You have one or more providers during the specified "
						+"period that are scheduled in more than one clinic at the same time.  Due to this, production goals cannot be reported accurately.\r\n"
						+"To run this report, please fix your scheduling so each provider is only scheduled at one clinic at a time, or select different providers or clinics.\r\n"
