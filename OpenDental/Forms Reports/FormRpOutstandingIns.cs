...
namespace OpenDental {
	public partial class FormRpOutstandingIns:ODForm {
		...
		private ComboBox comboPreauthOptions;
		...
		private void InitializeComponent() {
			...
			this.comboPreauthOptions = new System.Windows.Forms.ComboBox();
			...
			this.textCarrierPhone = new ValidPhone();
			...
			this.comboBoxMultiClinics.Location = new System.Drawing.Point(620, 38);
			...
			// labelClinic
			// 
			this.labelClinic.Location = new System.Drawing.Point(533, 38);
			...
			// comboPreauthOptions
			// 
			this.comboPreauthOptions.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			this.comboPreauthOptions.FormattingEnabled = true;
			...
			this.dateRangePicker.DefaultDateTimeTo = new System.DateTime(2018, 4, 9, 0, 0, 0, 0);
		...
		private void FormRpOutIns_Load(object sender,EventArgs e) {
			...
			rightClickMenu.Popup+=new EventHandler((o,ea) => {
				...
				if(PrefC.HasClinicsEnabled) {
					...
					rightClickMenu.MenuItems[0].Enabled=(clinicNum.In(_listUserClinics.Select(x => x.ClinicNum)));
				}
			});
			FillCustomTrack();
			FillPreauth();
		...
		private void FillPreauth() {
			foreach(RpOutstandingIns.PreauthOptions preauthOption in Enum.GetValues(typeof(RpOutstandingIns.PreauthOptions))) {
				comboPreauthOptions.Items.Add(new ODBoxItem<RpOutstandingIns.PreauthOptions>(Lan.g(this,preauthOption.GetDescription()),
					preauthOption));
			}
		...
		private void FillClinics() {
			_listUserClinics=Clinics.GetForUserod(Security.CurUser,true,"Unassigned");
			comboBoxMultiClinics.Items.Clear();
			comboBoxMultiClinics.Items.Add(new ODBoxItem<Clinic>("All"));//tag=null
			foreach(Clinic clinic in _listUserClinics) {
		...
		private void FillGrid(bool isOnLoad = false) {
			...
			_preauthOption=comboPreauthOptions.SelectedTag<RpOutstandingIns.PreauthOptions>();
			...
			ODProgress.ShowAction(
				() => { 
					List<RpOutstandingIns.OutstandingInsClaim> listOustandingInsClaims=
						RpOutstandingIns.GetOutInsClaims(_listSelectedProvNums,dateTimeFrom,dateTimeTo,
							_preauthOption,_listSelectedClinicNums,carrier,_listSelectedUserNums,comboFilterBy
					);
					...
					listRows=GetGridRows(listOustandingInsClaims,listDisplayFields,isIgnoreCustomChecked,selectedLastClaimTrackDef,selectedErrorDef);
				},
