	...
	//					  tableOut.Rows[i][j]=CovCats.GetDesc(PIn.PInt(tableOut.Rows[i][j].ToString()));
		...
		private void butPrintPreview_Click(object sender, System.EventArgs e) {
			...
			printPreviewControl2.Zoom=((double)printPreviewControl2.ClientSize.Height
				/ODprintout.DefaultPaperSize.Height);
			labelTotPages.Text="/ "+_totalPages.ToString();
			//We've had problems in the past where FormQuery goes behind all other OD windows at this point. Not sure why, but this will prevent that.
			BringToFront();
			TopMost=true;
			Focus();
			new ODThread((o) => {
				Thread.Sleep(1000);
				this.Invoke(() => TopMost=false);
			}).Start();
		}
		...
		public bool PrintReport(bool justPreview){
			...
			_pagesPrinted=0;
			_linesPrinted=0;
			PrinterL.CreateODprintout(pd2_PrintPage,Lan.g(this,"Query printed"),margins:margins,printoutOrientation:orient);
