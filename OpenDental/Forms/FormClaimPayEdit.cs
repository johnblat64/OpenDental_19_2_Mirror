			...
			for(int i=0;i<_listRequiredFields.Count;i++) {
				...
				switch(_listRequiredFields[i].FieldName) {
					case RequiredFieldName.InsPayEditClinic:
						SetRequiredComboBox(labelClinic,comboClinic,areConditionsMet,-1,"A clinic must be selected.");
						break;
					...
					case RequiredFieldName.DepositAccountNumber:
						...
						SetRequiredComboBox(labelDepositAccountNum,comboDepositAccountNum,areConditionsMet,-1,"An Auto Deposit Account Number must be selected.");
						break;
				}
			}
		}
