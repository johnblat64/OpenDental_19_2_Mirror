...
namespace OpenDental{
	...
	public partial class FormApptEdit : ODForm {
		...
		private void FormApptEdit_Load(object sender, System.EventArgs e) {
			...
			int minPerIncr=10;
			if(PrefC.GetBool(PrefName.ApptModuleUses2019Overhaul)){
				minPerIncr=(int)UserControlApptsPanelJ.GetMinPerIncr();
			}
			else{
				minPerIncr=ApptDrawing.MinPerIncr;
			}
			if(minPerIncr==5) {
		...
		private void CalculateTime(bool ignoreTimeLocked=false) {
			...
			if(apptPattern!=null) {//apptPattern is suppose to be in 5 minute increments. The slider might not be, based on PrefName.AppointmentTimeIncrement.
				strBTime=new StringBuilder(Appointments.ConvertPatternFrom5(apptPattern));
			}
		}
		...
		private bool UpdateListAndDB(bool isClosing=true,bool doCreateSecLog=false,bool doInsertHL7=false) {
				...
				&& !isAuxiliaryRole)//generic HL7 def enabled, appt module hidden and an inbound SIU msg defined, appts created from msgs so no overlap check
			{
				...
				if(PrefC.GetBool(PrefName.ApptModuleUses2019Overhaul)){
					if(ContrApptJ.TryAdjustAppointmentPattern(AptCur)) {
