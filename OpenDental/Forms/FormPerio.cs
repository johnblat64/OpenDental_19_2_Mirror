...
namespace OpenDental{
	...
	/// </summary>
	public partial class FormPerio : ODForm {
		...
		private void ResumePath() {
			...
			//Check teeth 1-16 F
			for(int i=1;i<=16;i++) {
				if(listSkippedTeeth.Contains(i)) {
					continue;
				}
				if(!TryGetNextAvailablePointForTooth(i,true,out Point nextAvailabePoint)) {
					continue;
				}
				gridP.DirectionIsRight=false;
				radioLeft.Checked=true;
				gridP.SetNewCell(nextAvailabePoint.X,nextAvailabePoint.Y);
				return;
			}
			//Check teeth 16-1 L
			for(int i=16;i>=1;i--) {
				...
				if(!TryGetNextAvailablePointForTooth(i,false,out Point nextAvailabePoint)) {
					continue;
				}
		...
		private void GoToSequenceType(PerioSequenceType probeType) {
			int toothNum=_curLocation.ToothNum;
			bool isDirectionIsRightOld=gridP.DirectionIsRight;
			bool isFacial=_curLocation.Surface==PerioSurface.Facial;
			Point toothPoint=GetPointForTooth(toothNum,isFacial);
				...
				|| toothNum!=GetToothNumFromPoint(nextAvailablePoint)) 
			{
				...
				gridP.DirectionIsRight=isDirectionIsRightOld;//change this value back just in case we changed it above.
			}
			else {
		...
		private Point GetPreviousToothPoint(int toothOld,bool isFacial,bool doSetDirection=true) {
			gridP.FindNextCell(GetPointForTooth(toothOld,isFacial),true,out Point prevPoint,doSetDirection);
			return prevPoint;
		}
		...
		private Point GetNextToothPoint(int toothOld,bool isFacial) {
			gridP.FindNextCell(GetPointForTooth(toothOld,isFacial),false,out Point nextPoint);
			return nextPoint;
		}
		...
		private bool TryGetNextAvailablePointForTooth(int toothNum,bool isFacial,out Point nextAvailablePoint) {
			...
			nextAvailablePoint=new Point(FirstEmptyPositionX(pointForTooth,isFacial),pointForTooth.Y);
		...
		private Point NextEmptyPointHelper(Point pointForTooth,bool isReverse) {
			gridP.FindNextCell(pointForTooth,isReverse,out Point nextCell);
		...
		private int FirstEmptyPositionX(Point pointCur,bool isFacial) {
			int xVal=pointCur.X;
			int yVal=pointCur.Y;
			if(isFacial) {
				if(string.IsNullOrEmpty(gridP.GetCellText(xVal,yVal))) {
