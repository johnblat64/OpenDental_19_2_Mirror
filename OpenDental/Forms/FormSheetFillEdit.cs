...
namespace OpenDental {
	...
	public partial class FormSheetFillEdit:ODForm {
		...
		private void RefreshPanel(){
			...
			foreach(SheetField field in SheetCur.SheetFields.Where(x => x.FieldType.In(SheetFieldType.SigBox,SheetFieldType.SigBoxPractice))) {//rectangles around specific output fields
		...
		private bool TryToSaveData() {
			...
			if(IsNewReferralLetter()) {
				...
				doSaveFieldsToDb=false;//When opening this saved sheet, we will display the PDF.
			}
			if(SheetCur.IsNew) {
			...
			if(doSaveFieldsToDb) {
				SheetFields.Sync(SheetCur.SheetFields.FindAll(x => !x.FieldType.In(SheetFieldType.SigBox,SheetFieldType.SigBoxPractice)),SheetCur.SheetNum,false);//Sync fields before sigBoxes
			}
		...
		private void ValidateSaveAndExit() {
			...
			//The DB sheet may not have truely had any changes for current sheet, but someone opened the sheet and clicked OK.
			if(!SheetCur.IsNew
				&& SheetCur.SheetNum!=0
