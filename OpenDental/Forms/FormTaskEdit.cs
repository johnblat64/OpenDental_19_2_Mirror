...
namespace OpenDental {
	...
	public class FormTaskEdit:ODForm {
		...
		private void FillObject() {
			...
			if(_taskCur.ObjectType==TaskObjectType.None) {
				...
				if(_taskCur.KeyNum>0) {
					...
					textObjectDesc.Text=_taskCur.PatientName;
				}
				else {
					textObjectDesc.Text="";
				}
		...
		private void butCreateJob_Click(object sender,EventArgs e) {
			...
			JobCategory category=(JobCategory)Enum.GetNames(typeof(JobCategory)).ToList().IndexOf(categoryChoose.comboSelection.SelectedItem.ToString());
			if(category==JobCategory.SpecialProject && !JobPermissions.IsAuthorized(JobPerm.SpecialProject)) {
				return;
			}
		...
		private void DisableMostControls() {
			...
			this.DisableForm(butCancel,butCopy,textDateTimeEntry,textDateTimeFinished,labelTaskChanged,splitContainerDescriptNote);
