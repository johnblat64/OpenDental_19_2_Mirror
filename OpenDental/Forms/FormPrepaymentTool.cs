...
namespace OpenDental {
	...
	public partial class FormPrepaymentTool:ODForm {
		...
		private void FillGridCodes() {
			...
			ODGridColumn col=new ODGridColumn(Lan.g(this,"Code"),50);
			GridCodes.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Description"),0);
			GridCodes.Columns.Add(col);
			GridCodes.Rows.Clear();
		...
		private void FillGridCompletedProcs() {
			...
			ODGridColumn col=new ODGridColumn(Lan.g(this,"Code"),50,HorizontalAlignment.Center);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Description"),0,HorizontalAlignment.Left);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Units"),30,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Base"),50,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Charge"),75,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Discount"),75,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Est Tax"),75,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			col=new ODGridColumn(Lan.g(this,"Amount"),75,HorizontalAlignment.Right);
			GridCompletedProcs.Columns.Add(col);
			GridCompletedProcs.Rows.Clear();
			foreach(ProcedureCharge proc in _listPreviouslyCompProcedureCharges) {
				...
				GridCompletedProcs.Rows.Add(row);
		...
		private void GridPrepayment_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			FormPrepaymentEdit FormPE=new FormPrepaymentEdit();
		...
		private void CreateProcedureCharge(ProcedureCode ProcCode,int count,bool isNew=true) {
			...
			if(listRcForProc.Count==1) {
				...
				foreach(RepeatCharge charge in listRcForProc) {
					...
					int index=_listProcedureCharge.FindIndex(x => x.BaseFee==charge.ChargeAmt && x.ProcCode==ProcCode);
					//Does not exist in our list, make a new one.
					if(index==-1) {
		...
		private void AddSelectedProcs(int count) {
			foreach(ODGridRow row in GridCodes.SelectedGridRows) {
		...
		private void CreateDiscount(ProcedureCharge proc,int count) {
			...
			proc.ProcDiscount=Math.Round(proc.ProcDiscount,2);
			proc.Calc();
		}
		...
		private void butOK_Click(object sender,EventArgs e) {
			...
			ReturnPayment.PayDate=DateTimeOD.Today;
