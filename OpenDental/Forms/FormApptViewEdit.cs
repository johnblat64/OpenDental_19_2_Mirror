	...
	public class FormApptViewEdit : ODForm {
		...
		private void FormApptViewEdit_Load(object sender, System.EventArgs e) {
			...
			if(!PrefC.HasClinicsEnabled) {
				...
				if(!Security.CurUser.ClinicIsRestricted) {
					comboClinic.Items.Add(Lan.g(this,"All"));//this is really the "unassigned" option
			...
			elementsAll.Add(ApptViewItems.ItemNames.ProphyPerioPastDue);
			elementsAll.Add("Provider");
			elementsAll.Add(ApptViewItems.ItemNames.RecallPastDue);
		...
		private void FillElements(){
			...
			for(int i=0;i<displayedElementsMain.Count;i++){
				...
				else if(displayedElementsMain[i].ElementDesc.In(ApptViewItems.ItemNames.RecallPastDue,ApptViewItems.ItemNames.ProphyPerioPastDue)
					&& displayedElementsMain[i].ElementColor.ToArgb()==Color.Empty.ToArgb()) //Default color is Empty
				{
			...
			for(int i=0;i<displayedElementsUR.Count;i++) {
				...
				else if(displayedElementsUR[i].ElementDesc.In(ApptViewItems.ItemNames.RecallPastDue,ApptViewItems.ItemNames.ProphyPerioPastDue)
					&& displayedElementsUR[i].ElementColor.ToArgb()==Color.Empty.ToArgb()) //Default color is Empty
				{
					row.ColorText=Color.Black;
				}
				else{
			...
			for(int i=0;i<displayedElementsLR.Count;i++) {
				...
				else if(displayedElementsLR[i].ElementDesc.In(ApptViewItems.ItemNames.RecallPastDue,ApptViewItems.ItemNames.ProphyPerioPastDue)
					&& displayedElementsLR[i].ElementColor.ToArgb()==Color.Empty.ToArgb()) //Default color is Empty
				{
					row.ColorText=Color.Black;
				}
				else{
		...
		private void butRight_Click(object sender, System.EventArgs e) {
			if(gridAvailable.GetSelectedIndex()!=-1) {
				...
				string strDescript=elementsAll[displayedAvailable[gridAvailable.GetSelectedIndex()]];
				Color color=Color.Black;
				if(strDescript.In(ApptViewItems.ItemNames.ProphyPerioPastDue,ApptViewItems.ItemNames.RecallPastDue)) {
					color=Color.Empty;//Will use the default color from the graphic
				}
