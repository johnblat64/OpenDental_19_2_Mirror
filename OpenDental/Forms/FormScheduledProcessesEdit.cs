...
namespace OpenDental {
	public partial class FormScheduledProcessesEdit:ODForm {
		...
		private void FormScheduledProcessesEdit_Load(object sender,EventArgs e) {
			comboScheduledAction.SetItemsWithEnum<ScheduledActionEnum>(fSelectItem:x => !_schedProcEdit.IsNew && x==_schedProcEdit.ScheduledAction);
			comboFrequency.SetItemsWithEnum<FrequencyToRunEnum>(fSelectItem: x => !_schedProcEdit.IsNew && x==_schedProcEdit.FrequencyToRun);
			textTimeToRun.Clear();
		...
		private void SetScheduledProcessObject() {
			_schedProcEdit.ScheduledAction=comboScheduledAction.SelectedTag<ScheduledActionEnum>(); 
			_schedProcEdit.FrequencyToRun=comboFrequency.SelectedTag<FrequencyToRunEnum>();
		...
		private bool ValidateFields() {
			...
			if(textTimeToRun.Text=="" || !textTimeToRun.IsEntryValid) {
