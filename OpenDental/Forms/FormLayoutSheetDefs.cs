...
namespace OpenDental {
	...
	public partial class FormLayoutSheetDefs:ODForm {
		...
		private void FillGrid(ODGrid grid,List<SheetDef> listSheetDefs,long selectedSheetNum=-1) {
			grid.BeginUpdate();
			grid.Columns.Clear();
			grid.Columns.Add(new ODGridColumn("Description",0));
			grid.Columns.Add(new ODGridColumn("User",120));
			grid.Rows.Clear();
		...
		private void gridOtherLayouts_Click(object sender,EventArgs e) {
			gridCustomLayouts.SetSelected(false);
		}
		...
		private void gridCustomLayouts_Click(object sender,EventArgs e) {
			gridOtherLayouts.SetSelected(false);
		}
