...
namespace OpenDental {
	public partial class FormEServicesSetup {
		...
		private void FillGridEClipboardSheetInUse() {
			...
			gridEClipboardSheetsInUse.Rows.Clear();
		...
		private void FillGridMobileAppDevices() {
			...
			foreach(MobileAppDevice device in listDevicesToShow) {
				...
				if(_eClipboardAllowEdit) {
					...
					row.Cells.Add(new ODGridCell("Delete",DeleteClick));
				}
				row.Tag=device;
				gridMobileAppDevices.Rows.Add(row);
			}
			gridMobileAppDevices.EndUpdate();
		}

		private void SetEClipboardSheetOrder() {
			if(gridEClipboardSheetsInUse.Rows.Count<1) {
				return;
			}
			int idx=0;
			foreach(EClipboardSheetDef sheetDef in gridEClipboardSheetsInUse.Rows.Select(x => (EClipboardSheetDef)x.Tag)) {
		...
		private void SwapSheets(bool moveUp) {
			...
			if(!moveUp && selectedIdx==gridEClipboardSheetsInUse.Rows.Count-1) {
				return;
			}
			int swapIdx=selectedIdx+(moveUp?-1:1);
			EClipboardSheetDef selectedSheet=(EClipboardSheetDef)gridEClipboardSheetsInUse.Rows[selectedIdx].Tag;
			EClipboardSheetDef swapSheet=(EClipboardSheetDef)gridEClipboardSheetsInUse.Rows[swapIdx].Tag;
		...
		private void ButEClipboardAddSheets_Click(object sender,EventArgs e) {
			FormSheetDefs formSD=new FormSheetDefs(SheetTypeEnum.PatientForm,SheetTypeEnum.MedicalHistory);
			formSD.ShowDialog();
			FillGridEClipboardSheetInUse();
		}
		...
		private void GridEClipboardSheetsInUse_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			EClipboardSheetDef eSheet=(EClipboardSheetDef)gridEClipboardSheetsInUse.Rows[e.Row].Tag;
