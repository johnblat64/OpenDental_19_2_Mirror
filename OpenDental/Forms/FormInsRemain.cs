			...
			if(listPatBenefits.Count > 0) {
				for(int i=0;i<listPatBenefits.Count;i++) {
					if(listPatBenefits[i].CovCatNum==0 //no category
						...
						|| (listPatBenefits[i].TimePeriod != BenefitTimePeriod.CalendarYear //neither calendar year, serviceyear, or 12 months
							&& listPatBenefits[i].TimePeriod != BenefitTimePeriod.ServiceYear
							&& listPatBenefits[i].TimePeriod != BenefitTimePeriod.NumberInLast12Months)
					...
					else if(listPatBenefits[i].TimePeriod==BenefitTimePeriod.NumberInLast12Months) {
						//today - 12 months - 1 day. Procedures exactly 1 year ago are not counted in the range
						listProcs=Procedures.GetCompletedForDateRange(DateTimeOD.Today.AddYears(-1).AddDays(1),DateTimeOD.Today,null,listPatNums);
					}
