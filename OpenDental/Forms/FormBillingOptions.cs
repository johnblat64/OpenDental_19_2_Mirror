...
namespace OpenDental{
	...
	public class FormBillingOptions : ODForm {
		...
		///<summary>A list of clinics that gets filled on load based on the user logged in.  Will be null if clinics is turned off.
		///Can contain a dummy clinic that represents "Unassigned" which will have a ClinicNum of 0.
		///Although this list will not contain a clinic that represents "All", it is an option in comboClinic which has a ClinicNum of -1.</summary>
    ...
    "e the last bill";
			...
			this.gridDun.EditableEnterMovesDown = false;
    ...
    "ly one clinic to see message";
		...
		private void SetClinicFilters() {
			...
			checkUseClinicDefaults.Visible=listSelectedClinicNums.Contains(-1) || listSelectedClinicNums.Count > 1;//Only visible when 'All' or multiple clinics are selected in comboClinic.
		...
		private bool DoSendSms(PatAging patAge,Dictionary<long,PatAgingData> dictPatAgingData) {
			...
			if(GetStatementMode(patAge).In(listModeToText.SelectedTags<StatementMode>())
				&& patAgingData!=null
		...
		private void butCreate_Click(object sender, System.EventArgs e) {
			...
			if(GetSelectedClinicNums().Contains(-1)) {//'All' selected
				...
				ODProgress.ShowAction(() => CreateHelper(GetSelectedClinicNums()[0]),
		...
		///<summary>Generates statements for all clinics passed in. Passing in no list of clinic nums will include all clinics visible to the user.</summary>
		///<param name="listClinicNums">The list of clinic nums to run billing for. If this value is not included, statements will be generated for 
