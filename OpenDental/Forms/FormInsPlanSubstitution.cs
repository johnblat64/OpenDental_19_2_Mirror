...
namespace OpenDental {
	public partial class FormInsPlanSubstitution:ODForm {
		...
		private void AddRow(ODGrid grid,ProcedureCode procCode,SubstitutionLink subLink=null) {
			...
			row.Cells.Add(Lan.g("enumSubstitutionCondition",enumSubstCondition),_listSubConditions.FindIndex(x => x==enumSubstCondition));
		...
		private void gridMain_TitleAddClick(object sender,EventArgs e) {
		...
		private void SortGridByProc(ODGrid grid) {
			List<ODGridRow> listRows=grid.Rows.ToList();
