...
namespace OpenDental{
	...
	public partial class FormClaimEdit : ODForm {
      ...
      ClaimProcList=_loadData.ListClaimProcs;
			...
			if(clearinghouseClin.IsAttachmentSendAllowed && clearinghouseClin.CommBridge==EclaimsCommBridge.ClaimConnect) {
		...
		private void FillGrids(bool doRefreshData=true){
			...
			gridProc.Columns.Add(col);
			col=new ODGridColumn(Lan.g("TableClaimProc","Billed to Ins"),75,HorizontalAlignment.Right);
			gridProc.Columns.Add(col);
			...
			gridProc.Columns.Add(col);
			col=new ODGridColumn(Lan.g("TableClaimProc","Pay Tracking"),90,HorizontalAlignment.Center);
			gridProc.Columns.Add(col);
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("CA") && _dictCanadianLabClaimProcs.ContainsKey(_listClaimProcsForClaim[i].ProcNum)) {//This proc has one or more labs attached.
					foreach(ClaimProc labClaimProc in _dictCanadianLabClaimProcs[_listClaimProcsForClaim[i].ProcNum]) {//Add one row for each attached proc, just below the parent proc.
						...
						if(_listClaimProcsForClaim[i].Status!=labClaimProc.Status//Needed for supplemental payments. Original estimates will show together and all supplementals will show together.
							|| (_listClaimProcsForClaim[i].Status==ClaimProcStatus.Supplemental && _listClaimProcsForClaim[i].DateEntry!=labClaimProc.DateEntry))//Needed when there are multiple supplemental payments.
						{
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private bool TrySelectClaimPayment(List<ClaimPayment> listClaimPayments,out ClaimPayment claimPayment) {
			claimPayment=null;
			List<ODGridColumn> listColumnHeaders=new List<ODGridColumn>() {
				new ODGridColumn(Lan.g(this,"Carrier"),0),
				new ODGridColumn(Lan.g(this,"Check Date"),70,HorizontalAlignment.Center),
				new ODGridColumn(Lan.g(this,"Amount"),80,HorizontalAlignment.Right)
			};
			List<ODGridRow> listRowValues=new List<ODGridRow>();
			listClaimPayments.ForEach(x => {
				ODGridRow row=new ODGridRow(x.CarrierName,x.CheckDate.ToShortDateString(),POut.Double(x.CheckAmt));
				row.Tag=x;
		...
		private bool CheckProviderTerm() {
			List<long> listInvalidProvs=Providers.GetInvalidProvsByTermDate(new List<long> 
				{ _selectedProvBillNum,_selectedProvOrderNum,_selectedProvTreatNum },PIn.DateT(textDateService.Text));
			if(listInvalidProvs.Count==0) {
				return true;
			}
			string message="";
			if(listInvalidProvs.Contains(_selectedProvBillNum) 
				&& listInvalidProvs.Contains(_selectedProvTreatNum)
				&& listInvalidProvs.Contains(_selectedProvOrderNum)) 
			{//Used for grammar
				...
				if(listInvalidProvs.Contains(_selectedProvTreatNum)) {
					if(message!="") {
						message+=" and ";
					}
					message+="Treating Provider";
				}
				if(listInvalidProvs.Contains(_selectedProvOrderNum)) {
					if(message!="") {
						message+=" and ";
					}
				...
				for(int i=0;i<_listClaimProcsForClaim.Count;i++){
					...
					if(ClaimCur.ClaimType=="P" && PatPlanList.Count>0){
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],proc,plan,PatPlanList[0].PatPlanNum,benList,null,null,PatPlanList,0,0,PatCur.Age,0
							,PlanList,SubList,ListSubLinks,false,null);
					}
						...
						ClaimProcs.ComputeBaseEst(_listClaimProcsForClaim[i],proc,plan,PatPlanList[1].PatPlanNum,benList,null,null,PatPlanList,paidOtherInsTotal
							,paidOtherInsTotal,PatCur.Age,writeOffOtherIns,PlanList,SubList,ListSubLinks,false,null);
					}
