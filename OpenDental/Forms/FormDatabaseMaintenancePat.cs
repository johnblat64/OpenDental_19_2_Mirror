...
namespace OpenDental {
	public partial class FormDatabaseMaintenancePat:ODForm {
		...
		private void FillGrid() {
			...
			gridMain.Columns.Add(new ODGridColumn(Lan.g(this,BREAKDOWN_COLUMN_NAME),40,HorizontalAlignment.Center));
			gridMain.Columns.Add(new ODGridColumn(Lan.g(this,RESULTS_COLUMN_NAME),0));
			gridMain.Rows.Clear();
			ODGridRow row;
		...
		private void FillGridOld() {
			...
			gridOld.Columns.Add(new ODGridColumn(Lan.g(this,BREAKDOWN_COLUMN_NAME),40,HorizontalAlignment.Center));
			gridOld.Columns.Add(new ODGridColumn(Lan.g(this,RESULTS_COLUMN_NAME),0));
			gridOld.Rows.Clear();
			ODGridRow row;
		...
		private void Run(ODGrid gridCur,DbmMode modeCur) {
			...
			foreach(ODGridRow row in gridCur.SelectedGridRows) {
				DbmMethodAttr methodAttributes=(DbmMethodAttr)Attribute.GetCustomAttribute((MethodInfo)row.Tag,typeof(DbmMethodAttr));
				...
				UpdateResultTextForRow((int)row.RowNum,result+status,gridCur);
		...
		private List<MethodInfo> DbmMethodsForGridHelper(bool isHidden=false,bool isOld=false) {
			...
			return _listDbmMethodInfos.FindAll(x => x.Name.In(listMethods));
		}

		///<summary>Updates the result column for the specified row in the grid with the text passed in.</summary>
		private void UpdateResultTextForRow(int rowNum,string text,ODGrid gridCur) {
			//RowNum is a long that starts at 1 but our ODgrid helper methods need an int index that starts at 0.
			int index=rowNum-1;
