...
namespace OpenDental {
    ...
    "the insurance EOB.";
            ...
            | System.Windows.Forms.AnchorStyles.Right)));
			this.gridPayments.CellFont = new System.Drawing.Font("Microsoft Sans Serif", 8.5F);
			...
			// butSplitProcs
			// 
			this.butSplitProcs.AdjustImageLocation = new System.Drawing.Point(0, 0);
			...
			this.butSplitProcs.Autosize = true;
			this.butSplitProcs.BtnShape = OpenDental.UI.enumType.BtnShape.Rectangle;
			this.butSplitProcs.BtnStyle = OpenDental.UI.enumType.XPStyle.Silver;
			this.butSplitProcs.CornerRadius = 4F;
		...
		private void FillGridProcedureBreakdown() {
			...
			gridProcedureBreakdown.Columns.Add(new ODGridColumn(insTitle,62,HorizontalAlignment.Right));
			...
			foreach(Hx835_Proc proc in _listAllClaimsPaid.SelectMany(x => x.ListProcs)) {
				...
				row.Cells.Add(new ODGridCell(proc.InsPaid.ToString("f2")));//InsPay or InsEst
		...
		private void FillGridProcedures(){
			...
			ODGridColumn col=new ODGridColumn(Lan.g("TableClaimProc","Split"),30,HorizontalAlignment.Center);//Split proc selection column
		...
		private void HighlightEraProcRow(int gridPaymentRowIndex) {
			...
			foreach(ODGridRow eraProcRow in gridProcedureBreakdown.Rows) {
				Hx835_Proc eraProc=(Hx835_Proc)eraProcRow.Tag;
				ClaimProc matchedClaimProc;
				eraProc.TryGetMatchedClaimProc(out matchedClaimProc,listClaimProcsToMatch,_isSupplementalPay);
				gridProcedureBreakdown.SetSelected((int)eraProcRow.RowNum-1,(matchedClaimProc!=null));
			}
		}
			...
			for(int i=0;i<gridPayments.Rows.Count;i++){
				if(gridPayments.Rows[i].Cells[deductIndex].Text!=""){//deduct
					try{
						...
						throw new ApplicationException(Lan.g(this,"Deductible not valid: ")+gridPayments.Rows[i].Cells[deductIndex].Text);
					}
				}
				if(gridPayments.Rows[i].Cells[allowedIndex].Text!=""){//allowed
					try{
						...
						throw new ApplicationException(Lan.g(this,"Allowed amt not valid: ")+gridPayments.Rows[i].Cells[allowedIndex].Text);
					}
				}
				if(gridPayments.Rows[i].Cells[insPayEstIndex].Text!=""){//inspay
					try{
						...
						throw new ApplicationException(Lan.g(this,"Ins Pay not valid: ")+gridPayments.Rows[i].Cells[insPayEstIndex].Text);
					}
				}
				if(gridPayments.Rows[i].Cells[writeoffIndex].Text!=""){//writeoff
					try{
						...
						if(dbl<0 && !_claimPaid.IsReversal){//Claim reversals have negative writeoffs.
							throw new ApplicationException(Lan.g(this,"Writeoff cannot be negative: ")+gridPayments.Rows[i].Cells[writeoffIndex].Text);
						}
					}
					catch{
						throw new ApplicationException(Lan.g(this,"Writeoff not valid: ")+gridPayments.Rows[i].Cells[writeoffIndex].Text);
					}
				}
			}
		...
		private void butSplitProcs_Click(object sender,EventArgs e) {
			List<ODGridRow> listSelectedPayRows=gridPayments.Rows.Where(x => x.Cells[splitIndex].Text=="X").ToList();//Split column is "checked".
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			if(!PrefC.GetBool(PrefName.EraAllowTotalPayments) && gridPayments.Rows.Select(x => (x.Tag as ClaimProc)).Any(x => x.ProcNum==0 && x.InsPayAmt>0)){
