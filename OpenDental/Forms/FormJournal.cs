	...
	public class FormJournal:ODForm {
		...
		private void FillGrid(bool isPrinting=false,bool isResizing=false) {
			...
			foreach(JournalEntry jeCur in _listJEntries) {
				...
				if(firstVisibleTransNum>0 && jeCur.TransactionNum==firstVisibleTransNum) {
					firstVisibleRowIndex=gridToFill.Rows.Count-1;
				}
				if(selectedTransNum>0 && jeCur.TransactionNum==selectedTransNum) {
					selectedIndex=gridToFill.Rows.Count-1;
				}
			}
		...
		private void FillColumns(bool isPrinting,bool isResizing) {
			...
			if((isPrinting || isResizing) && gridToFill.Columns!=null && gridToFill.Columns.Count>0) {//printing/resizing and cols already filled, adjust widths
				...
				listColHeadings.ForEach(x => gridToFill.Columns.Add(new ODGridColumn(Lan.g("TableJournal",x),dictColWidths[x].Item1,dictColWidths[x].Item2)));
			}
		}
