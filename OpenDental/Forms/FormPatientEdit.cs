	...
	public partial class FormPatientEdit : ODForm {
		...
		private void FormPatientEdit_Load(object sender, System.EventArgs e) {
			...
			if(PatCur!=null){
				...
				foreach(Patient pat in listFamPats){
					if(PatCur.HmPhone!=pat.HmPhone
						|| PatCur.Address!=pat.Address
						|| PatCur.Address2!=pat.Address2
						|| PatCur.City!=pat.City
						|| PatCur.State!=pat.State
						|| PatCur.Zip!=pat.Zip
						|| PatCur.Country!=pat.Country)
					{
						checkSame.Checked=false;
					}
					if(PatCur.CreditType!=pat.CreditType
						|| PatCur.BillingType!=pat.BillingType
						|| PatCur.PriProv!=pat.PriProv
						|| PatCur.SecProv!=pat.SecProv
						|| PatCur.FeeSched!=pat.FeeSched)
					{
						checkBillProvSame.Checked=false;
					}
					if(PatCur.AddrNote!=pat.AddrNote)
					{
						checkNotesSame.Checked=false;
					}
					if(PatCur.WirelessPhone!=pat.WirelessPhone
						|| PatCur.WkPhone!=pat.WkPhone
						|| PatCur.Email!=pat.Email) 
					{
						checkEmailPhoneSame.Checked=false;
					}
					if(PatCur.AskToArriveEarly!=pat.AskToArriveEarly) 
					{
				...
				&& PatCur.PatNum==PatCur.SuperFamily) //Has to be the Super Head.
			{
				...
				checkSameForSuperFam.Checked=Patients.SuperFamHasSameAddrPhone(PatCur,isAuthArchivedEdit);
			}
			...
			foreach(FeeSched feeSched in listFeeScheds) {
				...
				comboFeeSched.Items.Add(new ODBoxItem<FeeSched>(feeSched.Description+(feeSched.IsHidden ? " "+Lans.g(this,"(Hidden)") : ""),feeSched));
			...
			_listCommOptOuts=CommOptOuts.Refresh(PatCur.PatNum);
			foreach(CommOptOutMode mode in Enum.GetValues(typeof(CommOptOutMode))) {
				int idx=comboExcludeECR.Items.Add(new ODBoxItem<CommOptOutMode>(mode.GetDescription(),mode));
				if(_listCommOptOuts.Any(x => x.CommMode==mode)) {
		...
		private void FillComboZip() {
			...
			foreach(ZipCode zip in listZipCodes) {
				comboZip.Items.Add(new ODBoxItem<ZipCode>(zip.ZipCodeDigits+" ("+zip.City+")",zip));
			}
		}
		...
		private void comboZip_SelectionChangeCommitted(object sender, System.EventArgs e) {
			...
			ZipCode selectedZip=comboZip.SelectedTag<ZipCode>();
		...
		private bool IsAnySameChecked() {
			return (checkArriveEarlySame.Checked || checkSame.Checked || checkSameForSuperFam.Checked || checkBillProvSame.Checked || checkNotesSame.Checked
		...
		private void butOK_Click(object sender ,System.EventArgs e) {
			...
			List<CommOptOutMode> listModesOptOut=comboExcludeECR.SelectedTags<CommOptOutMode>();
			CommOptOuts.DeleteMany(_listCommOptOuts.Where(x => !x.CommMode.In(listModesOptOut)).ToList());
			List<CommOptOut> listOptOutsNew=new List<CommOptOut>();
			foreach(CommOptOutMode mode in listModesOptOut.Where(x => !_listCommOptOuts.Any(y => y.CommMode==x))) {
				listOptOutsNew.Add(new CommOptOut {
					CommMode=mode,
					CommType=CommOptOutType.eConfirm,
					PatNum=PatCur.PatNum,
				});
				listOptOutsNew.Add(new CommOptOut {
					CommMode=mode,
					CommType=CommOptOutType.eReminder,
					PatNum=PatCur.PatNum,
				});
			}
			CommOptOuts.InsertMany(listOptOutsNew);
