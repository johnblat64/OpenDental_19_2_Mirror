    ...
    " ID\'s for a default Clearinghouse.";
		...
		private void FillFields() {
			...
			if(ClearinghouseCur.CommBridge.In(EclaimsCommBridge.ClaimConnect,EclaimsCommBridge.EDS,EclaimsCommBridge.Claimstream,EclaimsCommBridge.ITRANS,
				EclaimsCommBridge.EmdeonMedical))
			{
		...
		private void FillListBoxEraBehavior() {
			bool isCanadaEraClearinghouse=comboCommBridge.SelectedTag<EclaimsCommBridge>().In(EclaimsCommBridge.Claimstream,EclaimsCommBridge.ITRANS);
			...
			if(listBoxEraBehavior.SelectedItems.Count==0//No selection made above
