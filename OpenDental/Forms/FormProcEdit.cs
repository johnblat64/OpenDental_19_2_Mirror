...
///<summary></summary>
	public partial class FormProcEdit : ODForm {
		...
		private void FillControlsOnStartup(){
			List<ODBoxItem<ProcStat>> listProcStatItems=ProcedureL.FillProcStatusCombo(_procCur.ProcStatus,_procCur.IsLocked,_procCur.ProcNum);
			comboProcStatus.Items.Clear();
			foreach(ODBoxItem<ProcStat> item in listProcStatItems) {
				comboProcStatus.Items.Add(item);
				if(_procCur.ProcStatus==item.Tag) {
					comboProcStatus.SetSelectedItem<ProcStat>(x => x==item.Tag,"");
				}
			}
			if(comboProcStatus.SelectedTag<ProcStat>()==ProcStat.TPi) {
				comboProcStatus.Enabled=false;
				butSetComplete.Enabled=false;
			}
			if(comboProcStatus.SelectedTag<ProcStat>()==ProcStat.D){//an invalidated proc
				comboProcStatus.Enabled=false;
				butInvalidate.Visible=false;
				butOK.Enabled=false;
				butDelete.Enabled=false;
		...
		private void SetControlsUpperLeft(){
			...
			switch (_procedureCode2.TreatArea){
				...
				case TreatmentArea.ToothRange:
					...
					if(_procCur.ToothRange==null){
						break;
					}
   			  string[] sArray=_procCur.ToothRange.Split(',');//in american
					int idxAmer;
      ...
      SetControlsUpperLeft();
		...
		private void comboProcStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(comboProcStatus.SelectedTag<ProcStat>()==ProcStat.C) {
				...
				if(!isAllowedToCompl) {
					...
					if(_procCur.ProcStatus==ProcStat.TP) {
						...
						{
							comboProcStatus.SetSelectedItem<ProcStat>(x => x==_procCur.ProcStatus,"");
						}
					}
					return;
				}
			...
			ProcStat selecteStat=comboProcStatus.SelectedTag<ProcStat>();
		...
		private void ProcNoteUiHelper() {
			...
			if(_isQuickAdd) {//Quick Procs should insert both TP Default Note and C Default Note.
				procNoteDefault=ProcCodeNotes.GetNote(_selectedProvNum,_procCur.CodeNum,ProcStat.TP);
				if(!string.IsNullOrEmpty(procNoteDefault)) {
					procNoteDefault+="\r\n";
				}
			}
			procNoteDefault+=ProcCodeNotes.GetNote(_selectedProvNum,_procCur.CodeNum,ProcStat.C);
		...
		private void comboStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			switch(comboStatus.SelectedIndex) {
				...
				case 1:
					...
					comboProcStatus.SetSelectedItem<ProcStat>(x => x==ProcStat.C,"");
					_procCur.ProcStatus=ProcStat.C;
					textTimeStart.Text=MiscData.GetNowDateTime().ToShortTimeString();
					break;
				case 2:
					comboProcStatus.SetSelectedItem<ProcStat>(x => x==ProcStat.EO,"");
				...
				case 3:
					...
					break;
				case 4:
					comboProcStatus.SetSelectedItem<ProcStat>(x => x==ProcStat.R,"");
				...
				case 11:
					...
					break;
				case 12:
					comboProcStatus.SetSelectedItem<ProcStat>(x => x==ProcStat.Cn,"");
		...
		private bool EntriesAreValid(){
			...
			if(!ProcedureL.ValidateProvider(_listClaimProcsForProc,_selectedProvNum,_procOld.ProvNum)) {
				return false;
			}
			#endregion
		...
		private void SaveAndClose() {
          ...
          string range="";
			...
			//The actual update----------------------------------------------------------------------------------------------------------------------------------
			Procedures.FormProcEditUpdate(_procCur,_procOld,_procedureCode2,IsNew,listBoxTeeth.Text);
			_listClaimProcsForProc.ForEach(x => x.ClinicNum=_selectedClinicNum);//These changes save in Form_Closing ComputeEstimates depending on DialogResult
			//Recall synch---------------------------------------------------------------------------------------------------------------------------------
