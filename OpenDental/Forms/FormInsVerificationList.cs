...
namespace OpenDental {
	public partial class FormInsVerificationList:ODForm {
		...
		private void FillClinicListBox(List<Clinic> listClinicsForUser) {
			...
			if(listBoxVerifyClinics.SelectedIndex==-1 && listBoxVerifyClinics.Items.Count > 0) {//Select the first one if none are selected.
		...
		private void OnOpenInsPlan() {
			...
			FormInsPlan FormIP=new FormInsPlan(ip,pp,insSub);
			FormIP.ShowDialog();
		...
		private void UpdateSelectedClinicNums() {
			_listClinicNumsVerifyClinicsFilter=listBoxVerifyClinics.SelectedTags<Clinic>().Select(x => x.ClinicNum).ToList();
