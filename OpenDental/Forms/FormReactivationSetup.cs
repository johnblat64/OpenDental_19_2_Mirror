...
///<summary></summary>
	public partial class FormReactivationSetup : ODForm {
		...
		private void FillManualReactivation() {
			...
			comboStatusMailedReactivation.SetItems(listRecallUnschedStatusDefs,(status) => status.ItemName,
				(status) => status.DefNum==PrefC.GetLong(PrefName.ReactivationStatusMailed));
			comboStatusEmailedReactivation.SetItems(listRecallUnschedStatusDefs,(status) => status.ItemName,
				(status) => status.DefNum==PrefC.GetLong(PrefName.ReactivationStatusEmailed));
			comboStatusTextedReactivation.SetItems(listRecallUnschedStatusDefs,(status) => status.ItemName,
				(status) => status.DefNum==PrefC.GetLong(PrefName.ReactivationStatusTexted));
			comboStatusEmailTextReactivation.SetItems(listRecallUnschedStatusDefs,(status) => status.ItemName,
				(status) => status.DefNum==PrefC.GetLong(PrefName.ReactivationStatusEmailedTexted));
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			didChange |= Prefs.UpdateLong(PrefName.ReactivationStatusEmailed,comboStatusEmailedReactivation.SelectedTag<Def>().DefNum);
			didChange |= Prefs.UpdateLong(PrefName.ReactivationStatusMailed,comboStatusMailedReactivation.SelectedTag<Def>().DefNum);
			didChange |= Prefs.UpdateLong(PrefName.ReactivationStatusTexted,comboStatusTextedReactivation.SelectedTag<Def>().DefNum);
			didChange |= Prefs.UpdateLong(PrefName.ReactivationStatusEmailedTexted,comboStatusEmailTextReactivation.SelectedTag<Def>().DefNum);
			if(didChange) {
				DataValid.SetInvalid(InvalidType.Prefs);
			}
