...
///<summary></summary>
	public class FormClaimsSend:ODForm {
		...
		private void GotoAccount_Clicked(object sender, System.EventArgs e){
			...
			ODEvent.Fire(ODEventType.FormClaimSend_GoTo,_arrayQueueFiltered[gridMain.SelectedIndices[0]]);
			SendToBack();
		}
		...
		private void FillGrid(bool rememberSelection){
			...
			gridMain.Rows.Clear();
			ODGridRow row;
			foreach(ClaimSendQueueItem queueItem in _arrayQueueFiltered){
				row=new ODGridRow();
		...
		private void toolBarButPrint_Click(){
			...
			if(gridMain.SelectedIndices.Length==0){
				for(int i=0;i<_arrayQueueFiltered.Length;i++){
					if((_arrayQueueFiltered[i].ClaimStatus=="W" || _arrayQueueFiltered[i].ClaimStatus=="P")	&& !_arrayQueueFiltered[i].CanSendElect) {
						gridMain.SetSelected(i,true);		
					}	
				}
		...
		private void SendEclaimsToClearinghouse(long hqClearinghouseNum) {
			...
			if(gridMain.SelectedIndices.Length==0){//if none are selected
				for(int i=0;i<_arrayQueueFiltered.Length;i++) {//loop through all rows
					if(_arrayQueueFiltered[i].CanSendElect) {
				...
				if(hqClearinghouseNum!=0){//if they used the dropdown list to specify clearinghouse
					...
					for(int i=0;i<selectedindices.Length;i++) {
						...
						if(_arrayQueueFiltered[selectedindices[i]].CanSendElect) {
							//Only change the text to the clearing house name for electronic claims.
							gridMain.Rows[selectedindices[i]].Cells[clearinghouseIndex].Text=clearDefault.Description;
						}
					}
					FillGrid(true);
				}
				...
				if(hqClearinghouseNum!=0) {//if they used the dropdown list to specify clearinghouse
					...
					for(int i=0;i<selectedindices.Length;i++) {
						...
						if(_arrayQueueFiltered[selectedindices[i]].CanSendElect) {
							//Only change the text to the clearing house name for electronic claims.
							gridMain.Rows[selectedindices[i]].Cells[clearinghouseIndex].Text=clearDefault.Description;//show the changed clearinghouse
						}
					}
				}
			}
			RefreshAndValidateSelections();
			if(gridMain.SelectedIndices.Length==0){//No claims selected after Validation
				MsgBox.Show(this,"No claims to send.");
				return;
			}
