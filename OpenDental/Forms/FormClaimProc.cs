...
namespace OpenDental {
	...
	public partial class FormClaimProc : ODForm {
		...
		public void Initialize() {
			...
			else if((ClaimProcCur.Status.In(ClaimProcStatus.CapComplete,ClaimProcStatus.Received,ClaimProcStatus.Supplemental,ClaimProcStatus.InsHist))
			...
			else if(ClaimProcCur.PlanNum>0 && ClaimProcCur.Status.In(ClaimProcStatus.CapEstimate,ClaimProcStatus.CapComplete)) { //not attached to a claim
			...
			if(Plan.PlanType!="c" && ClaimProcOld.Status.In(ClaimProcStatus.Estimate,ClaimProcStatus.InsHist)) {
				comboStatus.Enabled=false;
			}
		...
		private void FillAllowed(){
			if(IsProc){
				decimal allowed=InsPlans.GetAllowedForProc(proc,ClaimProcCur,PlanList,ListSubLinks,null);
				if(allowed==-1){
		...
		private void comboStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(!ClaimProcOld.Status.In(ClaimProcStatus.Estimate,ClaimProcStatus.InsHist)//not an estimate or inshist
