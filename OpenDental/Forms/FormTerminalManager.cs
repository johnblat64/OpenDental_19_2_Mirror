...
namespace OpenDental {
	...
	public partial class FormTerminalManager:ODForm {
		...
		private void FillGrid() {
			...
			gridMain.Columns.Add(new ODGridColumn("Action",50) { TextAlign=HorizontalAlignment.Center });
			if(_isSetupMode) {
				gridMain.Columns.Add(new ODGridColumn("Delete",50) { TextAlign=HorizontalAlignment.Center });
			} 
			gridMain.Rows.Clear();
			foreach(SheetDevice device in listDevices) {
				ODGridRow row=new ODGridRow();
				row.Tag=device;
				row.Cells.Add(new ODGridCell(device.Name));
				row.Cells.Add(new ODGridCell(device.SessionName));
				row.Cells.Add(new ODGridCell(device.PatName));
				if(PrefC.HasClinicsEnabled) {
					row.Cells.Add(new ODGridCell(device.ClinicDesc));
				}
				...
				row.Cells.Add(new ODGridCell(device.PatNum==0?"Load":"Clear",CellClick));
				if(_isSetupMode) {
		...
		private void gridMain_OnSelectionCommitted(object sender,EventArgs e) {
			FillPat();
		}
