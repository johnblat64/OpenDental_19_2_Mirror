...
namespace OpenDental {
	public partial class FormSmsTextMessaging:ODForm {
		...
		private void FillGridTextMessages(bool isRedrawOnly=false,bool retainSort=true) {
			...
			if(!isRedrawOnly) {
				_listSmsFromMobile=SmsFromMobiles.GetMessages(_dateFrom,_dateTo,_listClinicNumsSelected,_patNumCur,false,"",
		...
		private void FillGridMessageThread() {
			...
			List<SmsFromMobile> listSmsFromMobile=SmsFromMobiles.GetMessages(DateTime.MinValue,DateTime.MinValue,_listClinicNumsSelected,
			...
			List<SmsToMobile> listSmsToMobile=SmsToMobiles.GetMessages(DateTime.MinValue,DateTime.MinValue,_listClinicNumsSelected,patNumForDb,
				numberForDb);
