			...
			switch(_listAutoActions[comboAction.SelectedIndex]) {
				...
				case AutomationAction.ChangePatStatus:
					...
					foreach(PatientStatus patStatus in listPatStatus) {
						...
						comboActionObject.Items.Add(new ODBoxItem<PatientStatus>(Lans.g("enum"+nameof(PatientStatus),patStatus.GetDescription()),patStatus));
					}
					comboActionObject.SetSelectedItem<PatientStatus>(x => x==AutoCur.PatStatus,"");
					return;
			}
		}
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			switch(AutoCur.AutoAction) {
				...
				case AutomationAction.ChangePatStatus:
					...
					AutoCur.PatStatus=comboActionObject.SelectedTag<PatientStatus>();
					break;
			}
