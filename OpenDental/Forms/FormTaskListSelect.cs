...
namespace OpenDental {
	...
	public class FormTaskListSelect : ODForm {
		...
		private void LoadUnfilteredTaskLists() {
			switch(OType) {
				case TaskObjectType.Patient:
					_listUnfilteredTaskList=TaskLists.GetForObjectType(OType);
					break;
		...
		private void FillListSubTasks() {
			List<TaskList> listAllTaskLists=TaskLists.GetAllForDateType(TaskDateType.None);
		...
		private void listMain_DoubleClick(object sender, System.EventArgs e) {
			...
			ListSelectedLists=listMain.SelectedTags<TaskList>().Select(x => x.TaskListNum).ToList();
