...
///<summary></summary>
	public class FormInsPlan : ODForm {
		...
		private void InitializeComponent(){
			...
			this.comboSendElectronically = new System.Windows.Forms.ComboBox();
    ...
    "ot delete the plan.";
			...
			// 
			this.comboSendElectronically.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
			this.comboSendElectronically.Location = new System.Drawing.Point(107, 139);
			this.comboSendElectronically.MaxDropDownItems = 30;
			...
			this.butPickCarrier.CornerRadius = 4F;
			...
			this.butDefaultAutoOrthoProc.CornerRadius = 4F;
			...
			this.butPickOrthoProc.CornerRadius = 4F;
    ...
    "t.\n5 lines in 72 vert\n6 in 85";
		...
		private void FillFormWithPlanCur(bool isPicked) {
			...
			FillComboFeeSched(comboCopay,_planCur.CopayFeeSched,listFilteredCopayFeeSched);//Combo Copay Fee Sched
			FillComboFeeSched(comboAllowedFeeSched,_planCur.AllowedFeeSched,FeeSchedsAllowed);//Combo Allowed Fee Sched
		...
		///<summary>Fills the ComboBox with the description from each FeeSched in listFeeScheds and sets the FeeSched corresponding to feeSchedNum 
		///selected.  If feeSchedNum indicates a hidden FeeSched, its description is displayed followed by (hidden).</summary>
		private void FillComboFeeSched(ComboBox comboFeeSched,long feeSchedNum,List<FeeSched> listFeeScheds) {
			comboFeeSched.Items.Clear();
			comboFeeSched.Items.Add(Lan.g(this,"none"));
			comboFeeSched.SelectedIndex=0;
			int idx=-1;
			foreach(FeeSched feeSchedCur in listFeeScheds) {
				comboFeeSched.Items.Add(new ODBoxItem<FeeSched>(feeSchedCur.Description,feeSchedCur));
				if(feeSchedCur.FeeSchedNum==feeSchedNum) {
					idx=comboFeeSched.Items.Count-1;
				}
			}
			if(feeSchedNum==0) {//No fee schedule selected, select 'none' by default.
				idx=0;
			}
			//shows Description (hidden) if _planCur.AllowedFeeSched is a hidden FeeSched
			comboFeeSched.IndexSelectOrSetText(idx,() => FeeScheds.GetDescription(feeSchedNum));
		}
		...
		private void FillCarrier(long carrierNum) {
			...
			FillPayor();
			comboSendElectronically.SetSelectedItem<NoSendElectType>(x => x==_carrierCur.NoSendElect,"");
		}
		...
		private void butHistory_Click(object sender,EventArgs e) {
			FormInsHistSetup FormIHS=new FormInsHistSetup(PatPlanCur.PatNum,_subCur);
		...
		private void butChange_Click(object sender,EventArgs e) {
			...
			FormFamilyMemberSelect FormF=new FormFamilyMemberSelect(fam,true);
			FormF.ShowDialog();
		...
		private void butLabel_Click(object sender,System.EventArgs e) {//TODO: Implement ODprintout pattern
			...
			carrier.NoSendElect=comboSendElectronically.SelectedTag<NoSendElectType>();
			try {
		...
		private void FillBenefits() {
			...
			gridBenefits.Columns.Add(col);
			col=new ODGridColumn("Category",72);
			gridBenefits.Columns.Add(col);
		...
		private bool FillPlanCurFromForm(){
			...
			if(comboFeeSched.SelectedIndex==0) {
				...
				_planCur.FeeSched=comboFeeSched.SelectedTag<FeeSched>().FeeSchedNum;
			}
			if(comboCopay.SelectedIndex==0){
				_planCur.CopayFeeSched=0;//none
			}
			else if(comboCopay.SelectedIndex==-1) {//Hidden fee schedule selected in comboCopay
				if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"The selected Patient Co-pay Amounts fee schedule has been hidden. Are you sure you want to continue?")) {
					return false;
				}
				_planCur.CopayFeeSched=_planCurOriginal.CopayFeeSched;//No change, maintain hidden feesched.
			}
			else{
				_planCur.CopayFeeSched=comboCopay.SelectedTag<FeeSched>().FeeSchedNum;
			}
			if(comboAllowedFeeSched.SelectedIndex==0){
				...
				_planCur.AllowedFeeSched=comboAllowedFeeSched.SelectedTag<FeeSched>().FeeSchedNum;
			}
