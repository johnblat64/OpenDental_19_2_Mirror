...
namespace OpenDental{
	...
	public class FormSheetDefs:ODForm {
		...
		private void InitializeComponent()
		{
			...
			this.listFilter = new System.Windows.Forms.ListBox();
    ...
    "type.\r\nReferral slips are set in the referral edit window of each referral.";
			...
			this.butTools.Location = new System.Drawing.Point(422, 635);
			...
			this.grid1.HideVerticalScroll = false;
			this.grid1.HScrollVisible = false;
			this.grid1.Location = new System.Drawing.Point(12, 69);
			...
			this.grid2.HideVerticalScroll = false;
			...
			this.listFilter.Location = new System.Drawing.Point(398, 87);
			this.listFilter.Name = "listFilter";
			this.listFilter.SelectionMode = System.Windows.Forms.SelectionMode.MultiExtended;
			...
			this.MinimumSize = new System.Drawing.Size(870, 431);
		...
		private void FillGrid1(){
			grid1.BeginUpdate();
			grid1.Columns.Clear();
			grid1.Columns.Add(new ODGridColumn(Lan.g("TableSheetDef","Description"),-1));
			grid1.Columns.Add(new ODGridColumn(Lan.g("TableSheetDef","Type"),100));
			grid1.Rows.Clear();
			foreach(SheetDef internalDef in internalList){
				if(listFilter.SelectedItems.Count>0 && !listFilter.SelectedTags<SheetTypeEnum>().Contains(internalDef.SheetType)) {
					continue;
				}
		...
		private void FillGrid2(long selectedSheetDefNum=-1){
			...
			grid2.BeginUpdate();
			grid2.Columns.Clear();
			grid2.Columns.Add(new ODGridColumn(Lan.g("TableSheetDef","Description"),-1));
			grid2.Columns.Add(new ODGridColumn(Lan.g("TableSheetDef","Type"),100));
			grid2.Columns.Add(new ODGridColumn(Lan.g("TableSheetDef","Use Mobile\r\nLayout"),65,HorizontalAlignment.Center));
			grid2.Rows.Clear();
			int selectedIndex=-1;
			foreach(SheetDef sheetDef in _listSheetDefs){
				if(listFilter.SelectedItems.Count>0 && !listFilter.SelectedTags<SheetTypeEnum>().Contains(sheetDef.SheetType)) {
					continue;
				}
				ODGridRow row=new ODGridRow();
		...
		private void grid1_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			FormSheetDefEdit FormS=new FormSheetDefEdit(grid1.SelectedTag<SheetDef>());
