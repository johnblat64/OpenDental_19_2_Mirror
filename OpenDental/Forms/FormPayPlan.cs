...
namespace OpenDental{
	...
	public class FormPayPlan : ODForm {
		...
		private void butAdd_Click(object sender,System.EventArgs e) {
			PayPlanCharge ppCharge=PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,0,0,DateTime.Today,"");
		...
		private void butPayPlanTx_Click(object sender,EventArgs e) {
			FormPayPlanCredits FormPPC=new FormPayPlanCredits(_payPlanCur,_selectedProvNum);
      ...
      if(_listPayPlanCharges.Count(x => x.ChargeType==PayPlanChargeType.Debit)==0//amortization schedule does not exist
		...
		private PayPlanEdit.PayPlanFrequency GetFrequencyForFormPaymentPlanOptions(FormPaymentPlanOptions options) {
			if(options.radioWeekly.Checked) {
				return PayPlanEdit.PayPlanFrequency.Weekly;
			}
			else if(options.radioEveryOtherWeek.Checked) {
				return PayPlanEdit.PayPlanFrequency.EveryOtherWeek;
			}
			else if(options.radioOrdinalWeekday.Checked) {
				return PayPlanEdit.PayPlanFrequency.OrdinalWeekday;
			}
			else if(options.radioMonthly.Checked) {
				return PayPlanEdit.PayPlanFrequency.Monthly;
			}
			else {
				return PayPlanEdit.PayPlanFrequency.Quarterly;
			}
		}
		...
		private void CreateScheduleCharges(bool isRecalculate) {
			...
			if(downpayment!=0 && !isRecalculate) {
				_listPayPlanCharges.Add(PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,downpayment,0,DateTimeOD.Today
			...
			if(isRecalculate) {
				...
				if(paidTotal>=pastDueTotal) {//Overpaid
					...
					if(_formPayPlanRecalculate.IsPrepay) {
						_listPayPlanCharges.Add(PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,overPaidAmt,0,DateTimeOD.Today
				...
				if(pastDueTotal>paidTotal) {//The patient currently owes more than they have paid.  There is an amount past due.
					...
					if(nextChargeIdx==listPayPlanChargesCopy.Count-1) {//The original payment plan schedule has finished, but patinet still owes monie$.
						_listPayPlanCharges.Add(PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,0,interestInterimAmt,dateFirst
			...
			while(principalDecrementingAmt>0 && chargesCount<payPlanChargesCeiling) {//the ceiling prevents infinite loop
				...
				ppCharge=PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,0,0
				...
				if(isRecalculate && (double)overPaidDecrementingAmt>=ppCharge.Principal+ppCharge.Interest) {//Will only happen for prepay.  Skips a payplan charge.
					...
					_listPayPlanCharges.Add(PayPlanEdit.CreateDebitCharge(_payPlanCur,FamCur,_selectedProvNum,_selectedClinicNum,0,0,ppCharge.ChargeDate
