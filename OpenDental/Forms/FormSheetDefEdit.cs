	...
	public partial class FormSheetDefEdit:ODForm {
		...
		private void DrawGridHelper(SheetFieldDef sheetFieldDef,Graphics g,bool isSelected) {
			...
			switch(sheetFieldDef.FieldName) {
				...
				case "StatementPayPlan":
					...
					g.FillRectangle(Brushes.White,sheetFieldDef.XPos,yPosGrid,odGrid.Width,odGrid.TitleHeight);
				...
				case "Procedures":
					...
					if(sheetFieldDef.GrowthBehavior.In(GrowthBehaviorEnum.FillDown,GrowthBehaviorEnum.FillRightDown,GrowthBehaviorEnum.FillDownFitColumns)) {
						if(sheetFieldDef.GrowthBehavior==GrowthBehaviorEnum.FillDownFitColumns) {
						...
						gridRemainingHeight=Math.Max(1,(panelMain.Height-yPosGrid));
					}
					else {
			...
			if(sheetFieldDef.FieldName=="StatementPayPlan") {
				RectangleF rf=new RectangleF(sheetFieldDef.Width-sheetFieldDef.Width-60,yPosGrid,sheetFieldDef.Width,odGrid.TitleHeight);
		...
		private ODGrid CreateDyanmicGrid(SheetFieldDef fieldDef,List<DisplayField> listColumns) {
			...
			foreach(DisplayField column in listColumns) { 
				...
				odGrid.Columns.Add(new ODGridColumn(colHeader,column.ColumnWidth));
			}
			ODGridRow row=new ODGridRow();//Add empty row
		...
		private void FormSheetDefEdit_KeyDown(object sender,KeyEventArgs e) {
			List<SheetFieldDef> listSheetFieldDefs=listFields.SelectedTags<SheetFieldDef>();
			bool refreshBuffer=false;
			e.Handled=true;
