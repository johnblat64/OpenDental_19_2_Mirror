...
namespace OpenDental {
	public partial class FormArManager:ODForm {
		...
		private void FormArManager_Load(object sender,EventArgs e) {
			...
			if(PrefC.HasClinicsEnabled) {
				comboBoxMultiExcludedClinics.Visible=true;
				labelExcludedClinics.Visible=true;
				comboBoxMultiExcludedClinics.Visible=true;
				labelExcludedClinics.Visible=true;
				comboBoxMultiExcludedClinics.Items.Add(Lan.g(this,"All"));
				if(Clinics.ClinicNum==0) {
					comboBoxMultiExcludedClinics.SetSelected(0,true);
				}
				foreach(Clinic clinCur in _listClinics) {
					comboBoxMultiExcludedClinics.Items.Add(clinCur.Abbr);
					if(Clinics.ClinicNum>0 && clinCur.ClinicNum==Clinics.ClinicNum) {
						comboBoxMultiExcludedClinics.SetSelected(comboBoxMultiExcludedClinics.Items.Count-1,true);
					}
				}
				if(comboBoxMultiExcludedClinics.SelectedIndices.Count==0) {
					comboBoxMultiExcludedClinics.SetSelected(0,true);
				}
			}
...
//TODO
		...
		private void butSend_Click(object sender,EventArgs e) {
			...
			Dictionary<long,string[]> dictClinicSelectedServices=_dictClinicProgProps
				.Where(x => x.Value.Any(y => y.PropertyDesc=="SelectedServices" && !string.IsNullOrEmpty(y.PropertyValue)))
				.ToDictionary(x => x.Key,x => x.Value.Find(y => y.PropertyDesc=="SelectedServices").PropertyValue.Split(','));
			TsiDemandType demandType=(tabControlMain.SelectedTab==tabUnsent?comboDemandType.SelectedTag<TsiDemandType>():comboExcludedDemandType.SelectedTag<TsiDemandType>());
...
//TODO
		...
		private void FillGridExcluded(bool retainSelection=true) {
			...
			foreach(DisplayField fieldCur in listDisplayFields) {
				...
				gridExcluded.Columns.Add(new ODGridColumn(string.IsNullOrEmpty(fieldCur.Description)?fieldCur.InternalName:fieldCur.Description,
					fieldCur.ColumnWidth,hAlign,sortingStrat));
			}
			//this form initially set to the max allowed (by OD) form size 1246, which is also the minimum size for this form.  If the user resizes the form
			//to be larger, increase each column width by the same ratio to spread out the additional real estate
			...
			if(widthColsAndScroll<gridExcluded.Width) {
				gridExcluded.Columns.ToList().ForEach(x => x.ColWidth=(int)Math.Round((float)x.ColWidth*gridExcluded.Width/widthColsAndScroll,MidpointRounding.AwayFromZero));
				//adjust the last col width to take any remaining pixels so that the cols take the full width of the grid (to account for rounding above)
				gridExcluded.Columns[gridExcluded.Columns.Count-1].ColWidth-=gridExcluded.WidthAllColumns+20-gridExcluded.Width;
			}
			#endregion Set Grid Title and Columns
			#region Fill Grid Rows
			gridExcluded.Rows.Clear();
		...
		private List<int> GetPatAgingIndexExcludedFiltered() {
			...
			List<long> listClinicNums=new List<long>();
			if(PrefC.HasClinicsEnabled) {
				if(comboBoxMultiExcludedClinics.ListSelectedIndices.Contains(0)) {
					listClinicNums=_listClinics.Select(x => x.ClinicNum).ToList();
				}
				else {
					//x-1 works because we know index 0 isn't selected(from above contains(0)) and we -1 for All clinics
					listClinicNums=comboBoxMultiExcludedClinics.ListSelectedIndices.Select(x => _listClinics[x-1].ClinicNum).ToList();
				}
			}
			#endregion Get Filter Data
			#region Apply Filter Data to PatAging List
		...
		private void FillExcludedDemandTypes() {
			TsiDemandType selectedType=comboExcludedDemandType.SelectedTag<TsiDemandType>();//If they have nothing selected, this will default to 'Accelerator'.
			List<long> listClinicNums=new List<long>();
			if(!PrefC.HasClinicsEnabled || comboBoxMultiExcludedClinics.ListSelectedIndices.Contains(0)) {
				listClinicNums=_listClinics.Select(x => x.ClinicNum).ToList();//if clinics are disabled, this will only contain the HQ "clinic"
			}
			else {
				//x-1 works because we know index 0 isn't selected(from above contains(0)) and we -1 due to the "All"" clinic
			...
			if(listClinicNums.Any(x => dictClinicSelectedServices.ContainsKey(x) 
				&& dictClinicSelectedServices[x].Contains(((int)TsiDemandType.Accelerator).ToString()))) 
			{
				comboExcludedDemandType.Items.Add(new ODBoxItem<TsiDemandType>(TsiDemandType.Accelerator.GetDescription(),TsiDemandType.Accelerator));
			}
			if(listClinicNums.Any(x => dictClinicSelectedServices.ContainsKey(x) 
				&& dictClinicSelectedServices[x].Contains(((int)TsiDemandType.ProfitRecovery).ToString()))) 
			{
				comboExcludedDemandType.Items.Add(new ODBoxItem<TsiDemandType>(TsiDemandType.ProfitRecovery.GetDescription(),TsiDemandType.ProfitRecovery));
			}
			if(listClinicNums.Any(x => dictClinicSelectedServices.ContainsKey(x) 
				&& dictClinicSelectedServices[x].Contains(((int)TsiDemandType.Collection).ToString()))) 
			{
				comboExcludedDemandType.Items.Add(new ODBoxItem<TsiDemandType>(TsiDemandType.Collection.GetDescription(),TsiDemandType.Collection));
			}
			if(comboExcludedDemandType.Items.Count>0) {
				comboExcludedDemandType.SetSelectedItem<TsiDemandType>(x => x==selectedType,"");
			}
		}
		
		private void comboBoxMultiExcludedClinics_SelectionChangeCommitted(object sender,EventArgs e) {
			FillExcludedDemandTypes();
			FillGridExcluded();
		}
