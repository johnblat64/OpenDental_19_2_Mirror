...
namespace OpenDental {
	...
	public partial class FormPaySplitEdit : ODForm {
		...
		private void FormPaySplitEdit_Load(object sender, System.EventArgs e) {
			...
			comboUnearnedTypes.Items.Add(new ODBoxItem<Def>("None",new Def{ }));
			...
			for(int i=0;i<_listPaySplitUnearnedTypeDefs.Count;i++) {
				ODBoxItem<Def> boxItemDef=new ODBoxItem<Def>(_listPaySplitUnearnedTypeDefs[i].ItemName,_listPaySplitUnearnedTypeDefs[i]);
				comboUnearnedTypes.Items.Add(boxItemDef);
				if(_listPaySplitUnearnedTypeDefs[i].DefNum==PaySplitCur.UnearnedType) {
					comboUnearnedTypes.SelectedIndex=i+1;
					_unearnedOld=_listPaySplitUnearnedTypeDefs[i];
				}
			}
		...
		private void butAttachProc_Click(object sender, System.EventArgs e) {
			...
			if(ProcCur.ProcStatus==ProcStat.TP) {
				...
				comboUnearnedTypes.SetSelectedItem<Def>(x => x.DefNum==PaySplitCur.UnearnedType,"");
			}
			else {
				comboUnearnedTypes.SelectedIndex=0;
				PaySplitCur.UnearnedType=0;
		...
		private void butAttachPrepay_Click(object sender,EventArgs e) {
			...
			if(FormPSS.ListSelectedSplits[0].SplitNum==PaySplitCur.SplitNum) {
			...
			if(isNegSplit) {//if negative then we're allocating money from the original prepayment.
				...
				if(comboUnearnedTypes.SelectedIndex==0) {//unearned has not been set
					comboUnearnedTypes.SetSelectedItem<Def>(x => x.DefNum==SplitAssociated.PaySplitOrig.UnearnedType,"");
