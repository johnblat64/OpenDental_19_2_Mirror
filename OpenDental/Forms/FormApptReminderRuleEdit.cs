...
namespace OpenDental {
	...
	public partial class FormApptReminderRuleEdit:ODForm {
		...
		///<summary>The plain text of the email body.</summary>
		...
		private void FormApptReminderRuleEdit_Load(object sender,EventArgs e) {
			...
			_emailPlainText=ApptReminderRuleCur.TemplateEmail;
			RefreshEmail();
		...
		private void RefreshEmail() {
			try {
				string text=MarkupEdit.TranslateToXhtml(_emailPlainText,isPreviewOnly:true,hasWikiPageTitles:false,isEmail:true);
		...
		private void butOk_Click(object sender,EventArgs e) {
			...
			ApptReminderRuleCur.TemplateEmail=_emailPlainText;
		...
		private bool ValidateRule() {
			...
			if(string.IsNullOrWhiteSpace(_emailPlainText)) {
			...
			if(ApptReminderRuleCur.TypeCur==ApptReminderType.ConfirmationFutureDay) {
				...
				if(_emailPlainText.ToLower().Contains("[confirmcode]")) {
					errors.Add(Lan.g(this,"Confirmation emails should not contain the \"[ConfirmCode]\" tag."));
				}
				if(!_emailPlainText.ToLower().Contains("[confirmurl]")) {
			...
			if(PrefC.GetBool(PrefName.EmailDisclaimerIsOn) && !_emailPlainText.ToLower().Contains("[emaildisclaimer]")) {
