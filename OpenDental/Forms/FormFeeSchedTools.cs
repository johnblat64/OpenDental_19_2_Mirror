...
namespace OpenDental {
	...
	public class FormFeeSchedTools :ODForm {
		...
		///<summary>A list of security logs that should be inserted when the form closes and the fee cache is refreshed.</summary>
    ...
    "les prior to selecting patients";
			...
			this.butOK.Location = new System.Drawing.Point(296, 486);
		...
		private void butCopy_Click(object sender, System.EventArgs e) {
			...
			ODProgress.ShowAction(() => _listSecurityLogEntries.AddRange(FeeScheds.CopyFeeSchedule(_feeCache,fromFeeSched,
						fromClinicNum,fromProvNum,toFeeSched,_listSelectedClinicNumsTo,toProvNum)),
		...
		private void butExport_Click(object sender,EventArgs e) {
			...
			ODProgress.ShowAction(
				() => {
					FeeScheds.ExportFeeSchedule(_feeCache,feeSched.FeeSchedNum,clinicNum,provNum,Dlg.FileName);
				},
		...
		private void butImportCanada_Click(object sender,EventArgs e) {
			...
			FeeSched feeSched=_listFeeScheds[comboFeeSched.SelectedIndex];
			List<Fee> listNewFees=FeeScheds.ImportCanadaFeeSchedule(_feeCache,feeSched,feeData,clinicNum,provNum,out numImported,out numSkipped);
			actionCloseFeeSchedImportCanadaProgress?.Invoke();
			Cursor=Cursors.Default;
			_changed=true;
			_feeCache.BeginTransaction();
				...
				if(PrefC.HasClinicsEnabled) {
					...
					for(int i=0;i<listFeeClinics.Count;i++) {
						...
						if(listFeeClinics.Count>1) {
							progExtended.Fire(ODEventType.FeeSched,new ProgressBarHelper(Lan.g(this,"Overall"),(int)percentComplete+"%",i,
								listFeeClinics.Count,tagString:"OverallStatus"));
							progExtended.Fire(ODEventType.FeeSched,new ProgressBarHelper(Lan.g(this,clinicCur.Abbr),"0%",1,100,
								tagString:"Clinic"));
						}
						else {
							progExtended.Fire(ODEventType.FeeSched,new ProgressBarHelper(Lan.g(this,clinicCur.Abbr),"0%",1,100,
