...
namespace OpenDental.UI{
	...
	public class ODToolBar : System.Windows.Forms.UserControl{
		...
		private static LinearGradientBrush _brushTogglePushed=null;
		private static LinearGradientBrush _brushTogglePushError=null;
		private static LinearGradientBrush _brushHover=null;
		private static LinearGradientBrush _brushHoverError=null;
		private static LinearGradientBrush _brushMed=null;
		private static LinearGradientBrush _brushMedError=null;
		private static LinearGradientBrush _brushPushed=null;
		private static LinearGradientBrush _brushPushedError=null;
		private static LinearGradientBrush _brushNotify=null;
		...
		public static void ReloadBrushes() {
			if(_brushTogglePushed!=null) {
				_brushTogglePushed.Dispose();
			}
			_brushTogglePushed=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarTogglePushedTopColor,ODColorTheme.ToolBarTogglePushedBottomColor);
			if(_brushTogglePushError!=null) {
				_brushTogglePushError.Dispose();
			}
			_brushTogglePushError=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarTogglePushedTopColorError,ODColorTheme.ToolBarTogglePushedBottomColorError);
			if(_brushHover!=null) {
				_brushHover.Dispose();
			}
			_brushHover=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarHoverTopColor,ODColorTheme.ToolBarHoverBottomColor);
			if(_brushHoverError!=null) {
				_brushHoverError.Dispose();
			}
			_brushHoverError=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarHoverTopColorError,ODColorTheme.ToolBarHoverBottomColorError);			
			if(_brushMed!=null) {
				_brushMed.Dispose();
			}
			_brushMed=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarTopColor,ODColorTheme.ToolBarBottomColor);
			if(_brushMedError!=null) {
				_brushMedError.Dispose();
			}
			_brushMedError=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarTopColorError,ODColorTheme.ToolBarBottomColorError);
			if(_brushPushed!=null) {
				_brushPushed.Dispose();//there is no brushDisabled for error colors.
			}
			_brushPushed=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarPressedTopColor,ODColorTheme.ToolBarPressedBottomColor);
			if(_brushPushedError!=null) {
				_brushPushedError.Dispose();
			}
			_brushPushedError=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				ODColorTheme.ToolBarPressedTopColorError,ODColorTheme.ToolBarPressedBottomColorError);			
			if(_brushNotify!=null) {
				_brushNotify.Dispose();
			}
			_brushNotify=new LinearGradientBrush(new Point(0,0),new Point(0,25),
				Color.FromArgb(255,231,167),ODColorTheme.ColorNotify);			
		}
      ...
      base.OnMouseMove(e);
		...
		///<summary>Resets button appearance. This will also deactivate the button if it has been pressed but not released. A pressed button will still be 
		///hot, however, so that if the mouse enters again, it will behave properly.  Repaints only if necessary.</summary>
      ...
      if((e.Button & MouseButtons.Left)!=MouseButtons.Left){
		...
		protected override void OnPaint(System.Windows.Forms.PaintEventArgs e){
			...
			if(DesignMode){
				e.Graphics.DrawRectangle(ODColorTheme.ToolBarOutlinePen,0,0,Width-1,Height-1);//draw toolbar
				StringFormat format=new StringFormat();
				format.Alignment=StringAlignment.Center;
				format.LineAlignment=StringAlignment.Center;
				e.Graphics.DrawString(this.Name,Font,ODColorTheme.ToolBarTextForeBrush,new Rectangle(0,0,Width,Height),format);
				return;
			}
			...
			ODException.SwallowAnyException(() => {
				...
				foreach(ODToolBarButton button in Buttons) {
					...
					if(button.IsRed) {
						...
						DrawButton(e.Graphics,button,ODColorTheme.ToolBarPenOutlinePenError,ODColorTheme.ToolBarDividerPenError
							,ODColorTheme.ToolBarTextForeBrushError,ODColorTheme.ToolBarTextDisabledBrushError,_brushTogglePushError,_brushHoverError,_brushMedError,
							_brushPushedError);
					}
					else {
						//regular draw
						DrawButton(e.Graphics,button,ODColorTheme.ToolBarOutlinePen,ODColorTheme.ToolBarDividerPen,
							ODColorTheme.ToolBarTextForeBrush,ODColorTheme.ToolBarTextDisabledBrush,_brushTogglePushed,_brushHover,_brushMed,_brushPushed);
					}
				}
				e.Graphics.DrawLine(ODColorTheme.ToolBarOutlinePen,0,Height-1,Width-1,Height-1);
			});
		}
