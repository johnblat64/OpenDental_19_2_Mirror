...
namespace OpenDental.UI {
	public partial class PinBoard:Control {
		...
		public ApptOverlapOrdering OverlapOrdering;
		...
		///<Summary>Supply an aptNum to get the corresponding DataRow and DataTables necessary to create the ContrApptSingle. Calls directly to database.
		///Returns null if the apt cannot be found.</Summary>
		...
		protected override void OnPaint(PaintEventArgs pe) {
			...
			try {
				...
				for(int i=0;i<apptList.Count;i++) {
					using(Bitmap shadow=apptList[i].CreateShadow()) {
						if(shadow!=null) {
							//Stack each appt 13 pixels apart. They will still be drawn at their original height but subsequent appts will be drawn on top of their preceding sibling.
							g.DrawImage(shadow,0,i*13);
						}
					}
				}
				if(apptList.Count==0) {
