...
namespace OpenDental.UI {
	class ODInternalCustomerGrids : UserControl {
		...
		private ODGrid gridBugsFixed;
		private TabPage tabBugSubs;
		private ODGrid gridBugSubmissions;
		...
		private ODGrid gridBugsInProgress;
		...
		private void InitializeComponent() {
			...
			this.gridBugsInProgress = new OpenDental.UI.ODGrid();
			this.tabBugsFixed = new System.Windows.Forms.TabPage();
			this.gridBugsFixed = new OpenDental.UI.ODGrid();
			this.tabBugSubs = new System.Windows.Forms.TabPage();
			this.gridBugSubmissions = new OpenDental.UI.ODGrid();
            ...
            | System.Windows.Forms.AnchorStyles.Left) 
            | System.Windows.Forms.AnchorStyles.Right)));
			this.gridBugsInProgress.CellFont = new System.Drawing.Font("Microsoft Sans Serif", 8.5F);
			...
			this.gridBugsInProgress.ScrollValue = 0;
            ...
            | System.Windows.Forms.AnchorStyles.Left) 
            | System.Windows.Forms.AnchorStyles.Right)));
			this.gridBugsFixed.CellFont = new System.Drawing.Font("Microsoft Sans Serif", 8.5F);
			...
			this.gridBugsFixed.ScrollValue = 0;
            ...
            | System.Windows.Forms.AnchorStyles.Left) 
            | System.Windows.Forms.AnchorStyles.Right)));
			this.gridBugSubmissions.CellFont = new System.Drawing.Font("Microsoft Sans Serif", 8.5F);
			...
			this.gridBugSubmissions.ScrollValue = 0;
			...
			this.datePickerHqBugSub.DefaultDateTimeFrom = new System.DateTime(2018, 1, 1, 0, 0, 0, 0);
			this.datePickerHqBugSub.DefaultDateTimeTo = new System.DateTime(2018, 4, 20, 0, 0, 0, 0);
		...
		private void TryViewTask(ODGrid grid) { 
			...
			Bug selectedBug=(Bug)grid.Rows[index].Tag;
		...
		private void FillGrids(bool isRefreshNeeded=true) {
			...
			#region Init Columns
			if(gridBugsFixed.Columns.Count==0) {
				gridBugsInProgress.Columns.Add(new ODGridColumn("Description",-1));
				gridBugsInProgress.Columns.Add(new ODGridColumn("Expert",55,HorizontalAlignment.Center));
				gridBugsInProgress.Columns.Add(new ODGridColumn("Has Job",55,HorizontalAlignment.Center));
				gridBugsFixed.Columns.Add(new ODGridColumn("Description",-1));
				gridBugsFixed.Columns.Add(new ODGridColumn("Vers.",55,HorizontalAlignment.Center));
				gridBugsFixed.Columns.Add(new ODGridColumn("Has Job",55,HorizontalAlignment.Center));
				gridBugSubmissions.Columns.Add(new ODGridColumn("Description",-1));
				gridBugSubmissions.Columns.Add(new ODGridColumn("Vers.",55,HorizontalAlignment.Center));
				gridBugSubmissions.Columns.Add(new ODGridColumn("Has Job",55,HorizontalAlignment.Center));
			}
			#endregion
			if(isRefreshNeeded) {
			...
			gridBugsInProgress.Rows.Clear();
			gridBugsFixed.Rows.Clear();
			gridBugSubmissions.Rows.Clear();
			...
			foreach(BugSubmission sub in _listBugSubs) {
				...
				if(sub.BugObj!=null && !listBugNums.Contains(sub.BugObj.BugId)) {//BugSubmission has an associated bug and it is not a duplicate.
					if(string.IsNullOrEmpty(sub.BugObj.VersionsFixed)) {//Bug is not fixed
						...
						ODGridRow rowBug=new ODGridRow(sub.BugObj.Description,
							expertName,
							(hasJob?"X":"")
						);
						rowBug.Tag=sub.BugObj;
						gridBugsInProgress.Rows.Add(rowBug);
						#endregion
					}
					else {//Bug is fixed
						#region Fixed grid
				...
				rowSub.Tag=sub;
				gridBugSubmissions.Rows.Add(rowSub);
				#endregion
			}
		...
		private void gridBugs_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			formBE.BugCur=(Bug)((ODGrid)sender).Rows[e.Row].Tag;
			formBE.Show();
		}
		...
		private void gridBugSubmissions_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			FormBugSubmission formBS=new FormBugSubmission((BugSubmission)gridBugSubmissions.Rows[e.Row].Tag);
			formBS.Show();
		}
