...
namespace OpenDental {
	public partial class FormJobManagerDashboard:ODForm {
		...
		private void FillGridOldBugs(List<Job> listStaleBugJobs) {
			...
			gridBugJobs.Columns.Add(new ODGridColumn("Last Updated",180,HorizontalAlignment.Center,GridSortingStrategy.DateParse));
		...
		private void FillGridOldEnhancements(List<Job> listStaleEnhancementJobs) {
			...
			gridEnhancementJobs.Columns.Clear();
			gridEnhancementJobs.Columns.Add(new ODGridColumn("Last Updated",180,HorizontalAlignment.Center,GridSortingStrategy.DateParse));
			gridEnhancementJobs.Columns.Add(new ODGridColumn("",0));
			gridEnhancementJobs.Rows.Clear();
			foreach(Job job in listStaleEnhancementJobs) {
				gridEnhancementJobs.Rows.Add(
				new ODGridRow(
					new ODGridCell(job.ListJobTimeLogs.Count>0 ? job.ListJobTimeLogs.Max(x => x.DateTStamp).ToShortDateString() : job.DateTimeEntry.ToShortDateString()),
					new ODGridCell(job.ToString())
					) {
