...
namespace OpenDental {
	public partial class FormSprintManagerDashboard:ODForm {
		...
		private void FillGridOldBugs(List<Job> listStaleBugJobs) {
			...
			gridBugJobs.Columns.Clear();
			gridBugJobs.Columns.Add(new ODGridColumn("Last Updated",200,HorizontalAlignment.Center));
			gridBugJobs.Columns.Add(new ODGridColumn("",0));
			gridBugJobs.Rows.Clear();
			foreach(Job job in listStaleBugJobs) {
				gridBugJobs.Rows.Add(
				new ODGridRow(
					new ODGridCell(job.ListJobTimeLogs.Count>0 ? job.ListJobTimeLogs.Max(x => x.DateTStamp).ToShortDateString() : job.DateTimeEntry.ToShortDateString()),
		...
		private void FillGridQueue() {
			...
			gridSprintQueue.Columns.Clear();
			gridSprintQueue.Columns.Add(new ODGridColumn("Priority",90,HorizontalAlignment.Center));
			gridSprintQueue.Columns.Add(new ODGridColumn("Owner",55,HorizontalAlignment.Center));
			gridSprintQueue.Columns.Add(new ODGridColumn("Owner Action",110));
			gridSprintQueue.Columns.Add(new ODGridColumn("EstHrs",60,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
			gridSprintQueue.Columns.Add(new ODGridColumn("ActHrs",60,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
			gridSprintQueue.Columns.Add(new ODGridColumn("",300));
			gridSprintQueue.Rows.Clear();
			...
			foreach(KeyValuePair<JobPhase,List<Job>> kvp in dictPhases) {
				...
				gridSprintQueue.Rows.Add(new ODGridRow("","","","","",kvp.Key.ToString()) { ColorBackG=ODColorTheme.GridHeaderBackBrush.Color,Bold=true });
				foreach(Job job in kvp.Value) {
					Color backColor=Color.White;
					Def jobPriority=_listJobPriorities.FirstOrDefault(y => y.DefNum==job.Priority);
					gridSprintQueue.Rows.Add(
					new ODGridRow(
						...
						new ODGridCell(job.OwnerAction.GetDescription()),
						new ODGridCell(job.HoursEstimate.ToString()),
		...
		private void FillGridActiveDevelopment() {
			...
			gridActiveDevelopment.Columns.Clear();
			gridActiveDevelopment.Columns.Add(new ODGridColumn("Priority",90,HorizontalAlignment.Center));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("Last Updated",90,HorizontalAlignment.Center));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("Owner",55,HorizontalAlignment.Center));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("Owner Action",110));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("EstHrs",60,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("ActHrs",60,HorizontalAlignment.Center,GridSortingStrategy.AmountParse));
			gridActiveDevelopment.Columns.Add(new ODGridColumn("",300));
			gridActiveDevelopment.Rows.Clear();
			...
			foreach(Job job in listActiveJobs) {
				Color backColor=Color.White;
				Def jobPriority=_listJobPriorities.FirstOrDefault(y => y.DefNum==job.Priority);
				gridActiveDevelopment.Rows.Add(
				new ODGridRow(
					new ODGridCell(jobPriority.ItemName) {
						CellColor=jobPriority.ItemColor,
						ColorText=(job.Priority==_listJobPriorities.FirstOrDefault(y => y.ItemValue.Contains("Urgent")).DefNum) ? Color.White : Color.Black,
