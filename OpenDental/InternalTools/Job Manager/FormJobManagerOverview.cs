...
namespace OpenDental {
	public partial class FormJobManagerOverview:ODForm {
		...
		private void FormJobManagerOverview_Load(object sender,EventArgs e) {
			...
			foreach(JobProposedVersion category in Enum.GetValues(typeof(JobProposedVersion))) {
				comboProposedVersion.Items.Add(new ODBoxItem<JobProposedVersion>(category.ToString(),category));
			}
		...
		private void FillGridEngineers(JobSprint jobSprint) {
			...
			gridEngineers.Columns.Add(new ODGridColumn("Engineer",0));
			gridEngineers.Columns.Add(new ODGridColumn("Scheduled Hrs",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Columns.Add(new ODGridColumn("- Est. Breaks",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Columns.Add(new ODGridColumn("= Hrs Total",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Columns.Add(new ODGridColumn("Assigned Dev Hrs",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Columns.Add(new ODGridColumn("Free Dev Hrs",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Columns.Add(new ODGridColumn("Completed Hrs",150) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.AmountParse  });
			gridEngineers.Rows.Clear();
			...
			foreach(Userod userEng in listEngUsers) {
				...
				ODGridRow row=new ODGridRow() { Tag=userEng };
				...
				ODGridCell cell=new ODGridCell(freeHrs.ToString());
				cell.CellColor=freeHrs<20?Color.LightSalmon:Color.LightGreen;//Arbitrary 20 hours.
				row.Cells.Add(cell);
				row.Cells.Add(Math.Round(totalCompletedHours).ToString());
				gridEngineers.Rows.Add(row);
			}
		...
		private void FillGridJobs(List<Job> listJobs) {
			...
			gridJobs.Columns.Add(new ODGridColumn("Est. Version",90,listProposedVersions) { TextAlign=HorizontalAlignment.Center });
			...
			gridJobs.Columns.Add(new ODGridColumn("Last Updated",90) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.DateParse});
			gridJobs.Rows.Clear();
			foreach(Job job in listJobs) {
				...
				row.Cells.Add(job.ProposedVersion.ToString(),listProposedVersions.IndexOf(job.ProposedVersion.ToString()));
		...
		private List<Job> GetFilteredJobList() {
			...
			if (comboProposedVersion.SelectedIndex>0) {
				listJobs=listJobs.FindAll(x => x.ProposedVersion==comboProposedVersion.SelectedTag<JobProposedVersion>());
			}
