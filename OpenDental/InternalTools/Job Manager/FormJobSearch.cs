...
namespace OpenDental {
	public partial class FormJobSearch:ODForm {
		...
		private List<JobPhase> _listJobPhases;
		...
		///<summary>Returns all selected jobs after filling the in-memory lists from the database.</summary>
		public List<Job> ListSelectedJobs {
			get {
				//Fill the in-memory lists from the database
				Jobs.FillInMemoryLists(_listSelectedJobs.FindAll(x => !_listOriginalJobNums.Exists(y => x.JobNum==y)).ToList());
				return _listSelectedJobs;
			}
		}

		public FormJobSearch(List<Job> listJobsAll=null,bool isMultiSelect=false) {
			InitializeComponent();
			...
			if(listJobsAll==null) {
				...
				_listOriginalJobNums=listJobsAll.Select(x => x.JobNum).ToList();
				_listJobsAll=new List<Job>(listJobsAll);
			}
		...
		private void FormJobSearch_Load(object sender,EventArgs e) {
			...
			_listJobPhases=Enum.GetValues(typeof(JobPhase)).Cast<JobPhase>().ToList();
			foreach(JobPhase phase in _listJobPhases) {
				listBoxPhases.Items.Add(phase.ToString());
				if(!phase.In(JobPhase.Cancelled,JobPhase.Complete)) {
					listBoxPhases.SetSelected(listBoxPhases.Items.Count-1,true);
				}
			}
			//Categories
			...
			listBoxCategory.SetSelected(0,true);
			//Priorities
			listBoxPriorities.Items.Add(new ODBoxItem<Def>("Any",null));
			listBoxPriorities.SelectedIndices.Add(0);
			foreach(Def def in Defs.GetDefsForCategory(DefCat.JobPriorities,true).OrderBy(x => x.ItemOrder).ToList()) {
				listBoxPriorities.Items.Add(new ODBoxItem<Def>(def.ItemName,def));
			}
		}
		...
		public List<Job> GetSearchResults() {
			//Fill the in-memory lists from the database
			Jobs.FillInMemoryLists(_listJobsFiltered.FindAll(x => !_listOriginalJobNums.Exists(y => x.JobNum==y)).ToList());
		...
		private void FillGridMain() {
			...
			if(listBoxPriorities.SelectedIndices.Count>0 && !listBoxPriorities.SelectedIndices.Contains(0)) {
				jobPriorities=listBoxPriorities.SelectedTags<Def>().Select(x => x.DefNum).ToArray();
			}
			...
			gridMain.Columns.Add(new ODGridColumn("Est. Version",80));
			...
			foreach(Job jobCur in _listJobsAll) {
				...
				row.Cells.Add(new ODGridCell(jobPriority.ItemName) {
