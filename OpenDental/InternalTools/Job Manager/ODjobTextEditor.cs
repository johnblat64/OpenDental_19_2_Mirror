...
namespace OpenDental {
    ...
    private void ODjobTextEditor_Layout(object sender,LayoutEventArgs e) {
		...
		private void FillGridRequirements() {
			...
			gridRequirements.Columns.Add(new ODGridColumn("",20,HorizontalAlignment.Center));
			gridRequirements.Columns.Add(new ODGridColumn("",20,HorizontalAlignment.Center));
			gridRequirements.Rows.Clear();
			foreach(JobRequirement jobReq in GetListJobRequirements()) {
				gridRequirements.Rows.Add(
					new ODGridRow(
						new ODGridCell(jobReq.Description),
						new ODGridCell(jobReq.HasExpert?"X":""),
						new ODGridCell(jobReq.HasEngineer?"X":""),
						new ODGridCell(jobReq.HasReviewer?"X":""),
						new ODGridCell("▲"),
						new ODGridCell("▼")
						)
				);
		...
		public void ResizeTextFields() {
			...
			if(conceptHeight>=writeupHeight) {
				splitterReqWriteup.SplitPosition=heightRequirementsMax;
				splitterConceptReq.SplitPosition=conceptHeight;
			}
			else {
				splitterConceptReq.SplitPosition=conceptHeight;
				splitterReqWriteup.SplitPosition=heightRequirementsMax;
		...
		private void gridRequirements_DoubleClick(object sender, EventArgs e) {
			...
			if(cell.X==-1) {
				...
				gridRequirements.SetSelected(new Point(0,gridRequirements.Rows.Count-1));//Select the new cell so the user can start typing immediately.
			}
			if(cell.Y==gridRequirements.Rows.Count()-1) {
		...
		private void gridRequirements_CellLeave(object sender,ODGridClickEventArgs e) {
			...
			jobRequirement.Description=gridRequirements.Rows[e.Row].Cells[e.Col].Text;
		...
		private void gridRequirements_CellClick(object sender,ODGridClickEventArgs e) {
			...
			switch(e.Col) {
				...
				case 4://Up
					...
					break;
				case 5://Down
					if(e.Row==gridRequirements.Rows.Count-1) {
						return;
					}
