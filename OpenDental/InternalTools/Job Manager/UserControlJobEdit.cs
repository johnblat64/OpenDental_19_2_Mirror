...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlJobEdit:UserControl {
		...
		private void FillGridRoles() {
			...
			gridRoles.Rows.Add(new ODGridRow("Tester",Userods.GetName(_jobCur.UserNumTester)));
		...
		private void FillGridLog() {
			...
			gridLog.Columns.Add(new ODGridColumn("Date",80));
			gridLog.Columns.Add(new ODGridColumn("User",60) { TextAlign=HorizontalAlignment.Center });
			gridLog.Columns.Add(new ODGridColumn("Expert",60) { TextAlign=HorizontalAlignment.Center });
			gridLog.Columns.Add(new ODGridColumn("Engineer",60) { TextAlign=HorizontalAlignment.Center });
			...
			gridLog.Columns.Add(new ODGridColumn("HrEst",35) { TextAlign=HorizontalAlignment.Center });
			gridLog.Columns.Add(new ODGridColumn("Title",0));
			gridLog.Columns.Add(new ODGridColumn("Description",0));
			gridLog.Rows.Clear();
		...
		private bool NeedsRequirementReapproval() {
			if(JobPermissions.IsAuthorized(JobPerm.Approval,true)       //Users with the Approval job permission never need reapproval.
				|| _jobCur.Category.In(JobCategory.Bug,JobCategory.Query,JobCategory.Conversion) //Bug, Query, and Conversion jobs do not need requirement approval at all.
		...
		private void comboPriority_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(_jobCur.PhaseCur.In(JobPhase.Concept,JobPhase.Definition,JobPhase.Development,JobPhase.Quote) 
				&& ((ODBoxItem<Def>)comboPriority.SelectedItem).Tag.ItemValue.Contains("OnHold") 
				&& _jobCur.ListJobLinks.Where(x => x.LinkType==JobLinkType.Bug).Count()>0) 
			{
				comboPriority.SetSelectedItem<Def>(x => x.DefNum==_jobCur.Priority,Defs.GetName(DefCat.JobPriorities,_jobCur.Priority));
