...
namespace OpenDental {
	public partial class FormJobManager:ODForm {
		...
		private void gridAction_CellClick(object sender,ODGridClickEventArgs e) {
			...
			LoadJob(job,false);
			if(e.Button==MouseButtons.Right && job.Category!=JobCategory.Query) {//Only non-queries. I want to assume we open UserControlJobEdit
				ContextMenu actionMenu=new ContextMenu();
				ActionMenuOpen(actionMenu,job);
			}
		}
		...
		///<summary>This is a nasty method. Logic in this must closely match the butActions_Click method in UserControlJobEdit.</summary>
		private void ActionMenuOpen(ContextMenu actionMenu,Job selectedJob) {
			bool perm=false;
			switch(selectedJob.PhaseCur) {
				case JobPhase.Concept:
					...
					actionMenu.MenuItems.Add(new MenuItem((selectedJob.UserNumConcept==0 ? "A" : "Rea")+"ssign Submitter",userControlJobEdit.actionMenu_AssignSubmitterClick) { Enabled=perm });//x
					if(!selectedJob.IsApprovalNeeded) {//x
						perm=JobPermissions.IsAuthorized(JobPerm.Concept,true);//x
						actionMenu.MenuItems.Add(new MenuItem("Send for Approval",userControlJobEdit.actionMenu_RequestConceptApprovalClick) { Enabled=perm });//x
						if(selectedJob.Category.In(JobCategory.Bug,JobCategory.Conversion)) {
							actionMenu.MenuItems.Add(new MenuItem("Send to Writeup",userControlJobEdit.actionMenu_SendWriteupClick) { Enabled=perm });//x
							actionMenu.MenuItems.Add(new MenuItem("Send to In Development",userControlJobEdit.actionMenu_SendInDevelopmentClick) { Enabled=perm });//x
						}
						actionMenu.MenuItems.Add(new MenuItem("Cancel Concept",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm && selectedJob.UserNumConcept==Security.CurUser.UserNum });//x
					}
					else if(!JobPermissions.IsAuthorized(JobPerm.Approval,true) && selectedJob.UserNumConcept==Security.CurUser.UserNum) {
						actionMenu.MenuItems.Add(new MenuItem("Repeal Approval Request",userControlJobEdit.actionMenu_RepealApprovalRequestClick) { Enabled=true });
					}
					else {
						perm=JobPermissions.IsAuthorized(JobPerm.Approval,true);//x
						actionMenu.MenuItems.Add(new MenuItem("Approve Concept",userControlJobEdit.actionMenu_ApproveConceptClick) { Enabled=perm });//x
						actionMenu.MenuItems.Add(new MenuItem("Request Quote",userControlJobEdit.actionMenu_RequestQuoteClick) { Enabled=perm });//x
						actionMenu.MenuItems.Add(new MenuItem("Approve Job",userControlJobEdit.actionMenu_ApproveJobClick) { Enabled=perm });//x
						actionMenu.MenuItems.Add(new MenuItem("Request Clarification",userControlJobEdit.actionMenu_RequestClarificationClick) { Enabled=perm });//x
						actionMenu.MenuItems.Add(new MenuItem("Cancel Concept",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm });//x
					}
					break;
				case JobPhase.Quote:
					if(!selectedJob.IsApprovalNeeded) {//x
						if(selectedJob.UserNumApproverQuote==0) {
							perm=JobPermissions.IsAuthorized(JobPerm.Quote,true)||_isOverride;//x
							actionMenu.MenuItems.Add(new MenuItem("Send for Approval",userControlJobEdit.actionMenu_RequestQuoteApprovalClick) { Enabled=perm });//x
						}
						else {
							perm=JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true)||_isOverride;//x
							actionMenu.MenuItems.Add(new MenuItem("Send for Approval",userControlJobEdit.actionMenu_RequestCustQuoteApprovalClick) { Enabled=perm });//x
						}
					}
					else {
						perm=JobPermissions.IsAuthorized(JobPerm.Approval,true);//x
						if(selectedJob.UserNumApproverQuote==0) {
							actionMenu.MenuItems.Add(new MenuItem("Approve Quote",userControlJobEdit.actionMenu_ApproveQuoteClick) { Enabled=perm });//x
							actionMenu.MenuItems.Add(new MenuItem("Cancel Job",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm });//x
						}
						else {
							actionMenu.MenuItems.Add(new MenuItem("Send to Writeup",userControlJobEdit.actionMenu_SendWriteupClick) { Enabled=perm });//x
							actionMenu.MenuItems.Add(new MenuItem("Cancel Job",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm });//x
				...
				case JobPhase.Definition:
					if(!selectedJob.IsApprovalNeeded) {
						if(JobPermissions.IsAuthorized(JobPerm.Approval,true) || JobPermissions.IsAuthorized(JobPerm.Writeup,true) || _isOverride) {
							actionMenu.MenuItems.Add(new MenuItem((selectedJob.UserNumExpert==0 ? "A" : "Rea")+"ssign Expert",userControlJobEdit.actionMenu_AssignExpertClick) { Enabled=true });//x
						}
						perm=JobPermissions.IsAuthorized(JobPerm.Approval,true)
							|| (JobPermissions.IsAuthorized(JobPerm.Writeup,true) && (selectedJob.UserNumExpert==0 || selectedJob.UserNumExpert==Security.CurUser.UserNum));
						actionMenu.MenuItems.Add(new MenuItem("Send for Approval",userControlJobEdit.actionMenu_RequestJobApprovalClick) { Enabled=perm });//x
						if(selectedJob.Category.In(JobCategory.Bug,JobCategory.Conversion)) {
							actionMenu.MenuItems.Add(new MenuItem("Send to In Development",userControlJobEdit.actionMenu_SendInDevelopmentClick) { Enabled=perm });//x
						}
						if(JobPermissions.IsAuthorized(JobPerm.Approval,true)) {
							actionMenu.MenuItems.Add(new MenuItem("Unapprove Concept",userControlJobEdit.actionMenu_UnapproveJobClick) { Enabled=true });//x
						}
					}
					else if(!JobPermissions.IsAuthorized(JobPerm.Approval,true) && selectedJob.UserNumExpert==Security.CurUser.UserNum) {
						actionMenu.MenuItems.Add(new MenuItem("Repeal Approval Request",userControlJobEdit.actionMenu_RepealApprovalRequestClick) { Enabled=true });
					}
					else {
						perm=JobPermissions.IsAuthorized(JobPerm.Approval,true);
						actionMenu.MenuItems.Add(new MenuItem("Approve Job",userControlJobEdit.actionMenu_ApproveJobClick) { Enabled=perm });
						actionMenu.MenuItems.Add(new MenuItem("Request Clarification",userControlJobEdit.actionMenu_RequestClarificationClick) { Enabled=perm });
						actionMenu.MenuItems.Add(new MenuItem("Cancel Job",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm });
					}
					break;
				case JobPhase.Development:
					if(!selectedJob.IsApprovalNeeded) {
						...
						if(JobPermissions.IsAuthorized(JobPerm.Approval,true) || selectedJob.UserNumExpert==Security.CurUser.UserNum  || _isOverride) {//only expert may re-assign
							actionMenu.MenuItems.Add(new MenuItem((selectedJob.UserNumEngineer==0 ? "A" : "Rea")+"ssign Engineer",userControlJobEdit.actionMenu_AssignEngineerClick) { Enabled=true });//x
						}
						else if(selectedJob.UserNumEngineer==0 && JobPermissions.IsAuthorized(JobPerm.Engineer,true)) {
							actionMenu.MenuItems.Add(new MenuItem("Take Job",userControlJobEdit.actionMenu_TakeJobClick) { Enabled=true });//x
						}
						if(selectedJob.UserNumEngineer==Security.CurUser.UserNum) {
							actionMenu.MenuItems.Add(new MenuItem("Request Review",userControlJobEdit.actionMenu_RequestReviewClick) { Enabled=true });
						}
						bool isExpert=JobPermissions.IsAuthorized(JobPerm.Writeup,true) && (selectedJob.UserNumExpert==0 || selectedJob.UserNumExpert==Security.CurUser.UserNum);
						bool isEngineer=JobPermissions.IsAuthorized(JobPerm.Engineer,true) && (selectedJob.UserNumEngineer==Security.CurUser.UserNum);
						actionMenu.MenuItems.Add(new MenuItem("Request Approval",userControlJobEdit.actionMenu_RequestChangeApprovalClick) { Enabled=isExpert ? true : isEngineer });//delayed save, after user can make edits.
						if(selectedJob.Category==JobCategory.Research) {
							perm=(isExpert || isEngineer) && selectedJob.UserNumEngineer>0 && selectedJob.ListJobReviews.Count>0 && selectedJob.ListJobReviews.Exists(x => x.ReviewStatus==JobReviewStatus.Done);
							actionMenu.MenuItems.Add(new MenuItem("Mark Complete",userControlJobEdit.actionMenu_ResearchComplete) { Enabled=perm });//not until the engineer set, and reviews completed
						}
						else {
							perm=(isExpert || isEngineer) && selectedJob.UserNumEngineer>0 && selectedJob.ListJobReviews.Count>0 && selectedJob.ListJobReviews.Exists(x => x.ReviewStatus==JobReviewStatus.SaveCommit);
							actionMenu.MenuItems.Add(new MenuItem("Save Commit",userControlJobEdit.actionMenu_SaveCommitClick) { Enabled=perm });//not until the engineer set, and reviews completed
							perm=(isExpert || isEngineer) && selectedJob.UserNumEngineer>0 && selectedJob.ListJobReviews.Count>0 && selectedJob.ListJobReviews.Exists(x => x.ReviewStatus==JobReviewStatus.Done);
							actionMenu.MenuItems.Add(new MenuItem("Mark as Implemented",userControlJobEdit.actionMenu_ImplementedClick) { Enabled=perm });//not until the engineer set, and reviews completed
						}
						if(JobPermissions.IsAuthorized(JobPerm.Approval,true)) {
							actionMenu.MenuItems.Add(new MenuItem("Unapprove Job",userControlJobEdit.actionMenu_UnapproveJobClick) { Enabled=true });//x
						}
					}
					else {
						perm=JobPermissions.IsAuthorized(JobPerm.Approval,true);
						if(!JobPermissions.IsAuthorized(JobPerm.Approval,true) && (selectedJob.UserNumExpert==Security.CurUser.UserNum || selectedJob.UserNumEngineer==Security.CurUser.UserNum)) {
							actionMenu.MenuItems.Add(new MenuItem("Repeal Approval Request",userControlJobEdit.actionMenu_RepealApprovalRequestClick) { Enabled=true });
						}
						actionMenu.MenuItems.Add(new MenuItem("Approve Changes",userControlJobEdit.actionMenu_ApproveChangeClick) { Enabled=perm });
						actionMenu.MenuItems.Add(new MenuItem("Request Clarification",userControlJobEdit.actionMenu_RequestClarificationClick) { Enabled=perm });
						actionMenu.MenuItems.Add(new MenuItem("Cancel Job",userControlJobEdit.actionMenu_CancelJobClick) { Enabled=perm });
					}
					break;
				case JobPhase.Documentation:
					if(JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true) && selectedJob.DateTimeCustContact.Year<1880) {
						actionMenu.MenuItems.Add(new MenuItem("Email Attached Customers",userControlJobEdit.actionMenu_EmailAttachedClick) { Enabled=true });//x
					}
					if(JobPermissions.IsAuthorized(JobPerm.Documentation,true)) {
						actionMenu.MenuItems.Add(new MenuItem((selectedJob.UserNumDocumenter==0 ? "A" : "Rea")+"ssign Documenter",userControlJobEdit.actionMenu_AssignDocumenterClick) { Enabled=true });//x
					}
					perm=JobPermissions.IsAuthorized(JobPerm.Documentation,true) && (selectedJob.UserNumDocumenter==0 || selectedJob.UserNumDocumenter==Security.CurUser.UserNum);
					actionMenu.MenuItems.Add(new MenuItem("Mark as Documented",userControlJobEdit.actionMenu_DocumentedClick) { Enabled=perm });
					break;
				case JobPhase.Complete:
					perm=JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true) && (selectedJob.UserNumCustContact==0 || selectedJob.UserNumCustContact==Security.CurUser.UserNum);
					if(JobPermissions.IsAuthorized(JobPerm.NotifyCustomer,true) || _isOverride) {
						actionMenu.MenuItems.Add(new MenuItem((selectedJob.UserNumCustContact==0 ? "A" : "Rea")+"ssign Contacter",userControlJobEdit.actionMenu_AssignContacterClick) { Enabled=true });//x
					}
					if(selectedJob.DateTimeCustContact.Year<1880) {
						actionMenu.MenuItems.Add(new MenuItem("Email Attached Customers",userControlJobEdit.actionMenu_EmailAttachedClick) { Enabled=perm });//x
						actionMenu.MenuItems.Add(new MenuItem("Mark as Contacted",userControlJobEdit.actionMenu_ContactClick) { Enabled=perm });
					}
					else {
						actionMenu.MenuItems.Add(new MenuItem("Mark as Uncontacted",userControlJobEdit.actionMenu_UnContactClick) { Enabled=perm });
					}
					//Nothing to do really, except override to change something.
					break;
				case JobPhase.Cancelled:
					perm=JobPermissions.IsAuthorized(JobPerm.Approval,true);
					actionMenu.MenuItems.Add(new MenuItem("Reopen as Concept",userControlJobEdit.actionMenu_ApproveConceptClick) { Enabled=perm });//x
					actionMenu.MenuItems.Add(new MenuItem("Reopen as Job",userControlJobEdit.actionMenu_ApproveJobClick) { Enabled=perm });//x
					break;
				default:
					actionMenu.MenuItems.Add(new MenuItem("Unhandled status "+selectedJob.PhaseCur.ToString(),(o,ea) => { }) { Enabled=false });
					break;
			}
			if(selectedJob.UserNumCheckout>0 && selectedJob.UserNumCheckout!=Security.CurUser.UserNum && !_isOverride) {
			...
			if(JobPermissions.IsAuthorized(JobPerm.Override,true)) {
				...
				actionMenu.MenuItems.Add("Override",userControlJobEdit.actionMenu_OverrideClick);
			}
			...
			if(gridAction.CurrentClickLocation!=null) {
				actionMenu.Show(gridAction,gridAction.CurrentClickLocation);
			}
		}
