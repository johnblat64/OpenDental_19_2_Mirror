...
namespace OpenDental {
	public partial class EscalationViewControl:UserControl, INotifyPropertyChanged {
		...
		///<summary>Key: Employee name, Value: True if the employee is on a GTA chat and no on a Web Chat.</summary>
		...
		private void EscalationViewControl_Paint(object sender,PaintEventArgs e) {
			...
			try {
				...
				for(int i=0;i<_items.Count;i++) {
					...
					bool hasBackgroundColor=(DictColors.ContainsKey(item) && DictColors[item]!=this.BackColor);
					bool hasExtension=(DictShowExtension.ContainsKey(item) && DictShowExtension[item]==true && DictExtensions.ContainsKey(item));
					bool hasWebChat=(DictWebChat.ContainsKey(item) && DictWebChat[item]==true);
					bool hasGtaChat=(DictGTAChat.ContainsKey(item) && DictGTAChat[item]==true);
					bool hasProximity=(DictProximity.ContainsKey(item) && DictProximity[item]==true);
					...
					using(Brush brushText=new SolidBrush(Color.FromArgb(alpha,ForeColor))) {
						if(!hasExtension && !hasWebChat && !hasGtaChat && !hasProximity) {
