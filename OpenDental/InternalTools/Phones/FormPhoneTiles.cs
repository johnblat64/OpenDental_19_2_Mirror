...
namespace OpenDental {
	public partial class FormPhoneTiles : ODForm {
		...
		///<summary>Stores a list of PhoneTile controls. Gets filled on load.</summary>
		private List<PhoneTile> _listPhoneTiles;
		...
		private int _tilesPerColumn=26;
		...
		private void FormPhoneTiles_Load(object sender,EventArgs e) {
			...
			if(_listPhones.Count>=1) {
				...
				int autoWidth=(columns*(_tileWidth+1))+1;
				int autoHeight=_tileStart+((_tileHeight+1)*_tilesPerColumn);
				if(autoWidth > 1650) {//Window was going off side of screen.  1650 chosen to accommodate side mounted windows taskbar.
					...
					autoWidth=1650;
					autoHeight=autoHeight+5;
				}
				if(autoHeight>1008) {
					autoHeight=1008; //the largest height visible for a 1080p monitor with task bar at the bottom on windows 10
				}
			...
			for(int i=1;i<_listPhones.Count+1;i++) {
				...
				if(_listPhones[numCur].ClockStatus!=ClockStatusEnum.Home
					...
					if(_listPhones[numCur].Description!="") {
						...
						g.DrawImage(Properties.Resources.WebChatIcon,(x*_tileWidth)+imageLocationX,(y*_tileHeight)+yTop+imageHeight);
		...
		private void FormPhoneTiles_DoubleClick(object sender,EventArgs e) {
		...
		public void SetPhoneList(List<PhoneEmpDefault> listPhoneEmpDefaults,List<Phone> listPhones,List<ChatUser> listChatUsers,bool isFillTiles=true) {
			...
			_listPhoneEmpDefaults=listPhoneEmpDefaults;
			if(isFillTiles) {
				FilterPhoneList();
				FillTiles(false);
			}
