	...
	public class ProcedureL {
		...
		public static List<ODBoxItem<ProcStat>> FillProcStatusCombo(ProcStat procCurStatus,bool isProcLocked,long procNum) {
			...
			if(procCurStatus==ProcStat.D && isProcLocked) {//only set this when coming in with this status. 
				listProcStat.Add(new ODBoxItem<ProcStat>(Lan.g("Procedures","Invalidated"),ProcStat.D));
				return listProcStat;
			}
			listProcStat.Add(new ODBoxItem<ProcStat>(Lan.g("Procedures","Treatment Planned"),ProcStat.TP));
			...
			if(!PrefC.GetBool(PrefName.EasyHideClinical)) {
				...
				listProcStat.Add(new ODBoxItem<ProcStat>(Lan.g("Procedures","Condition"),ProcStat.Cn));
			}
			if(procCurStatus==ProcStat.TPi) {//only set this when coming in with that status, users should not choose this status otherwise.
				listProcStat.Add(new ODBoxItem<ProcStat>(Lan.g("Procedures","Treatment Planned Inactive"),ProcStat.TPi));
			}
			return listProcStat;
		}
