	...
	public class AppointmentL {
		...
		public static Appointment MakeNewAppointment(Patient PatCur,bool useApptDrawingSettings,DateTime? dateTNew=null,long? opNumNew=null) {
			...
			if(PrefC.GetBool(PrefName.ApptModuleUses2019Overhaul)){
				...
				apptDateTime=GetApptDrawingSelectedDateTime();
				opNum=ContrAppt.SheetClickedonOp;
			}
		...
		public static bool DoPreventChangesToCompletedAppt(Appointment apt,PreventChangesApptAction action,List<Procedure> listAttachedProcs=null) {
			...
			MsgBox.Show($"Not allowed to {strAction} when there are completed procedures attached to this appointment.\r\n\r\nDetach the procedures from the " +
				"appointment if they were completed or change the status of the completed procedures to treatment planned if they were not completed " +
				"before trying again.");
			return true;
		}
	}
