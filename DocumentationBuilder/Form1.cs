...
namespace DocumentationBuilder {
	public partial class Form1:Form {
		...
		private void SchemaChanges() {
			...
			string pathDest=PATH_OPEN_DENTAL_SUBVERSION+@"opendental"+_versionCur.Major+"."+_versionCur.Minor+@"\DocumentationBuilder\OpenDentalDocumentation.xml";
			List<ODTable> listTablesNewVersion=GetFromXML(pathDest);
			pathDest=PATH_OPEN_DENTAL_SUBVERSION+@"opendental"+_versionPrevious.Major+"."+_versionPrevious.Minor+@"\DocumentationBuilder\OpenDentalDocumentation.xml";
			...
			WriteXml(outputFile,listTableChanges,includeSummary:false);
		}
		...
		private void WriteXml(string outputFile,List<ODTable> listTables,bool includeSummary=true) {
			...
			using(XmlWriter writer = XmlWriter.Create(outputFile,settings)) {
				...
				foreach(ODTable tbl in listTables) {
					WriteTable(writer,tbl.Name,tbl.ListColumns,includeSummary);
				}
		...
		private void WriteTable(XmlWriter writer,string tableName,List<ODColumn> listColumns,bool includeSummary){
			...
			foreach(ODColumn col in listColumns) {
				...
				WriteColumn(writer,order,tableName,col.Name,col.DataType,ancestorTables,isDeleted:col.IsDeleted);
			}
			writer.WriteEndElement();
		}
		...
		private List<ODTable> GetFromXML(string filePathToXML) {
			...
			foreach(XElement col in listXElements) {
				...
				listTables.FirstOrDefault(x => x.Name==table).ListColumns.Add(new ODColumn(name,type,order));
			}
			return listTables;
		}
