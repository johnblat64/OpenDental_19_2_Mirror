...
<soapenv:Envelope xmlns:soapenv=""http://schemas.xmlsoap.org/soap/envelope/"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"">
	<soapenv:Body>
		...
		///Callers of this method should catch that scenario.</summary>
		public static ValidateClaimResponse ValidateClaim(Claim claim,bool doValidateForAttachment) {
			...
			xml.Serialize(Console.Out,textRequest);
#endif
		...
		public static string CreateAttachment(List<ImageAttachment> listImages,string narrative,Claim claim) {
			...
			#region Xml Serialization (Debug Only)
			#if DEBUG
			...
			//xml1.Serialize(Console.Out,DxcCredentials.GetDentalxchangeCredentials(claim));
			...
			//xml2.Serialize(Console.Out,BuildAttachmentRequest(claim,narrative));
			//Commented out because it takes a few seconds to output the raw base64 to xml
			//System.Xml.Serialization.XmlSerializer xml3=new System.Xml.Serialization.XmlSerializer(arrayAttachments.GetType());
			//xml3.Serialize(Console.Out,arrayAttachments);
			#endif
			#endregion
		...
		///<summary>This method is used to add attachments to claims that already have an existing attachmentID. 
		...
		public static void AddAttachment(Claim claim,List<ImageAttachment> listImages) {
			...
			foreach(ImageAttachment image in listImages) {
				DentalxchangePartnerService.ImageReferenceResponses imageResponse=service.addImage(DxcCredentials.GetDentalxchangeCredentials(claim),attachmentRef,image.ConvertToAttachmentImage());
