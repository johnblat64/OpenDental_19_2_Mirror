...
namespace OpenDentBusiness.Eclaims {
	public class Canadian {
		...
		///For a normal claim, primaryEOB will be blank.  But if this is a COB(type7), then we need to embed the primary EOB by passing it in.
...
//Warning: this path is not handled properly if trailing slash is missing:
			...
			if(claim.ClaimType=="S") {//cob
				...
				if(carrierReceiver==null) {
					throw new ODException("Invalid secondary carrier associated to insurance plan.  Please run Database Maintenance to fix this.");
				}
			}
			...
			else if(insSub.AssignBen && clearinghouseClin.IsEraDownloadAllowed!=EraBehaviors.None && (fieldInputter.MsgType.In("21","23"))) {
						...
						switch(field.fieldId) {
							...
							case "G23"://Benefit Amount
								if(isPreEob) {
									...
									cpByTotal.InsEstTotalOverride=PIn.Double(RawMoneyStrToDisplayMoney(field.valuestr));//Set this so if it's not marked received, pat port is still reflected correctly
									if(eobBehavior==EraBehaviors.DownloadAndReceive) {
		...
		///otherwise errorMsg will be blank to indicate success.
