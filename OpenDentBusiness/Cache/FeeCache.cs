...
namespace OpenDentBusiness {
	...
	public class FeeCache:IEnumerable<Fee> {
		...
		///<summary>Constructs a new FeeCache where the size of _queueClinicNums will be limited to numClinics passed in.
		///The size limitation of _queueClinicNums will default to 5 if numClinics is set to a value that is less than 1.
		///If a clinic other than HQ is currently selected, add that clinic to the queue.
		///Loads in all of the HQ fees and the fees for the current clinic (if there is one) when doInitialize is set to true.</summary>
		...
		public FeeCache(List<Fee> listFees) {
			...
			Initialize();
			Add(listFees.Select(x => (FeeLim)x).ToList());
		}
		...
		public FeeCache GetCopy() {
			...
			foreach(long feeSched in listFeeSchedsToRefresh) {
				//Any fee schedule that cannot be refreshed right now can be left as "invalid" and will simply be tried again later.
				try {
					...
					retVal.Invalidate(feeSched);//Updates _setInvalidFeeScheds in the deep copy accordingly.
				}
			}
		...
		public Fee GetFee(long codeNum,long feeSchedNum,long clinicNum=0,long provNum=0,bool doGetExactMatch=false) {
			...
			if(!_dict.ContainsFeeSched(feeSchedNum)) {
				...
				List<FeeLim> listFeeSchedFees=Fees.GetByFeeSchedNumsClinicNums(new List<long> { feeSchedNum },this._queueClinicNums.ToList());
			...
			//Refresh this fee schedule if it needs to be refreshed.
			//If refreshing the fee schedule fails then we want calling methods to know that something went terribly wrong.
			RefreshInvalidFees(feeSchedNum);//Purposefully throws exceptions that HQ will want to know about in order to help reproduce the error.
		...
		///<summary>Return the list of fees exactly matching all three of the provided parameters.</summary>
		...
		private bool AddToCacheOnly(FeeLim fee) {
			return _dict.TryAdd(new FeeKey(fee),fee);
		}
		...
		private void AddClinicFees(long clinicNum) {
			...
			List<long> listFeeScheds=FeeScheds.GetDeepCopy(true).Select(x => x.FeeSchedNum).ToList();
			List<FeeLim> listFees=Fees.GetByFeeSchedNumsClinicNums(listFeeScheds,new List<long>() { clinicNum });
			Add(listFees);
		}
		...
		public void Update(Fee fee) {
			...
			//Refresh this fee schedule if it needs to be refreshed.
			//If refreshing the fee schedule fails then we want calling methods to know that something went terribly wrong.
			RefreshInvalidFees(fee.FeeSched);//Purposefully throws exceptions that HQ will want to know about in order to help reproduce the error.
			FeeKey feeKey=new FeeKey(fee);
			if(_dict.ContainsKey(feeKey)) {
				_dict[feeKey].Amount=fee.Amount;
			}
			else {
				AddToCacheOnly((FeeLim)fee);
			}
		...
		public void SaveToDb() {
			...
			if(_queueFeeUpdates.All(x => x.UpdateType==FeeUpdateType.Remove)) { //all updates are removes, so do bulk delete
			...
			else { //The updates are all Adds or all Updates or some mixture of Adds, Removes, and Updates.
		...
		/// from the db.  Throws an exception if _queueClinicNums is malformed which would have caused the fee schedule passed in to be emptied.</summary>
		public void RefreshInvalidFees(long feeSchedNum) {
			if(!_dict.IsFeeSchedInvalidated(feeSchedNum)) {
				return;//The fee schedule in the FeeCache is up to date and does not need to be refreshed.
			}
			//Purposefully bubble exceptions up if _queueClinicNums is in an invalid state.  The bug submission might help us determine the cause.
			List<FeeLim> listFeeSchedFees=Fees.GetByFeeSchedNumsClinicNums(new List<long> { feeSchedNum },this._queueClinicNums.ToList());
			//At this point we know that we went out to the database and got all updated fees for the passed in schedule and can update the FeeCache.
		...
		private class FeeCacheQueue:LimitedSizeQueue<long> {
			...
			///<summary>Construct a FeeCacheQueue with the provided size limit.  Sets limit to 5 if a value less than 1 was passed in.</summary>
			public FeeCacheQueue(int limit) : base(limit > 0 ? limit : 5) { }
		...
		private class FeeCacheDictionary:ConcurrentDictionary<FeeKey,FeeLim> {
			...
			public FeeCacheDictionary(IEnumerable<FeeLim> listFees) {
				foreach(FeeLim fee in listFees) {
					TryAdd(new FeeKey(fee),fee);
				}
			}
			...
			///<summary></summary>
			public void RefreshFeeSched(long feeSchedNum,List<FeeLim> listFeeSchedFees) {
				lock(_lockObj) {
					foreach(FeeLim fee in listFeeSchedFees) {
						this[new FeeKey(fee)]=fee;
					}
