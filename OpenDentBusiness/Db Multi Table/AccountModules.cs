	...
	public class AccountModules {
		...
		public static LoadData GetAll(long patNum,bool viewingInRecall,DateTime fromDate,DateTime toDate,bool intermingled,bool showProcBreakdown,
			...
			Action getPrePayForFam=new Action(() => {
				Logger.LogAction("PaySplits.GetPrepayForFam",LogPath.AccountModule,() => retVal.ListPrePayments=PaySplits.GetPrepayForFam(fam,false));
			...
			Action actionSetProcDate=() => {
				refreshOrGetFirstOrthoProcDate();
				getFirstOrthoProcDate();
			};
			...
			ODThread.RunParallel(new List<Action> { actionSetFamily,actionSetProcDate,actionSetClaims,actionSetMisc},Timeout.Infinite,4);
			return retVal;
		}
		...
		private static DataSet GetAccount(long patNum,DateTime fromDate,DateTime toDate,bool intermingled,bool singlePatient,long statementNum
			...
			,bool doIncludePatLName=false,List<long> listPayPlanNumsExclude=null) 
		{
			...
			#region Adjustments
			//Adjustments---------------------------------------------------------------------------------------
			command="SELECT AdjAmt,AdjDate,AdjNum,AdjType,ClinicNum,PatNum,ProcNum,ProvNum,AdjNote,SecDateTEdit "
				+"FROM adjustment ";
			if(isInvoice) {
			...
			foreach(DataRow rowRp in rawPay.Rows) {//Each row is a payment
				...
				listPaySplitMatches=listPaysplits.FindAll(x => x.DatePay==rpDatePay && x.PayPlanNum==rpPayPlanNum && x.PayNum==rpPayNum && x.PatNum==rpPatNum 
					&& x.ClinicNum==rpClinicNum && x.AdjNum>0);
				if(listPaySplitMatches.Count>0) {
			...
			for(int i=0;i<rawPay.Rows.Count;i++){
				...
				List<long> listFSplitNums=listPaysplits.Where(x=>x.PayNum==payNumCur).Select(y=>y.FSplitNum).ToList();
			...
			command="SELECT SUM(COALESCE(payplancharge.Principal,0)) principal_,SUM(COALESCE(payplancharge.Interest,0)) interest_,"
				...
				+"COALESCE(MAX(payplancharge.ClinicNum),0) ClinicNum "
				+"FROM payplan "
			...
			List<PaySplit> listSplitsCur = null/*,List<Adjustment> listAdjsCur = null,List<PayPlanCharge> listChargesCur = null*/) 
		{
			...
			List<ProcExtended> listProcExtendeds = Procedures.GetProcExtendedEntriesFromProcedures(listProcs,listAdjs,listPaySplits,listClaimProcs,
				listPayPlanCharges,listSplitsCur);
			listProcExtendeds.ForEach(x => listAccountEntries.Add(new AccountEntry(x)));
			...
			InsSub SubCur,Relat relatOther,List<Fee> listFees=null) 
		{
			...
			if (listProcs.Count==0) {
				claimError=claimError.AppendLine(Lans.g("ContrAccount","Not allowed to create a claim with no procedures."));
