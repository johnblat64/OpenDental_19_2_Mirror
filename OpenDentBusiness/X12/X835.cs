...
namespace OpenDentBusiness {
    ...
    private List<X12Segment> _listSegments;
		...
		private bool IsClp02Equivelent(Hx835_Claim claim1,Hx835_Claim claim2) {
		...
		public List<X835ClaimData> GetClaimDataList(List<Claim> listClaims) {
			...
			Dictionary<long,bool> dictClaimPaymentsExist=ClaimPayments.HasClaimPayment(listClaimNums);
			...
			foreach(Hx835_Claim eraClaim in this.ListClaimsPaid) {
				Claim matchedClaim;//Either null, manually detached or valid claim.
				switch(eraClaim.ClaimStatus) {
					default://Just in case.
					case EraClaimStatus.NotMatched:
						matchedClaim=null;
				...
				listRetVals.Add(new X835ClaimData(matchedClaim,dictClaimPaymentsExist[claimNum],dictMatchedEraClaims[claimNum]));
			}
			return listRetVals;
		}

		public X835Status GetStatus(List<Claim> listClaims,List<ClaimProc> listAllClaimProcs,List<Etrans835Attach> listAllAttaches) {
			return GetStatus(this.GetClaimDataList(listClaims),listAllClaimProcs,listAllAttaches);
		}

		public X835Status GetStatus(List<X835ClaimData> listClaimDatas,List<ClaimProc> listAllClaimProcs,List<Etrans835Attach> listAllAttaches) { 
	...
	public class X835ClaimData {
		...
		public X835ClaimData(Claim attachedClaim,bool hasPayment,List<Hx835_Claim> listMatched835Claims) {
	...
	public class Hx835_Claim {
		...
		public bool IsProcessed(List<ClaimProc> listEraClaimProcs,List<Etrans835Attach> listAttaches) {
			...
			List<ClaimProc> listClaimProcsForEraProcs=GetClaimProcsForEraProcs(listEraClaimProcs,listSplitClaimNums);
		...
		private List<ClaimProc> GetClaimProcsForEraProcs(List<ClaimProc> listAllClaimProcs,List<long> listSplitClaimNums) {
			List<ClaimProc> listClaimProcs=new List<ClaimProc>();
			foreach(ClaimProc claimProc in listAllClaimProcs) {
		...
		public bool GetIsSupplemental(List<Etrans835Attach> listAttaches,List<ClaimProc> listEraClaimProcs) {
			if(this.ClaimNum==0) {
		...
		private bool HasPreviousSupplemental(List<ClaimProc> listEraClaimProcs) {
	...
	public class Hx835_Proc {
		...
		public bool TryGetMatchedClaimProc(out ClaimProc matchedClaimProc,List<ClaimProc> listClaimProcs,bool isSupplemental) {
		...
		private bool IsBasicMatch(ClaimProc claimProc,bool isSupplemental) {
			...
			return ((decimal)claimProc.FeeBilled==ProcFee && claimProc.Status.In(ClaimProcStatus.Received,ClaimProcStatus.NotReceived,ClaimProcStatus.Preauth));
		}

		private bool HasPaymentBeenEntered(ClaimProc claimProc) { 
		...
		private bool IsPayAmountMatch(ClaimProc claimProc) {
