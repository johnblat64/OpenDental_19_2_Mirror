...
namespace OpenDentBusiness {
	public class DatabaseMaintenances {
		...
		#region Ebill, EduResource, EmailAttach, Etrans-------------------------------------------------------------------------------------------------
		...
		public static string InsSubNumMismatchPlanNum(bool verbose,DbmMode modeCur,long patNumSpecific=0) {
			...
			switch(modeCur) {
				...
				case DbmMode.Fix:
					...
					#region Etrans
					//etrans---------------------------------------------------------------------------------------------------
					where="WHERE PlanNum!=0 AND PlanNum != (SELECT inssub.PlanNum FROM inssub WHERE inssub.InsSubNum=etrans.InsSubNum)"
						+PatientAndClauseHelper(patNumSpecific,"etrans");
		...
		public static string PreferenceInsBillingProv(bool verbose,DbmMode modeCur) {
			...
			if(insBillingProvNum==0 || prov!=null) {//0 means the program will use the default practice provider.
				if(verbose) {
