...
namespace OpenDentBusiness{
	...
	public class Prefs{
		...
		public static List<Pref> GetPrefs(List<string> listPrefNames) {
			...
			return _prefCache.GetWhere(x=>x.PrefName.In(listPrefNames));
		}
		...
		public static bool UpdateDouble(PrefName prefName,double newValue,bool doRounding=true) {
			...
			bool retVal=true;
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				retVal=Meth.GetBool(MethodBase.GetCurrentMethod(),prefName,newValue,doRounding);
			}
			else {
				Db.NonQ(command);
			}
		...
		public static PrefName GetSheetDefPref(SheetTypeEnum sheetType) {
			...
			switch(sheetType) {
				...
				case SheetTypeEnum.RxInstruction:
					retVal=PrefName.SheetsDefaultRxInstructions;
					break;
				default:
