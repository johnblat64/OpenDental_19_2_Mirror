...
//		}
		...
		public static List<Supply> Sync(List<Supply> listNew,List<Supply> listOld) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Supply>>(MethodBase.GetCurrentMethod(),listNew,listOld);
			}
			Crud.SupplyCrud.Sync(listNew,listOld);
			return listNew;
		}
