	...
	public class Benefits {
			...
			List<ClaimProcHist> histList,List<ClaimProcHist> loopList,InsPlan plan,long patNum) 
		{
			...
			for(int i=0;i<histList.Count;i++) {
				...
				//}
					...
					//We currently have no way to specify how much deductible has been used in a category.  We only have an adjustment to general deductible.
		...
		public static bool IsBitewingFrequency(Benefit ben) {
			if(ben.CodeNum==ProcedureCodes.GetCodeNum(ProcedureCodes.BitewingCode)
				...
				&& ben.PatPlanNum==0
				&& ben.Percent==-1
				&& ((ben.QuantityQualifier.In(BenefitQuantity.Months,BenefitQuantity.Years) && ben.TimePeriod==BenefitTimePeriod.None)
					|| (ben.QuantityQualifier==BenefitQuantity.NumberOfServices && ben.TimePeriod.In(BenefitTimePeriod.ServiceYear,
