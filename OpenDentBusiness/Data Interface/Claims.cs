...
namespace OpenDentBusiness{
	...
	public class Claims{
		...
		public static ClaimSendQueueItem[] GetQueueList(List<long> listClaimNums,long clinicNum,long customTracking) {
			...
			string command=
				"SELECT claim.ClaimNum,carrier.NoSendElect,patplan.Ordinal,"
				+DbHelper.Concat("patient.LName","', '","patient.FName","' '","patient.MiddleI")+" patName,"
				...
				+"LEFT JOIN insplan ON claim.PlanNum=insplan.PlanNum "
				+"LEFT JOIN carrier ON insplan.CarrierNum=carrier.CarrierNum "
				+"LEFT JOIN patplan ON patplan.InsSubNum=claim.InsSubNum AND patplan.PatNum=claim.PatNum "
				+"WHERE "+string.Join("AND ",listWhereAnds)//listWhereAnds guaranteed to have at least one item in it (based on count of listClaimNums)
				+"ORDER BY claim.DateService,patient.LName,patient.FName";
			DataTable table=Db.GetTable(command);
			return table.Select().Select(x => new ClaimSendQueueItem() { 
				ClaimNum         = PIn.Long(x["ClaimNum"].ToString()),
				NoSendElect      = PIn.Enum<NoSendElectType>(x["NoSendElect"].ToString()),
				Ordinal          = PIn.Int(x["Ordinal"].ToString()),
				PatName          = PIn.String(x["patName"].ToString()),
				ClaimStatus      = PIn.String(x["ClaimStatus"].ToString()),
				Carrier          = PIn.String(x["CarrierName"].ToString()),
				PatNum           = PIn.Long(x["PatNum"].ToString()),
				MedType          = PIn.Enum<EnumClaimMedType>(x["MedType"].ToString()),
				ClearinghouseNum = Clearinghouses.AutomateClearinghouseHqSelection(PIn.String(x["ElectID"].ToString()),
					PIn.Enum<EnumClaimMedType>(x["MedType"].ToString())),
				DateService      = PIn.Date(x["DateService"].ToString()),
				ClinicNum		     = PIn.Long(x["ClinicNum"].ToString()),
				CustomTracking   = PIn.Long(x["CustomTracking"].ToString()),
				HasIcd9          = PIn.Bool(x["HasIcd9"].ToString())
			}).ToArray();
		}
		...
		public static List <long> GetClaimFromX12(List <X12ClaimMatch> listX12claims) {
			...
			#endregion
			#region Set feeMin and feeMax for given X12ClaimMatches.  All claims in DB must be inside this range because all ERA claims are within this range.
			double feeMin=0;
			double feeMax=0;
		...
		public static void CalculateAndUpdate(List<Procedure> listProcedures,List <InsPlan> listInsPlans,Claim claimCur,List <PatPlan> patPlans,List <Benefit> benefitList,Patient patient,List<InsSub> subList){
			...
			List<Fee> listFees=Fees.GetListFromObjects(listProcedureCodes,listProcedures.Select(x=>x.MedicalCode).ToList(),listProcedures.Select(x=>x.ProvNum).ToList(),
				...
				listSubstLinks,patient.DiscountPlanNum);
			double claimFee=0;
			double dedApplied=0;
			double insPayEst=0;
			double insPayAmt=0;
