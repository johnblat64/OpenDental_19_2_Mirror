...
namespace OpenDentBusiness{
	...
	public class Appointments {
			...
			,DateTime schedDateEvaluating,List<Schedule> listSchedules,List<Appointment> listAppointments) 
		{
			...
			List<Schedule> listSchedulesOrdered=listSchedules.OrderByDescending(x => x.BlockoutType.In(listBlockoutTypesDoNotSchedule))
			...
			foreach(Schedule schedule in listSchedulesOrdered) {
				...
				if(listProvNums.Contains(schedule.ProvNum)) {//schedule applies to one of the selected providers
					...
					for(int i=0;i<scheduleLengthInBlocks;i++) {
						if(schedule.BlockoutType > 0 && schedule.BlockoutType.In(listBlockoutTypesDoNotSchedule)) {
		...
		///<summary>If aptnum is specified, then the dates are ignored in the Appointment table queries; they are still used in the Adjustment table 
		///queries.  If getting data for one planned appt, then pass isPlanned=1.  
		///The times of the dateStart and dateEnd are ignored.  This changes which procedures are retrieved.
		///Any ApptNums within listPinApptNums will get forcefully added to the DataTable.
		///Since dateStart and dateEnd are used to calcualte adjustment totals, when in 'Day' view mode, dateStart should be set to the beginning of the
		///day and dateEnd set to the end of the day.</summary>
			...
			List<long> listOpNums=null,List<long> listProvNums=null,bool doRunQueryOnNoOps=true) 
		{
			...
			string command="SELECT patient.Address patAddress1,patient.Address2 patAddress2,patient.AddrNote patAddrNote,"
				...
				+"patient.DiscountPlanNum discountPlan,(CASE patient.DiscountPlanNum WHEN 0 THEN 0 ELSE 1 END) hasDiscount "
				+"FROM appointment "
			...
			List<Recall> listPastDueRecalls=Recalls.GetPastDueForPats(listPatNums);
			...
			if(rawProc!=null && rawProc.Rows.Count > 0) {
				string procNums=string.Join(",",rawProc.AsEnumerable().Select(x => x["ProcNum"].ToString()));
			...
			foreach(DataRow rowRaw in raw.Rows) {
				...
				actions.Add(new Action(() => {
					...
					row["recallPastDue[R]"]=(Recalls.IsPatientPastDue(PIn.Long(rowRaw["apptPatNum"].ToString()),false,listPastDueRecalls) ? "R" : "");
			...
			string codeRangeEnd="") 
		{
			...
			if(listCodeNums.Count>0 && listAppts.Count>0) {
				...
				listAppts.RemoveAll(x => !listFilteredAptNum.Contains(x.AptNum));
			}
			return listAppts;
		}
			...
			string codeEnd,DateTime dateStart,DateTime dateEnd)
		{
			...
			command+="AND appointment.AptDateTime BETWEEN "+POut.Date(dateStart)+" AND "+POut.Date(dateEnd.Year<1880 ? DateTime.MaxValue : dateEnd);
		...
		public static void UpdateProcDescriptionForAppt(Procedure proc) {
			if(proc.AptNum==0 && proc.PlannedAptNum==0) {
			...
			if(proc.AptNum>0) {
				...
				apt=Appointments.GetOneApt(proc.PlannedAptNum);
			}
			if(apt==null) {
		...
		public static bool TryAdjustAppointment(Appointment apt,List<Operatory> visOps,bool canChangeOp,bool canShortenPattern,bool hasEndTimeCheck
			,bool hasBlockoutCheck,out bool isPatternChanged,out bool doesOverlapBlockout) 
		{
			isPatternChanged=false;
			doesOverlapBlockout=false;
			...
			HasOverlap funcHasOverlap=(long opNum,out bool hasOverlapBlockout) => {
				hasOverlapBlockout=false;
			...
			if(!funcHasOverlap(apt.Op,out doesOverlapBlockout)) { //No overlap on our original op so we are good.
				return false;
			}
			if(canShortenPattern) { //If we are allowed to shorten the pattern then we will not change ops, we will shorten the pattern down to no less than 1 and return it.
				...
				do {
					if(apt.Pattern.Length==1) { //Pattern has been reduced to smallest allowed size.
						return funcHasOverlap(apt.Op,out doesOverlapBlockout);
					}
					//Reduce the pattern by 1.
					apt.Pattern=apt.Pattern.Substring(0,apt.Pattern.Length-1);
				} while(funcHasOverlap(apt.Op,out doesOverlapBlockout));
		...
		public static bool TryAdjustAppointmentInCurrentOp(Appointment apt,bool canShortenPattern,bool hasEndTimeCheck,out bool isPatternChanged,
			out bool doesOverlapBlockout) 
		{
			return TryAdjustAppointment(apt,null,false,canShortenPattern,hasEndTimeCheck,true,out isPatternChanged,out doesOverlapBlockout);
		}
		...
		public static bool TryAdjustAppointmentOp(Appointment apt,List<Operatory> listOps) {
			bool notUsed;
			return !Appointments.TryAdjustAppointment(apt,listOps,true,false,true,true,out notUsed,out notUsed);
		}
			...
			List<PatPlan> listPatPlans=null,List<InsSub> listInsSubs=null,List<InsPlan> listInsPlans=null,List<Benefit>listBenefits=null)
		{
			...
			if(listAptTypeProcs.Count>0) {//AppointmentType is associated to procs.
				...
				List<Fee> listFees=Fees.GetListFromObjects(listAptTypeProcs,null,null,//no existing procs to pull medCodes and provNums out of
					pat.PriProv,pat.SecProv,pat.FeeSched,listInsPlans,new List<long>(){appt.ClinicNum },new List<Appointment>(){appt},listSubstLinks,pat.DiscountPlanNum);
		...
		public static bool IsRecallAppointment(Appointment appt,List<Procedure> listApptProcs=null) {
			...
			if(!string.IsNullOrEmpty(strRecallTypesShowingInList)) {//Limit RecallTypes to check against if RecallTypesShowingInList preference is set.
				...
				listRecalls=listRecalls.FindAll(x => x.RecallTypeNum.In(listRecallTypeNums));
			}
			foreach(Recall recall in listRecalls) {
				...
				if(recall.DateScheduled.Date==appt.AptDateTime.Date && listApptProcs.Exists(x => x.CodeNum.In(listTriggerCodeNums))) {
