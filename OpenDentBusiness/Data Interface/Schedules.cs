	...
	public class Schedules {
			...
			foreach(long provNum in listProvNums) {//Potentially check each provider for overlap.
				...
				//====================COMPLEX OVERLAP, Ops and All====================
					...
					isOverlapping=(listProvSchedsOnly.Where(x => x.Ops.Count>0 //Can only overlap if Ops are involved
							&& schedTemp.StartTime<=x.StopTime //Overlapping Time
							&& schedTemp.StopTime>=x.StartTime //Overlapping Time
							&& x.Ops.Any(y => y.In(listSelectedOpNums)))//Schedule contains any operatory in question.
						.Count() > 1);
				}
				if(isOverlapping) {
			...
			,List<long> listIgnoreProvNums=null)
		{
			...
			if(listIgnoreProvNums!=null) {
				listDbProvScheds.RemoveAll(x => x.ProvNum.In(listIgnoreProvNums));
			}
			...
			DateTime dateCopyEnd,DateTime dateSelectedStart,DateTime dateSelectedEnd,int numRepeat) 
		{
			...
			if(isWeek) {
				...
				if(includeWeekend) {
					...
					dateEnd=dateSelectedEnd.AddWeekDays(numRepeat-1);
				}
			}
		...
		///<summary>Returns a table of schedules with their clinic (gotten from operatory if a link exists) for the providers specified, for the date 
		///range specified, and for the clinics specified.</summary>
		...
		public static List<Schedule> GetSchedulesForAppointmentSearch(DateTime dateStart,DateTime dateEnd,List<long> listClinicNums,
			List<long> listOpNums,List<long> listProvNums,long blockoutType)
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Schedule>>(MethodBase.GetCurrentMethod(),dateStart,dateEnd,listClinicNums,listOpNums,listProvNums,blockoutType);
			}
			...
			return GetSchedulesHelper(dateStart,dateEnd,listClinicNums,listOpNums,listProvNums,listBlockoutTypeDefNums,listSchedTypes);
		}
		...
		public static List<Schedule> GetProvSchedsForOp(List<Schedule> listForPeriod,Operatory op){
			//No need to check RemotingRole; no call to db.
			...
			,List<long> listProvNums,List<long> listBlockoutTypeDefNums,List<int> listSchedTypes,Logger.IWriteLine log=null) 
		{
			...
			string command=$@"
				(SELECT schedule.* FROM schedule
					...
					WHERE operatory.OperatoryNum IN ({string.Join(",",listOperatoryNums)})
					{((listClinicNums==null || listClinicNums.Count < 1) ? "" : "AND operatory.ClinicNum IN ("+string.Join(",",listClinicNums)+") ")}
					AND schedule.ProvNum IN ({string.Join(",",listProvNums)})
					AND schedule.BlockoutType IN ({string.Join(",",listBlockoutTypeDefNums.Select(x => POut.Long(x)))})
			...
			foreach(string provColumnName in new List<string>(){ "ProvDentist","ProvHygienist" }) {
				...
				command+=$@"
					...
					(SELECT schedule.* FROM schedule
						...
						AND operatory.OperatoryNum IN ({string.Join(",",listOperatoryNums)})
						{((listClinicNums==null || listClinicNums.Count < 1) ? "" : "AND operatory.ClinicNum IN ("+string.Join(",",listClinicNums)+") ")}
