...
namespace OpenDentBusiness{
	...
	public class SecurityLogs {
		...
		public static void DeleteBeforeDate(DateTime dateKeep) {
			...
			//Delete the hashes
			MiscDataEvent.Fire(CodeBase.ODEventType.MiscData,"Removing old data from securityloghash table.");
			string command=$"SELECT SecurityLogNum FROM securitylog WHERE LogDateTime < {POut.DateT(dateKeep)}";
			List<long> listSecurityLogNums=Db.GetListLong(command);
			...
			MiscDataEvent.Fire(CodeBase.ODEventType.MiscData,"Removing old data from securitylog table.");
