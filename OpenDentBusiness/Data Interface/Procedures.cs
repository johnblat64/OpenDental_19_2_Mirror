...
namespace OpenDentBusiness {
	public class Procedures {
		...
		///<summary>Helper method. Be very careful when modifiying  and only do so when absolutely necessary. 
		///The referencing methods are used in many places.</summary>
		...
		public static long Insert(Procedure procedure,bool doCalcTax=true,bool isRepeatCharge=false) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb){
				procedure.ProcNum=Meth.GetLong(MethodBase.GetCurrentMethod(),procedure,doCalcTax,isRepeatCharge);
		...
		public static bool Update(Procedure procedure,Procedure oldProcedure,bool isPaySplit=false,bool isSilent=false) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				bool retVal=Meth.GetBool(MethodBase.GetCurrentMethod(),procedure,oldProcedure,isPaySplit,isSilent);
			...
			if(oldProcedure.ProcStatus!=ProcStat.C && procedure.ProcStatus==ProcStat.C) {
				...
				if(!procedure.Discount.IsZero()) { //Discounted Procedure
			...
			if(result && oldProcedure.ProcStatus==ProcStat.TP && procedure.ProcStatus==ProcStat.C) {
				...
				List<PaySplit> listSplitsForProc=PaySplits.GetPaySplitsFromProc(procedure.ProcNum);
			...
			for(int i=0;i<claimProcs.Count;i++) {
				...
				if(patplan==null) {//the plan for this claimproc was dropped 
					...
					ClaimProcs.ComputeBaseEst(claimProcs[i],proc,PlanCur,patplan.PatPlanNum,
						...
						listSubstLinks,useProcDateOnProc,lookupFees);
				}
			...
			List<Procedure> listProcsForAppt,List<InsSub> subList,Userod curUser)
		{
			...
			List<long> listClaimNumsForApptProcs=claimProcList.Where(x => x.ClaimNum>0 && x.ProcNum.In(listProcsForAppt.Select(y=>y.ProcNum)))
				.Select(x => x.ClaimNum).ToList();
			List<ClaimProcHist> histListForApptProcs=histList.Where(x => x.ClaimNum.In(listClaimNumsForApptProcs)).ToList();
