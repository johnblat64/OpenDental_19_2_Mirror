...
namespace OpenDentBusiness {
	...
	public class Recalls {
		...
		public static List<Recall> GetPastDueForPats(List<long> listPatNums) {
			if(listPatNums==null || listPatNums.Count<=0) {
				return new List<Recall>();
			}
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			string command="SELECT * FROM recall "+
				...
				"AND DateDue<"+POut.Date(DateTimeOD.Today)+" "+
				"AND DateDue>"+POut.Date(dateMin)+" "+
				"AND DateScheduled<"+POut.Date(dateMin);
			return Crud.RecallCrud.SelectMany(command);
		}
			...
			bool doShowReminded=false) 
		{
			...
			command="SELECT webschedrecall.WebSchedRecallNum,webschedrecall.PatNum,webschedrecall.SmsSendStatus,webschedrecall.EmailSendStatus,"
				+"webschedrecall.DateTimeSendFailed,webschedrecall.ResponseDescript "
				+"FROM webschedrecall "
				+"INNER JOIN ("
			...
			foreach(KeyValuePair<long,List<DataRow>> kvp in dictRecallRows) {
				...
				foreach(DataRow rowCur in kvp.Value) {
					...
					if(!DoIncludeRecall(numberOfReminders,dateRemind,recallMaxNumberReminders,showReminders,PIn.Double(rowCur["DisableUntilBalance"].ToString()),
						PIn.Date(rowCur["DisableUntilDate"].ToString()),familyBalance,isAsap,doShowReminded))
					{
						continue;
					}
					#region Create Row
					...
					WebSchedRecall webSchedSendMostRecent=null;
					if(dictWebSchedRecalls.TryGetValue(patNum,out webSchedSendMostRecent)) {
		...
		public static bool IsPatientPastDue(long patNum,bool isProphyOrPerio,List<Recall> listPastDueRecalls) {
			if(listPastDueRecalls==null) {
				return false;
			}
			return listPastDueRecalls.Where(x => x.PatNum==patNum)
				.Where(x => x.DateDue.Date<DateTimeOD.Today.Date && x.DateDue.Date.Year>1880 && x.DateScheduled.Year<1880)
