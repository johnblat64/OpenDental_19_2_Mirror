	...
	public class SheetFields{
		...
		public static void Sync(List<SheetField> listSheetFieldsNew,long sheetNum,bool isSigBoxOnly) {
			...
			if(!isSigBoxOnly) {
				...
				List<SheetField> listNoSigDB=listSheetFieldsDB.FindAll(x => x.FieldType!=SheetFieldType.Parameter && !x.FieldType.In(SheetFieldType.SigBox,SheetFieldType.SigBoxPractice));
