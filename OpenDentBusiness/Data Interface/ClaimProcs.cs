...
namespace OpenDentBusiness{
	...
	public class ClaimProcs{
		...
		public static decimal GetPatPortion(Procedure proc,List<ClaimProc> listClaimProcs,List<Adjustment> listAdjustments=null) {
			...
			#region Write-offs
			patPort-=listProcClaimProcs
				.Where(x => x.Status.In(GetEstimatedStatuses()))
				.Sum(x => (decimal)(x.WriteOffEstOverride==-1 ? (x.WriteOffEst==-1 ? 0 : x.WriteOffEst) : x.WriteOffEstOverride));
			patPort-=listProcClaimProcs
				.Where(x => x.Status.In(GetInsPaidStatuses()))
				.Sum(x => (decimal)x.WriteOff);
			#endregion
			#region Insurance Payments
			patPort-=listProcClaimProcs
				.Where(x => x.Status.In(GetEstimatedStatuses()))
				.Sum(x => (decimal)x.InsPayEst);
			patPort-=listProcClaimProcs
				.Where(x => x.Status.In(GetInsPaidStatuses()))
				.Sum(x => (decimal)x.InsPayAmt);
			#endregion
			...
			//
		{
			...
			if(plan.PlanType=="p" && feeSchedCopay!=null && feeSchedCopay.FeeSchedType==FeeScheduleType.FixedBenefit) {
				...
				if(feeFixedBenefit!=null && feeFixedBenefit.Amount.IsGreaterThan(0)) {
			...
			int patientAge,bool useProcDateOnProc)
		{
			...
			if(listProcClaimProcs.Count > 0) {//There exists 1 or 2 estimates for the current lab proc.
				listProcClaimProcs.ForEach(claimProcForLab => { 
					...
					loopList.AddRange(ClaimProcs.GetHistForProc(new List<ClaimProc> () {claimProcForLab},procLab.ProcNum,procLab.CodeNum));
