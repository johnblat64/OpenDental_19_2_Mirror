...
namespace OpenDentBusiness {
	...
	public class Patients {
		...
		public static List<PatAging> GetAgingList() {
			...
			using(DataTable table=Db.GetTable(command)) {
				foreach(DataRow row in table.Rows) {
					...
					dictAll[guarNum].HasUnsentProcs=PIn.Bool(row["HasUnsentProcs"].ToString());
				}
			}
			return dictAll.Values.ToList();
		}

		public static void SetDateBalBegan(ref List<PatAging> listPatAgingAll) {
		...
		public static DataTable GetPtDataTable(PtTableSearchParams ptSearchArgs)
		{
			...
			string phonedigits=new string(ptSearchArgs.Phone.Where(x=>char.IsDigit(x)).ToArray());
			string regexp="";
			...
			+(string.IsNullOrEmpty(ptSearchArgs.InvoiceNumber)?"'' ":"statement.")+$@"StatementNum
				FROM patient"
			+(!PrefC.GetBool(PrefName.DistributorKey)?"":$@"
				LEFT JOIN phonenumber ON phonenumber.PatNum=patient.PatNum{(string.IsNullOrEmpty(regexp)?"":$@"
			...
			foreach(DataRow dRow in table.Rows){
				...
				r["clinic"]=Clinics.GetAbbr(SIn.Long(dRow["ClinicNum"].ToString()));
			...
			List<long> listPendingInsPatNums,List<long> listUnsentPatNums,SerializableDictionary<long,List<PatAgingTransaction>> dictPatAgingTransactions)
		{
			...
			if(isSuperStatements) {
				...
				List<long> listSuperFamilyNumsNeeded=listSuperNums.FindAll(x => !x.In(listPatNums)); 
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				if(dateLastStatement.Date>=new[] { lastStatement.AddDays(1),maxDate }.Max().Date
		...
		public static List<PatComm> GetPatComms(List<long> patNums,Clinic clinic,bool isGetFamily=true,CommOptOutType optOutType=CommOptOutType.None) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<PatComm>>(MethodBase.GetCurrentMethod(),patNums,clinic,isGetFamily,optOutType);
			}
			...
			if(optOutType!=CommOptOutType.None) {
				Dictionary<long,List<CommOptOut>> dictOptOuts=CommOptOuts.GetForPats(listPatComms.Select(x => x.PatNum).ToList(),optOutType)
					.GroupBy(x => x.PatNum)
					.ToDictionary(x => x.Key,y => y.ToList());
				foreach(PatComm patComm in listPatComms) {
					if(dictOptOuts.ContainsKey(patComm.PatNum)) {
						patComm.ListCommOptOuts=dictOptOuts[patComm.PatNum];
					}
				}
			}
			return listPatComms;
		...
		public static List<Patient> GetPatsToChangeStatus(bool doSetStatusToPatient,DateTime fromDate,bool doIncludeTPProc
			,bool doIncludeCompletedProc,bool doIncludeAppointments,List<long> listClinicNums) 
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Patient>>(MethodBase.GetCurrentMethod(),doSetStatusToPatient,fromDate,doIncludeTPProc,doIncludeCompletedProc,
					doIncludeAppointments,listClinicNums);
			}
			string inOrNot=(doSetStatusToPatient?"IN":"NOT IN");
			string andOrNot=(doSetStatusToPatient?"OR":"AND");
			string activateOrDeactivate=(doSetStatusToPatient?"2":"0");//PatStatus.Inactive or PatStatus.Patient
			string whereClause="WHERE patstatus="+activateOrDeactivate+" AND (";
	...
	public class PatComm {
		...
		public List<CommOptOut> ListCommOptOuts=new List<CommOptOut>();
		...
		public string GetReasonCantText() {
	...
	public class PtTableSearchParams {
		...
		///<summary>Only for serialization purposes.  Private so it won't be used.  Use the constructor that takes all of the necessary params instead.</summary>
		private PtTableSearchParams() { }
		...
		public PtTableSearchParams(bool doLimit,string lname,string fname,string phone,string address,bool hideInactive,string city,
			...
			string subscriberId,string email,string country,string regKey,string clinicNums,string invoiceNumber,List<long> listExplicitPatNums=null,
