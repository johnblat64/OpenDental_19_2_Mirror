...
namespace OpenDentBusiness{
	...
	public class TaskLists {
		...
		///<summary>Gets all task lists from the database for a certain DateType.</summary>
		public static List<TaskList> GetAllForDateType(TaskDateType dateType) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<TaskList>>(MethodBase.GetCurrentMethod(),dateType);
			}
			string command="SELECT * FROM tasklist WHERE DateType="+POut.Long((int)dateType);
			return Crud.TaskListCrud.SelectMany(command);
		}
