...
namespace OpenDentBusiness {
	...
	public class Jobs {
		...
		///<summary>Gets the parent, siblings, and children of the passed in job.</summary>
		public static List<Job> GetJobsForOneLayer(Job job) {
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetObject<List<Job>>(MethodBase.GetCurrentMethod(),job);
			}
			if(job==null) {
				return new List<Job>();
			}
			string command=$@"SELECT * FROM job 
				WHERE (ParentNum={POut.Long(job.JobNum)/*Job is a child of current job*/}";
			if(job.ParentNum!=0) {
				command+=$@" OR ParentNum = { POut.Long(job.ParentNum)/*Job is a sibling of current job*/}
					OR JobNum = { POut.Long(job.ParentNum)/*Job is the parent of current job*/}";
			}
			command+=")";
		...
		public static List<Job> GetForTesting(string version,List<long> listExcludeJobNums=null) {
			...
			string command=$@"SELECT * FROM job
				...
				AND Category NOT IN ('{JobCategory.Query}','{JobCategory.Research}','{JobCategory.Conversion}') 
		...
		public static TreeNode GetJobTree(Job job,List<Job> listJobs) {
			...
			for(int i=0;i<jobHierarchy.Count;i++) {
				Job j=listJobs.FirstOrDefault(x => x.JobNum==jobHierarchy[i].ParentNum);
				if(j==null) {
					break;
				}
				jobHierarchy.Add(j);
