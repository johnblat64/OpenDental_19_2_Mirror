...
namespace OpenDentBusiness.UI {
	public class ApptDrawing {
		...
		public static float WeekDayWidth;
		...
		public static float WeekApptWidth;
		...
		private static Brush _brushTimeBar {
			get {
				return Brushes.LightGray;
			}
		}
		private static Brush _brushLinesAndText {
			get {
		...
		#endregion
		#region Pens
		private static Pen _penWeekPrimary {
			get {
				return Pens.DarkGray;
			}
		}
		private static Pen _penDayPrimary {
			get {
				return Pens.DarkSlateGray;
			}
		}
		private static Pen _penWeekMinutes {
			get {
				return Pens.White;
			}
		}
		private static Pen _penDayHour {
			get {
				return Pens.Black;
			}
		}
		private static Pen _penDaySecondary {
			get {
		...
		public static void DrawAllButAppts(Graphics g,bool showRedTimeLine,DateTime startTime,DateTime stopTime,int colsPerPage,int pageColumn,int fontSize,bool isPrinting) {
			...
			g.FillRectangle(_brushBackground,0,0,ApptSheetWidth,ApptSheetHeight);
			g.FillRectangle(_brushTimeBar,0,0,TimeWidth,ApptSheetHeight);//L time bar
			g.FillRectangle(_brushTimeBar,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,0,TimeWidth,ApptSheetHeight);//R time bar
			...
			if(IsWeeklyView) {
				for(int d=0;d<NumOfWeekDaysToDisplay;d++) {//for each day of the week displayed
					...
					for(int i=0;i<ColCount;i++) {//for each operatory visible for this view and day
						schedsForOp=Schedules.GetProvSchedsForOp(SchedListPeriod,dayofweek,VisOps[i]);
						...
						foreach(Schedule schedCur in schedsForOp.Where(x => x.SchedType==ScheduleType.Provider)) {
							g.FillRectangle(openBrush
								,TimeWidth+1+d*WeekDayWidth+(float)i*WeekApptWidth
				...
				for(int i=0;i<colsPerPage;i++) {//ops per page in day view
					...
					schedsForOp=Schedules.GetProvSchedsForOp(SchedListPeriod,VisOps[k]);
			...
			for(int i=0;i<schedForType.Length;i++) {
				...
				for(int o=0;o<schedForType[i].Ops.Count;o++) {
					...
					if(IsWeeklyView) {
						...
						rect=new RectangleF(
							TimeWidth+1+(dayofweek)*WeekDayWidth
							+WeekApptWidth*(GetIndexOp(schedForType[i].Ops[o],VisOps)-(colsPerPage*pageColumn))
					...
					using(SolidBrush solidBrushBlockText=new SolidBrush(colorBlockText)) {
						g.DrawString(blockText,blockFont,solidBrushBlockText,rect);
					}
				}
				blockBrush.Dispose();
				penOutline.Dispose();
				...
				for(int o=0;o<schedForType[i].Ops.Count;o++) {
					...
					if(IsWeeklyView) {
						...
						rect=new RectangleF(
							...
							,(schedForType[i].StartTime.Hours-startHour)*LineH*RowsPerHr
							+schedForType[i].StartTime.Minutes*LineH/MinPerRow
							,WeekApptWidth-5 //Shortened so that blockouts will be visible underneath
							,(schedForType[i].StopTime-schedForType[i].StartTime).Hours*LineH*RowsPerHr
							+(schedForType[i].StopTime-schedForType[i].StartTime).Minutes*LineH/MinPerRow);
					}
					else {//Daily view
					...
					g.DrawLine(Pens.Black,rect.X,rect.Y+1,rect.Right-1,rect.Y+1);
					FillWithDiagonalLines(g,rect,Pens.LightGray,12);
					using(SolidBrush solidBrushBlockText=new SolidBrush(colorBlockText)) {
						g.DrawString(blockText,blockFont,solidBrushBlockText,rect);
					}
				}
		...
		///<summary>Fills the rectangle with downward-sloping or upward-sloping diagonal lines.</summary>
		public static void FillWithDiagonalLines(Graphics g,RectangleF rectToFill,Pen linePen,int pixelsBetweenLines,bool isUpwardSloping=false) {
			if(isUpwardSloping) {
					...
					switch(ProvBar[j][i]) {
						...
						case 1:
							try {
								...
								g.FillRectangle(Brushes.White,TimeWidth+ProvWidth*j+1,((i-startingPoint)*LineH)+1,ProvWidth-1,LineH-1);
							}
							break;
						case 2:
							...
							break;
						default://more than 2
							g.FillRectangle(_brushLinesAndText,TimeWidth+ProvWidth*j+1,((i-startingPoint)*LineH)+1,ProvWidth-1,LineH-1);
							break;
					}
				}
			}
		...
		public static void DrawGridLines(Graphics g) {
			//Vert
			if(IsWeeklyView) {
				g.DrawLine(_penWeekPrimary,0,0,0,ApptSheetHeight);
				g.DrawLine(_penWeekMinutes,TimeWidth-1,0,TimeWidth-1,ApptSheetHeight);
				g.DrawLine(_penWeekPrimary,TimeWidth,0,TimeWidth,ApptSheetHeight);
				for(int d=0;d<NumOfWeekDaysToDisplay;d++) {
					g.DrawLine(_penWeekPrimary,TimeWidth+WeekDayWidth*d,0,TimeWidth+WeekDayWidth*d,ApptSheetHeight);
				}
				g.DrawLine(_penWeekPrimary,TimeWidth+WeekDayWidth*NumOfWeekDaysToDisplay,0,TimeWidth+1+WeekDayWidth*NumOfWeekDaysToDisplay,ApptSheetHeight);
				g.DrawLine(_penWeekPrimary,TimeWidth*2+WeekDayWidth*NumOfWeekDaysToDisplay,0,TimeWidth*2+1+WeekDayWidth*NumOfWeekDaysToDisplay,ApptSheetHeight);
			}
			else {
				g.DrawLine(_penWeekPrimary,0,0,0,ApptSheetHeight);
				g.DrawLine(_penWeekMinutes,TimeWidth-2,0,TimeWidth-2,ApptSheetHeight);
				g.DrawLine(_penWeekPrimary,TimeWidth-1,0,TimeWidth-1,ApptSheetHeight);
				for(int i=0;i<ProvCount;i++) {
					g.DrawLine(_penWeekPrimary,TimeWidth+ProvWidth*i,0,TimeWidth+ProvWidth*i,ApptSheetHeight);
				}
				for(int i=0;i<ColCount;i++) {
					g.DrawLine(_penWeekPrimary,TimeWidth+ProvWidth*ProvCount+ColWidth*i,0,TimeWidth+ProvWidth*ProvCount+ColWidth*i,ApptSheetHeight);
				}
				g.DrawLine(_penWeekPrimary,TimeWidth+ProvWidth*ProvCount+ColWidth*ColCount,0,TimeWidth+ProvWidth*ProvCount+ColWidth*ColCount,ApptSheetHeight);
				g.DrawLine(_penWeekPrimary,TimeWidth*2+ProvWidth*ProvCount+ColWidth*ColCount,0,TimeWidth*2+ProvWidth*ProvCount+ColWidth*ColCount,ApptSheetHeight);
			}
			//horiz gray
			for(int i=0;i<(ApptSheetHeight);i+=LineH*RowsPerIncr) {
				g.DrawLine(_penDaySecondary,TimeWidth,i,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,i);
			}
			//horiz Hour lines
			for(int i=0;i<ApptSheetHeight;i+=LineH*RowsPerHr) {
				g.DrawLine(_penDaySecondary,0,i-1,TimeWidth*2+ColWidth*ColCount+ProvWidth*ProvCount,i-1);
				g.DrawLine(_penDayPrimary,0,i,TimeWidth,i);
				g.DrawLine(_penDayHour,TimeWidth,i,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,i);
				g.DrawLine(_penDayPrimary,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,i,TimeWidth*2+ColWidth*ColCount+ProvWidth*ProvCount,i);
			}
		}
		...
		public static void DrawMinutes(Graphics g,DateTime startTime,DateTime stopTime) {
			...
			for(int i=startTime.Hour;i<stop;i++) {
				...
				g.DrawString(sTime,bfont,_brushLinesAndText,TimeWidth-sizef.Width-2,index*LineH*RowsPerHr+1);
				g.DrawString(sTime,bfont,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+1);
				if(MinPerIncr==5) {
					g.DrawString(":15",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*6);
					g.DrawString(":45",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*9);
					g.DrawString(":15",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*6);
					g.DrawString(":45",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*9);
				}
				else if(MinPerIncr==10) {
					g.DrawString(":10",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr);
					g.DrawString(":20",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*2);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
					g.DrawString(":40",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*4);
					g.DrawString(":50",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*5);
					g.DrawString(":10",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr);
					g.DrawString(":20",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*2);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
					g.DrawString(":40",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*4);
					g.DrawString(":50",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*5);
				}
				else {//15
					g.DrawString(":15",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*2);
					g.DrawString(":45",font,_brushLinesAndText,TimeWidth-19,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
					g.DrawString(":15",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr);
					g.DrawString(":30",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*2);
					g.DrawString(":45",font,_brushLinesAndText,TimeWidth+ColWidth*ColCount+ProvWidth*ProvCount,index*LineH*RowsPerHr+LineH*RowsPerIncr*3);
				}
				index++;
			}
		}
