...
namespace OpenDentBusiness.UI {
	public class ApptSingleDrawing {
		///<summary>The provider bar that is drawn on the appointment.
		///Warning, not every usage of the prov bar width is using this variable.</summary>
		private const int _provBarWidthSingle=7;
			...
			,int fontSize,bool isPrinting,List<DateRange> listOverlapTimes)
		{
			...
			for(int i=0;i<patternShowing.Length;i++) {//Info.MyApt.Pattern.Length;i++){
				if(patternShowing.Substring(i,1)=="X") {
					if(isPrinting) {
						g.FillRectangle(brushProvBackground,0,i*ApptDrawing.LineH,7,ApptDrawing.LineH);
					}
					else {
						g.FillRectangle(brushProvBackground,1,i*ApptDrawing.LineH+1,6,ApptDrawing.LineH);
					}
				}
				else {
					//leave empty
				...
				if(Math.IEEERemainder((double)i,(double)ApptDrawing.RowsPerIncr)==0) {//0/1
					if(isPrinting) {
						g.DrawLine(_penTimeDiv,0,i*ApptDrawing.LineH,7,i*ApptDrawing.LineH);
					}
					else {
						g.DrawLine(_penTimeDiv,1,i*ApptDrawing.LineH,6,i*ApptDrawing.LineH);
					}
				}
			}
			...
			foreach(DateRange overlapTimeRange in listOverlapTimes) {
				...
				RectangleF rect=new RectangleF(_provBarWidthSingle,startOverlapHeight,(int)totalWidth-_provBarWidthSingle,overlapHeight);
				ApptDrawing.FillWithDiagonalLines(g,rect,Pens.Gray,12,true);
			}
		...
		public static Point DrawElement(Graphics g,int elementI,Point drawLoc,ApptViewStackBehavior stackBehavior,ApptViewAlignment align,Brush backBrush,DataRow dataRoww,List<ApptViewItem> apptRows,DataTable tableApptFields,DataTable tablePatFields,float totalWidth,float totalHeight,int fontSize,bool isPrinting) {
			...
			if(apptRows[elementI].ApptFieldDefNum>0) {
				...
				for(int i=0;i<tablePatFields.Rows.Count;i++) {
					...
					case "Production":
						...
						break;
					case ApptViewItems.ItemNames.ProphyPerioPastDue:
						text=dataRoww["prophy/PerioPastDue[P]"].ToString();
						isRecallPastDue=true;
						break;
					case "Provider":
						if(isNote) {
							text="";
						...
						break;
					case ApptViewItems.ItemNames.RecallPastDue:
						//Only show the R if the appointment does not have the Prophy/PerioPastDue[P] field  or the Propy/PerioPastDue[P] is blank.
						if(!apptRows.Any(x => x.ElementDesc==ApptViewItems.ItemNames.ProphyPerioPastDue) 
							|| string.IsNullOrEmpty(dataRoww["prophy/PerioPastDue[P]"].ToString())) 
						{
			...
			if(align==ApptViewAlignment.Main) {//always stacks vertical
				if(isRecallPastDue) {
					...
					return DrawPastRecallDueImage(apptRows[elementI],g,drawLoc,align,stackBehavior);
				}
				if(isGraphic) {
		...
		///<summary>Draws the past recall image for either Propy/PerioPastDue[P] or RecallPastDue[R].</summary>
		private static Point DrawPastRecallDueImage(ApptViewItem apptViewItem,Graphics g,Point drawLoc,ApptViewAlignment align,
			ApptViewStackBehavior stackBehavior) 
		{
			//We don't need to check if the appointment view has both "Propy/PerioPastDue[P]" and "RecallPastDue[R]". This should have been done before calling this method.
			Bitmap bitmap;
			if(apptViewItem.ElementDesc==ApptViewItems.ItemNames.RecallPastDue) {
				bitmap=ChangeColor(Properties.Resources.RecallPastDue_R,apptViewItem.ElementColor);
			}
			else if(apptViewItem.ElementDesc==ApptViewItems.ItemNames.ProphyPerioPastDue) {
				bitmap=ChangeColor(Properties.Resources.RecallPastDue_P,apptViewItem.ElementColor);
			}
			else {
				//Don't draw the image if the passed in element is neither Propy/PerioPastDue[P] or RecallPastDue[R]
				return drawLoc;
			}
			SizeF imgSize=new SizeF(bitmap.Width,bitmap.Height);
			Point drawLocCur;
			Point retVal;//This will be set to the new location where we can draw on.
			if(align==ApptViewAlignment.Main) {
				drawLocCur=drawLoc;//No need to change the location. Draw image at current location. 
				retVal=new Point(drawLoc.X,drawLoc.Y+(int)imgSize.Height);//Set the new location
			}
			else if(align==ApptViewAlignment.UR) {
				drawLocCur=new Point(drawLoc.X-(int)imgSize.Width,drawLoc.Y+1);//upper left corner of this element
				if(stackBehavior==ApptViewStackBehavior.Vertical) {
					retVal=new Point(drawLoc.X,drawLoc.Y+(int)imgSize.Height);//Set the new location.
				}
				else {
					retVal=new Point(drawLoc.X-(int)imgSize.Width-2,drawLoc.Y);//Set the new location.
				}
			}
			else {//ApptViewAlignment.LR
				if(stackBehavior==ApptViewStackBehavior.Vertical) {
					drawLocCur=new Point(drawLoc.X-(int)imgSize.Width,drawLoc.Y+1-ApptDrawing.LineH);//upper left corner of this element
					retVal=new Point(drawLoc.X,drawLoc.Y-ApptDrawing.LineH);//Set the new location.Move up a certain number of lines for next element.
				}
				else {
					drawLocCur=new Point(drawLoc.X-(int)imgSize.Width+1,drawLoc.Y+1-ApptDrawing.LineH);//upper left corner of this element
					retVal=new Point(drawLoc.X-(int)imgSize.Width-1,drawLoc.Y);//Set the new location.Move to left.  Subtract a little from x to space out elements.
				}
			}
			g.DrawImage(bitmap,drawLocCur.X,drawLocCur.Y);
			DisposeObjects(bitmap);
			return retVal;
		}

		///<summary>Changes the color of the bitmap passed in to colorNew. Will leave bitmap's original color if the passed in color is Color.Empty.</summary>
		private static Bitmap ChangeColor(Bitmap bitmapOld, Color colorNew) {
			//This is needed because for some reason the image looked bigger when the original bitmapOld was used instead of creating a new one.
			bool isNewColorEmpty=colorNew.ToArgb()==Color.Empty.ToArgb();
			//Make an empty bitmap the same size as bitmapOld
			Bitmap bitmapNew=new Bitmap(bitmapOld.Width, bitmapOld.Height);
			for(int i=0;i<bitmapOld.Width;i++) {
				for(int j=0;j<bitmapOld.Height;j++) {
					//Get the pixel from the bitmapOld image
					Color colorOld=bitmapOld.GetPixel(i,j);
					if(colorOld.A<=0 || isNewColorEmpty) { 
						bitmapNew.SetPixel(i,j,colorOld);
					}
					else {
						//Create a new color using the colorOld Alpha. 
						Color colorToUse=Color.FromArgb(colorOld.A,colorNew.R,colorNew.G,colorNew.B);
						bitmapNew.SetPixel(i,j,colorToUse);
					}
				}
			}
			//Dispose the old bitmap
			DisposeObjects(bitmapOld);
			return bitmapNew;
		}
		...
		public static Point SetLocation(DataRow dataRoww,int beginHour,int colsPerPage,int pageColumn) {
			if(ApptDrawing.IsWeeklyView) {
				ApptSingleWidth=(int)ApptDrawing.WeekApptWidth;
		...
		public static int ConvertToX(DataRow dataRoww,int colsPerPage,int pageColumn) {
			if(ApptDrawing.IsWeeklyView) {
				...
				return (int)(ApptDrawing.TimeWidth
					+ApptDrawing.WeekDayWidth*(dayofweek)+1
					+(ApptDrawing.WeekApptWidth*(ApptDrawing.GetIndexOp(PIn.Long(dataRoww["Op"].ToString()))-(colsPerPage*pageColumn))));
			}
			else {
