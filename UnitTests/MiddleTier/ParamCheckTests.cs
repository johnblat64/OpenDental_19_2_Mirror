	...
	public class ParamCheckTests:TestBase {
		...
		public void MiddleTier_ParametersTest() {
			...
			foreach(MethodDefinition method in listAllMethods) {
				...
				if(methodCallInstructionMeth==null) {
					...
					if(!method.DeclaringType.Name.ToLower().In(listSClassNames)) {
						continue;
					}
			...
			if(strBuilderNoRemotingRolesCheck.Length!=0) {
				Console.WriteLine("The following methods call the database (crud/db/datacore) without a remoting roles check.");
				Console.WriteLine(strBuilderNoRemotingRolesCheck.ToString());
			}
			if(strBuilderNoGetCurrentMethod.Length!=0) {
				Console.WriteLine("In the following methods, there was no call to MethodBase.GetCurrentMethod().");
				Console.WriteLine(strBuilderNoGetCurrentMethod.ToString());
			}
			if(strBuilderIncorrectParameters.Length!=0) {
				Console.WriteLine("In the following methods, there was a mismatching number of middle tier parameters.");
				Console.WriteLine(strBuilderIncorrectParameters.ToString());
			}
			Assert.AreEqual(0,strBuilderNoRemotingRolesCheck.Length);
			Assert.AreEqual(0,strBuilderNoGetCurrentMethod.Length);
			Assert.AreEqual(0,strBuilderIncorrectParameters.Length);
		}
		...
		public void MiddleTier_ReturnAfterVoid() {
			...
			List<MethodDefinition> listAllSClassMethods=asm.Modules
				.SelectMany(x => x.Types)
				.Where(x => x.Name.ToLower().In(listSClassNames))
				.SelectMany(x => x.Methods)
		...
		private static Dictionary<MethodDefinition,List<MethodDefinition>> FillDictOfMethodReferences(List<TypeDefinition> listSClassTypes) {
			...
			foreach(MethodDefinition method in listMethods) {
				...
				foreach(Instruction instruc in listInstructions) {
					...
					MethodDefinition methodDefBeingCalled=methodRefBeingCalled.Resolve();
