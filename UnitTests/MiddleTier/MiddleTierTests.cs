	...
	public class MiddleTierTests:TestBase {
		...
		public void MiddleTier_GetPrimaryKeysFromInsert() {
			...
			foreach(MethodInfo insertMethod in listInsertMethods) {
				...
				else if(insertMethod.DeclaringType.Name.In("Schedules","Signalods")) {//Bulk inserts. Does not return or set primary keys. Skip
					continue;
				}
				#endregion
				#region HQ Only Skip
				else if(insertMethod.DeclaringType.Name.In("Jobs","SiteLinks","BugSubmissions")) {//HQ only. Skip because they require bugs db.
					continue;
				}
				#endregion
				#endregion
		...
		private void AppendError(MethodInfo insertMethod,StringBuilder strBuilderErrors,int countFailure,Exception ex=null) {
			...
			string error=(ex==null ? "Insert method failed to assign primary key " : "Exception occured ");
