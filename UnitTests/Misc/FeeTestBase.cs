...
namespace UnitTests {
	...
	public abstract class FeeTestBase:TestBase {
		...
		///<summary>Primarily for fee import/copy. Returns true if we can retrieve all of the listActual fees from the feecache and all of the fee amounts
		///in the cache match those in the list.</summary>
		protected bool DoAmountsMatch(List<Fee> listActual, long feeSchedNum, long clinicNum, long provNum) {
			if(PrefC.FeesUseCache) {
				return DoAmountsMatchCached(listActual,feeSchedNum,clinicNum,provNum);
			}
		...
		private bool DoAmountsMatchCached(List<Fee> listActual,long feeSchedNum,long clinicNum,long provNum) {
			FeeCache cache=new FeeCache();
			for(int i=0;i<listActual.Count;i++) {
				Fee fee=listActual[i];
				Fee originalFee=cache.GetFee(fee.CodeNum,feeSchedNum,fee.ClinicNum,fee.ProvNum);
				try {
					if(originalFee.Amount!=fee.Amount) {
						return false;
					}
