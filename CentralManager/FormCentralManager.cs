...
namespace CentralManager {
	public partial class FormCentralManager:Form {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(conn.ConnectionStatus!="OK" && !checkDynamicMode.Checked) {
...
#region Menu Setup
		...
		private void menuConnSetup_Click(object sender,EventArgs e) {
			...
			FormCC.LabelText.Text=Lans.g("FormCentralConnections","Double click an existing connection to edit or click the 'Add' button to add a new connection."+
				"\r\nUse the alphabetize button to re-order all items in the grid alphabetically based on the connection note.");
...
#region Menu Reports
		...
		private void menuProdInc_Click(object sender,EventArgs e) {
			...
			if(!_listReportPermissions.Exists(x => x.FKey==_listProdInc.FirstOrDefault(y => 
		...
		private void RefreshConnections(bool doRefreshAll=false) {
			Cursor=Cursors.WaitCursor;
			if(gridMain.SelectedIndices.Length==0 || doRefreshAll) {
		...
		private void butSecurity_Click(object sender,EventArgs e) {
			...
			CodeBase.MsgBoxCopyPaste MsgBoxCopyPaste=new CodeBase.MsgBoxCopyPaste(CentralSyncHelper.SyncAll(listSelectedConns));
		...
		private void butUsers_Click(object sender,EventArgs e) {
			...
			CodeBase.MsgBoxCopyPaste MsgBoxCopyPaste=new CodeBase.MsgBoxCopyPaste(CentralSyncHelper.SyncUsers(listSelectedConns));
			MsgBoxCopyPaste.ShowDialog();
			FillGrid();
		}
		...
		private void butLocks_Click(object sender,EventArgs e) {
			...
			CodeBase.MsgBoxCopyPaste MsgBoxCopyPaste=new CodeBase.MsgBoxCopyPaste(CentralSyncHelper.SyncLocks(listSelectedConns));
			MsgBoxCopyPaste.ShowDialog();
			FillGrid();
		}
		...
		private void butPtSearch_Click(object sender,EventArgs e) {
			...
			FormCPS.UseDynamicMode=checkDynamicMode.Checked;
			FormCPS.ShowDialog();
			FillGrid();
		}

		private void menuTransferPatient_Click(object sender,EventArgs e) {
			//At least one database should be selected
			if(gridMain.SelectedIndices.Count()!=1) {
				MsgBox.Show(this,"Please select one and only one connection to start transfer.");
				return;
			}
			long selectedCentralConnNum=gridMain.SelectedTag<CentralConnection>().CentralConnectionNum;
			RefreshConnections(true);
			//Only one database was selected. Continue on with transfer.
			CentralConnection sourceConnection=gridMain.GetTags<CentralConnection>().FirstOrDefault(x => x.CentralConnectionNum==selectedCentralConnNum);
			//Db must be current.
			if(!sourceConnection.IsConnectionValid()) {
				MsgBox.Show(this,"Server Offline.  Fix connection and check status again to connect.");
				return;
			}
			FormCentralPatientTransfer FormCentralConnectionPatientTransfer=new FormCentralPatientTransfer(sourceConnection,
				_listConns.Where(x=>x.IsConnectionValid()).ToList());
