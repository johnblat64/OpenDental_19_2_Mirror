...
namespace CentralManager {
	public partial class FormCentralPatientSearch:Form {
		...
		public DataRow SelectedPatient;
		...
		private void FormCentralPatientSearch_Load(object sender,System.EventArgs e) {
			...
			_invalidConnsLog="";
			if(IsSelectionMode) {
				gridMain.Title="Double Click to Select Patient";
			}
		...
		private void FillGrid() {
			...
			for(int i=0;i<_dataConnPats.Tables.Count;i++) {
				for(int j=0;j<_dataConnPats.Tables[i].Rows.Count;j++) {
					...
					CentralConnection conn=ListConns.Find(x => ((x.ServerName+", "+x.DatabaseName)==_dataConnPats.Tables[i].TableName || x.ServiceURI==_dataConnPats.Tables[i].TableName));
					row.Tag=new ODGridRowTag(conn,_dataConnPats.Tables[i].Rows[j]);
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			ODGridRowTag gridRowSelectedTag=(ODGridRowTag)gridMain.Rows[e.Row].Tag;
			if(IsSelectionMode) {
				SelectedPatient=gridRowSelectedTag.DataRow;
				DialogResult=DialogResult.OK;
			}
			else {//not selection mode. Launch OD
				CentralConnectionHelper.LaunchOpenDental(gridRowSelectedTag.CentralConnection,UseDynamicMode,AutoLogIn,
					gridRowSelectedTag.DataRow["PatNum"].ToString());
			}
		}
	...
	public class ODGridRowTag {

		public CentralConnection CentralConnection;
		public DataRow DataRow;

		public ODGridRowTag(CentralConnection centralConn,DataRow row) {
