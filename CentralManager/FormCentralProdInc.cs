...
namespace CentralManager {
	public partial class FormCentralProdInc:Form {
		...
		private void RunMonthly() {
			...
			query.AddGroupSummaryField("Total Income (Pt Income + Ins Income): ","Writeoff","Total Income",SummaryOperation.Sum,
		...
		private void RunProvider() {
			...
			if(strFailedConn!="") {
				CodeBase.MsgBoxCopyPaste msgBoxCP=new CodeBase.MsgBoxCopyPaste(strFailedConn);
				msgBoxCP.ShowDialog();
			}
			// display the report
			FormReportComplex FormR=new FormReportComplex(report);
