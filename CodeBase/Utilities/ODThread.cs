...
namespace CodeBase {
	...
	public class ODThread {
		...
		public static void RegisterForUnhandledExceptions(Control ctrlMainThread=null,Action<Exception,String> onException=null) {
			OnUnhandledException=new Action<Exception,Thread>((e,th) => {
				if(ctrlMainThread==null) {
					onException?.Invoke(e,th.Name);
					return;
				}
